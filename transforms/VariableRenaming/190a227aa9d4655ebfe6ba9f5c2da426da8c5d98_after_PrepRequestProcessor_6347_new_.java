            ...
            case OpCode.deleteContainer:
                {
                    ...
                    String var7 = getParentPathAndValidate(path);
                    ...
                    ChangeRecord parentRecord = getRecordForPath(var7);
                    request.setTxn(new DeleteTxn(path));
                    parentRecord = parentRecord.duplicate(request.getHdr().getZxid());
                    parentRecord.childCount--;
                    parentRecord.stat.setPzxid(request.getHdr().getZxid());
                    parentRecord.precalculatedDigest = precalculateDigest(DigestOpCode.UPDATE, var7, parentRecord.data, parentRecord.stat);
            ...
            case OpCode.closeSession:
                ...
                synchronized (zks.outstandingChanges) {
                    ...
                    for (String path2Delete : es) {
                        if (digestEnabled) {
                            var7 = getParentPathAndValidate(path2Delete);
                            parentRecord = getRecordForPath(var7);
                            parentRecord = parentRecord.duplicate(request.getHdr().getZxid());
                            parentRecord.stat.setPzxid(request.getHdr().getZxid());
                            parentRecord.precalculatedDigest = precalculateDigest(DigestOpCode.UPDATE, var7, parentRecord.data, parentRecord.stat);
                            addChangeRecord(parentRecord);
