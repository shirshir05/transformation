...
public class Login {
            ...
            public void run() {
                ...
                while (true) {
                    ...
                    if (tgt == null) {
                        ...
                        if ((nextRefresh > expiry) || ((now + MIN_TIME_BEFORE_RELOGIN) > expiry)) {
                            ...
                            if (nextRefresh < (now + MIN_TIME_BEFORE_RELOGIN)) {
                                ...
                                LOG.warn("TGT refresh thread time adjusted from : {} to : {} since " + "the former is sooner than the minimum refresh interval (" + "{} seconds) from now.", logPayload);
                            }
                    ...
                    if (now < nextRefresh) {
                        ...
                        LOG.error("nextRefresh:{} is in the past: exiting refresh thread. Check" + " clock sync between this host and KDC - (KDC's clock is likely ahead of this host)." + " Manual intervention will be required for this client to successfully authenticate." + " Exiting refresh thread.", nextRefreshDate);
                        return;
                    }
    ...
    private long getRefreshTime(KerberosTicket var0) {
        long start = var0.getStartTime().getTime();
        long expires = var0.getEndTime().getTime();
        LOG.info("TGT valid starting at:        {}", var0.getStartTime().toString());
        LOG.info("TGT expires:                  {}", var0.getEndTime().toString());
    ...
    private boolean hasSufficientTimeElapsed() {
        ...
        if (now - getLastLogin() < MIN_TIME_BEFORE_RELOGIN) {
            LOG.warn("Not attempting to re-login since the last re-login was " + "attempted less than {} seconds before.", (MIN_TIME_BEFORE_RELOGIN / 1000));
            return false;
        }
