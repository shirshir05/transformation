    ...
    protected void pRequest2Txn(int type, long zxid, Request request, Record record, boolean deserialize) throws KeeperException, IOException, RequestProcessorException {
        ...
        switch(type) {
            ...
            case OpCode.deleteContainer:
                {
                    ...
                    ChangeRecord var13 = getRecordForPath(path);
                    if (var13.childCount > 0) {
                        throw new KeeperException.NotEmptyException(path);
                    }
                    if (EphemeralType.get(var13.stat.getEphemeralOwner()) == EphemeralType.NORMAL) {
            ...
            case OpCode.delete:
                ...
                checkAndIncVersion(var13.stat.getVersion(), deleteRequest.getVersion(), path);
                if (var13.childCount > 0) {
                    throw new KeeperException.NotEmptyException(path);
                }
                request.setTxn(new DeleteTxn(path));
            ...
            case OpCode.setData:
                ...
                var13 = getRecordForPath(path);
                checkACL(zks, var13.acl, ZooDefs.Perms.WRITE, request.authInfo);
                int newVersion = checkAndIncVersion(var13.stat.getVersion(), setDataRequest.getVersion(), path);
                request.setTxn(new SetDataTxn(path, setDataRequest.getData(), newVersion));
                var13 = var13.duplicate(request.getHdr().getZxid());
                var13.stat.setVersion(newVersion);
                addChangeRecord(var13);
                break;
            ...
            case OpCode.setACL:
                ...
                var13 = getRecordForPath(path);
                checkACL(zks, var13.acl, ZooDefs.Perms.ADMIN, request.authInfo);
                newVersion = checkAndIncVersion(var13.stat.getAversion(), setAclRequest.getVersion(), path);
                request.setTxn(new SetACLTxn(path, listACL, newVersion));
                var13 = var13.duplicate(request.getHdr().getZxid());
                var13.stat.setAversion(newVersion);
                addChangeRecord(var13);
                break;
            ...
            case OpCode.check:
                ...
                validatePath(path, request.sessionId);
                var13 = getRecordForPath(path);
                checkACL(zks, var13.acl, ZooDefs.Perms.READ, request.authInfo);
                request.setTxn(new CheckVersionTxn(path, checkAndIncVersion(var13.stat.getVersion(), checkVersionRequest.getVersion(), path)));
                break;
            default:
