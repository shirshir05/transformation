...
class QuorumPeerInstance implements Instance {
    ...
    public void configure(String params) {
        if (clientAddr == null) {
            String[] var16 = params.split(" ");
            // us which machine we are
            serverId = Integer.parseInt(var16[0]);
            if (LOG.isDebugEnabled()) {
                LOG.debug("Setting up server " + serverId);
            }
            if (var16.length > 1 && var16[1].equals("false")) {
            ...
            String[] var16 = quorumSpecs.split(",");
            peers = new HashMap<Long, QuorumServer>();
            for (int i = 0; i < var16.length; i++) {
                // parts[i] == "host:leaderPort:leaderElectionPort;clientPort"
                String[] subparts = (var16[i].split(";"))[0].split(":");
                String clientPort = (var16[i].split(";"))[1];
