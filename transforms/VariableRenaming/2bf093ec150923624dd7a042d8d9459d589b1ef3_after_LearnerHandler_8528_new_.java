            ...
            try {
                ...
                if (proposals.size() != 0) {
                    if ((maxCommittedLog >= peerLastZxid) && (minCommittedLog <= peerLastZxid)) {
                        ...
                        for (Proposal var12 : proposals) {
                            if (var12.packet.getZxid() > peerLastZxid) {
                                queuePacket(var12.packet);
                                QuorumPacket qcommit = new QuorumPacket(Leader.COMMIT, var12.packet.getZxid(), null, null);
                                queuePacket(qcommit);
                            }
                        }
