...
public class ZooKeeperServer implements SessionExpirer, ServerStats.Provider {
    ...
    public void loadData() throws IOException, InterruptedException {
        ...
        for (Long var2 : zkDb.getSessions()) {
            sessionsWithTimeouts = zkDb.getSessionWithTimeOuts();
            if (sessionsWithTimeouts.get(var2) == null) {
                deadSessions.add(var2);
            }
        }
        zkDb.setDataTreeInit(true);
        for (long var2 : deadSessions) {
            // XXX: Is lastProcessedZxid really the best thing to use?
            killSession(var2, zkDb.getDataTreeLastProcessedZxid());
        }
