...
public class NIOServerCnxn implements Watcher, ServerCnxn {
    ...
    static public class Factory extends Thread {
        ...
        synchronized public void clear() {
            ...
            synchronized (cnxns) {
                ...
                for (Iterator<NIOServerCnxn> it = cnxns.iterator(); it.hasNext(); ) {
    ...
    private void readConnectRequest() throws IOException, InterruptedException {
        ...
        if (connReq.getLastZxidSeen() > zk.dataTree.lastProcessedZxid) {
            String msg = "Refusing session request for client " + sock.socket().getRemoteSocketAddress() + " as it has seen zxid 0x" + Long.toHexString(connReq.getLastZxidSeen()) + " our last zxid is 0x" + Long.toHexString(zk.dataTree.lastProcessedZxid) + " client must try another server";
            LOG.info(msg);
    ...
    private boolean checkFourLetterWord(SelectionKey k, int len) throws IOException {
        ...
        if (len == ruokCmd) {
            ...
            LOG.info("Processing " + (len == statCmd ? "stat" : "srvr") + " command from " + sock.socket().getRemoteSocketAddress());
            packetReceived();
            StringBuffer sb = new StringBuffer();
            if (zk != null) {
            ...
            LOG.info("Processing cons command from " + sock.socket().getRemoteSocketAddress());
            packetReceived();
            StringBuffer sb = new StringBuffer();
            if (zk != null) {
                synchronized (factory.cnxns) {
            ...
            LOG.info("Processing crst command from " + sock.socket().getRemoteSocketAddress());
            packetReceived();
            synchronized (factory.cnxns) {
                for (NIOServerCnxn c : factory.cnxns) {
                    c.getStats().reset();
    ...
    public void close() {
        ...
        try {
            ...
            LOG.warn("Failed to unregister with JMX", var6);
        }
        ...
        try {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("ignoring exception during output shutdown", var6);
            }
        }
        try {
            ...
            // This is a relatively common exception that we can't avoid
            if (LOG.isDebugEnabled()) {
                LOG.debug("ignoring exception during input shutdown", var6);
            }
        }
        try {
            sock.socket().close();
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("ignoring exception during socket close", var6);
            }
        }
        try {
            sock.close();
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("ignoring exception during socketchannel close", var6);
            }
        }
        sock = null;
        if (sk != null) {
            try {
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.debug("ignoring exception during selectionkey cancel", var6);
                }
            }
        }
    }
    ...
    synchronized public void sendResponse(ReplyHeader h, Record r, String tag) {
        try {
            if (closed) {
                if (LOG.isTraceEnabled()) {
                    LOG.trace("send called on closed session 0x" + Long.toHexString(sessionId) + " with record " + r);
                }
                return;
            }
    ...
    public void finishSessionInit(boolean valid) {
        ...
        try {
            ...
            if (!valid) {
                ...
                LOG.info("Established session 0x" + Long.toHexString(sessionId) + " for client " + sock.socket().getRemoteSocketAddress());
            }
    ...
    class CnxnStats implements ServerCnxn.Stats {
        ...
        public String toString(boolean brief) {
            ...
            if (channel instanceof SocketChannel) {
                sb.append(" ").append(((SocketChannel) channel).socket().getRemoteSocketAddress()).append("[").append(Integer.toHexString(sk.interestOps())).append("](queued=").append(getOutstandingRequests()).append(",recved=").append(getPacketsReceived()).append(",sent=").append(getPacketsSent());
                if (!brief) {
                    long sessionId = getSessionId();
                    if (sessionId != 0) {
                        sb.append(",sid=0x").append(Long.toHexString(sessionId)).append(",lop=").append(getLastOperation()).append(",est=").append(getEstablished().getTime()).append(",to=").append(getSessionTimeout());
                        long lastCxid = getLastCxid();
                        if (lastCxid >= 0) {
                            sb.append(",lcxid=0x").append(Long.toHexString(lastCxid));
                        }
                        sb.append(",lzxid=0x").append(Long.toHexString(getLastZxid())).append(",lresp=").append(getLastResponseTime()).append(",llat=").append(getLastLatency()).append(",minlat=").append(getMinLatency()).append(",avglat=").append(getAvgLatency()).append(",maxlat=").append(getMaxLatency());
                    }
                }
                sb.append(")\n");
            }
