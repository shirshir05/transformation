...
public class NIOServerCnxn implements Watcher, ServerCnxn {
    ...
    void doIO(SelectionKey k) throws InterruptedException {
        try {
            ...
            LOG.warn("Exception causing close of session 0x" + Long.toHexString(sessionId) + " due to " + var12);
            if (LOG.isDebugEnabled()) {
                LOG.debug("CancelledKeyException stack trace", var12);
            }
            close();
        } catch (CloseRequestException var12) {
            // expecting close to log session closure
            close();
        } catch (EndOfStreamException var12) {
            // tell user why
            LOG.warn(var12);
            // expecting close to log session closure
            close();
            ...
            LOG.warn("Exception causing close of session 0x" + Long.toHexString(sessionId) + " due to " + var12);
            if (LOG.isDebugEnabled()) {
                LOG.debug("IOException stack trace", var12);
            }
            close();
        }
    }
