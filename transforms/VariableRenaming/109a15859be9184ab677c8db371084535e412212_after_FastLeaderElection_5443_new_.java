...
public class FastLeaderElection implements Election {
    ...
    public Vote lookForLeader() throws InterruptedException {
        ...
        while (self.getPeerState() == ServerState.LOOKING) {
             ...
             */
            Notification var2 = recvqueue.poll(2 * finalizeWait, TimeUnit.MILLISECONDS);
            /*
            ...
            if (var2 == null) {
                ...
                switch(var2.state) {
                    case LOOKING:
                        ...
                        LOG.info("Notification: " + var2.leader + ", " + var2.zxid + ", " + var2.epoch + ", " + self.getId() + ", " + self.getPeerState() + ", " + var2.state + ", " + var2.sid);
                        if (var2.epoch > logicalclock) {
                            logicalclock = var2.epoch;
                            recvset.clear();
                            updateProposal(self.getId(), self.getLastLoggedZxid());
                            sendNotifications();
                        } else if (var2.epoch < logicalclock) {
                            break;
                        } else if (totalOrderPredicate(var2.leader, var2.zxid)) {
                            updateProposal(var2.leader, var2.zxid);
                            sendNotifications();
                        }
                        recvset.put(var2.sid, new Vote(var2.leader, var2.zxid, var2.epoch));
                        ...
                        if (self.quorumPeers.size() == recvset.size()) {
                            ...
                            while ((var2 = recvqueue.poll(finalizeWait, TimeUnit.MILLISECONDS)) != null) {
                                if (totalOrderPredicate(var2.leader, var2.zxid)) {
                                    recvqueue.put(var2);
                                    break;
                                }
                            }
                    ...
                    case FOLLOWING:
                        LOG.info("Notification: " + var2.leader + ", " + var2.zxid + ", " + var2.epoch + ", " + self.getId() + ", " + self.getPeerState() + ", " + var2.state + ", " + var2.sid);
                        outofelection.put(var2.sid, new Vote(var2.leader, var2.zxid, var2.epoch, var2.state));
                        if (termPredicate(outofelection, new Vote(var2.leader, var2.zxid, var2.epoch, var2.state)) && checkLeader(outofelection, var2.leader, var2.epoch)) {
                            synchronized (this) {
                                logicalclock = var2.epoch;
                                self.setPeerState((var2.leader == self.getId()) ? ServerState.LEADING : ServerState.FOLLOWING);
                            }
                            leaveInstance();
                            return new Vote(var2.leader, var2.zxid);
                        }
                        break;
                    default:
                        break;
