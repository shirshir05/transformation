    ...
    private boolean fixupACL(List<Id> authInfo, List<ACL> acl) {
        ...
        while (it.hasNext()) {
            ...
            if (id.getScheme().equals("world") && id.getId().equals("anyone")) {
                ...
                for (Id var7 : authInfo) {
                    AuthenticationProvider ap = ProviderRegistry.getProvider(var7.getScheme());
                    if (ap == null) {
                        LOG.error("Missing AuthenticationProvider for " + var7.getScheme());
                    } else if (ap.isAuthenticated()) {
                        authIdValid = true;
                        toAdd.add(new ACL(a.getPerms(), var7));
                    }
                }
                if (!authIdValid) {
                    return false;
