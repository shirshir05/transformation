...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    public static class QuorumServer {
        ...
        private List<InetSocketAddress> excludedSpecialAddresses(List<InetSocketAddress> addrs) {
            ...
            for (InetSocketAddress addr : addrs) {
                ...
                if (inetaddr == null || // wildCard address(0.0.0.0)
                inetaddr.equals(wcAddr) || inetaddr.isLoopbackAddress()) {
    ...
    public QuorumPeer(Map<Long, QuorumServer> quorumPeers, File snapDir, File logDir, int clientPort, int electionAlg, long myid, int tickTime, int initLimit, int syncLimit) throws IOException {
        this(quorumPeers, snapDir, logDir, electionAlg, myid, tickTime, initLimit, syncLimit, false, ServerCnxnFactory.createFactory(getClientAddress(quorumPeers, myid, clientPort), -1), new QuorumMaj(quorumPeers));
    }
    ...
    public QuorumPeer(Map<Long, QuorumServer> quorumPeers, File snapDir, File logDir, int clientPort, int electionAlg, long myid, int tickTime, int initLimit, int syncLimit, QuorumVerifier quorumConfig) throws IOException {
        this(quorumPeers, snapDir, logDir, electionAlg, myid, tickTime, initLimit, syncLimit, false, ServerCnxnFactory.createFactory(getClientAddress(quorumPeers, myid, clientPort), -1), quorumConfig);
    }
    ...
    public void setLastSeenQuorumVerifier(QuorumVerifier var0, boolean writeToDisk) {
        synchronized (QV_LOCK) {
            if (lastSeenQuorumVerifier != null && lastSeenQuorumVerifier.getVersion() > var0.getVersion()) {
                LOG.error("setLastSeenQuorumVerifier called with stale config " + var0.getVersion() + ". Current version: " + quorumVerifier.getVersion());
            }
            // version is the same, nothing to do here.
            if (lastSeenQuorumVerifier != null && lastSeenQuorumVerifier.getVersion() == var0.getVersion()) {
                return;
            }
            lastSeenQuorumVerifier = var0;
            connectNewPeers();
            if (writeToDisk) {
                try {
                    String fileName = getNextDynamicConfigFilename();
                    if (fileName != null) {
                        QuorumPeerConfig.writeDynamicConfig(fileName, var0, true);
                    }
                } catch (IOException e) {
