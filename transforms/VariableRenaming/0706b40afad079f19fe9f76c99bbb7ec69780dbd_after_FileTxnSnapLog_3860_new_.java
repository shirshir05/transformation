    ...
    public long restore(DataTree dt, Map<Long, Integer> sessions, PlayBackListener listener) throws IOException {
        ...
        try {
            while (true) {
                ...
                if (var9.getZxid() < highestZxid && highestZxid != 0) {
                    LOG.error("{}(highestZxid) > {}(next log) for type {}", highestZxid, var9.getZxid(), var9.getType());
                } else {
                    highestZxid = var9.getZxid();
                }
                try {
                    processTransaction(var9, dt, sessions, itr.getTxn());
                } catch (KeeperException.NoNodeException e) {
                    throw new IOException("Failed to process transaction type: " + var9.getType() + " error: " + e.getMessage(), e);
                }
                listener.onTxnLoaded(var9, itr.getTxn());
                if (!itr.next())
                    break;
            }
        } finally {
