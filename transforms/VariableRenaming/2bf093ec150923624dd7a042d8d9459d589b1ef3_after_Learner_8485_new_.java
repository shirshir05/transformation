            ...
            outerLoop: while (self.isRunning()) {
                ...
                switch(qp.getType()) {
                    case Leader.PROPOSAL:
                        PacketInFlight var7 = new PacketInFlight();
                        var7.hdr = new TxnHeader();
                        BinaryInputArchive ia = BinaryInputArchive.getArchive(new ByteArrayInputStream(qp.getData()));
                        var7.rec = SerializeUtils.deserializeTxn(ia, var7.hdr);
                        if (var7.hdr.getZxid() != lastQueued + 1) {
                            LOG.warn("Got zxid 0x" + Long.toHexString(var7.hdr.getZxid()) + " expected 0x" + Long.toHexString(lastQueued + 1));
                        }
                        lastQueued = var7.hdr.getZxid();
                        packetsNotCommitted.add(var7);
                        break;
                    case Leader.COMMIT:
                        var7 = packetsNotCommitted.peekFirst();
                        if (var7.hdr.getZxid() != qp.getZxid()) {
                            LOG.warn("Committing " + qp.getZxid() + ", but next proposal is " + var7.hdr.getZxid());
                        } else {
                            zk.getZKDatabase().processTxn(var7.hdr, var7.rec);
