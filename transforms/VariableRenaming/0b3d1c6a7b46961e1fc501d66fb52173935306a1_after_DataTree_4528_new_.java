...
public class DataTree {
    ...
    public ProcessTxnResult processTxn(TxnHeader header, Record txn) {
        ProcessTxnResult var2 = new ProcessTxnResult();
        String debug = "";
        try {
            var2.clientId = header.getClientId();
            var2.cxid = header.getCxid();
            var2.zxid = header.getZxid();
            var2.type = header.getType();
            var2.err = 0;
            var2.multiResult = null;
            if (var2.zxid > lastProcessedZxid) {
                lastProcessedZxid = var2.zxid;
            }
            switch(header.getType()) {
                case OpCode.create:
                    CreateTxn createTxn = (CreateTxn) txn;
                    debug = "Create transaction for " + createTxn.getPath();
                    var2.path = createTxn.getPath();
                    createNode(createTxn.getPath(), createTxn.getData(), createTxn.getAcl(), createTxn.getEphemeral() ? header.getClientId() : 0, createTxn.getParentCVersion(), header.getZxid(), header.getTime());
                    break;
                case OpCode.delete:
                    DeleteTxn deleteTxn = (DeleteTxn) txn;
                    debug = "Delete transaction for " + deleteTxn.getPath();
                    var2.path = deleteTxn.getPath();
                    deleteNode(deleteTxn.getPath(), header.getZxid());
                    break;
                case OpCode.setData:
                    SetDataTxn setDataTxn = (SetDataTxn) txn;
                    debug = "Set data transaction for " + setDataTxn.getPath() + " to new value=" + Arrays.toString(setDataTxn.getData());
                    var2.stat = setData(setDataTxn.getPath(), setDataTxn.getData(), setDataTxn.getVersion(), header.getZxid(), header.getTime());
                    break;
                case OpCode.setACL:
                    SetACLTxn setACLTxn = (SetACLTxn) txn;
                    debug = "Set ACL transaction for " + setACLTxn.getPath();
                    var2.stat = setACL(setACLTxn.getPath(), setACLTxn.getAcl(), setACLTxn.getVersion());
                    break;
                ...
                case OpCode.error:
                    ...
                    var2.err = errTxn.getErr();
                    break;
                case OpCode.check:
                    CheckVersionTxn checkTxn = (CheckVersionTxn) txn;
                    debug = "Check Version transaction for " + checkTxn.getPath() + " and version=" + checkTxn.getVersion();
                    var2.path = checkTxn.getPath();
                    break;
                case OpCode.multi:
                    MultiTxn multiTxn = (MultiTxn) txn;
                    List<Txn> txns = multiTxn.getTxns();
                    debug = "Multi transaction with " + txns.size() + " operations";
                    var2.multiResult = new ArrayList<ProcessTxnResult>();
                    ...
                    for (Txn subtxn : txns) {
                        ...
                        var2.multiResult.add(subRc);
                        if (subRc.err != 0 && var2.err == 0) {
                            var2.err = subRc.err;
                        }
                    }
                    break;
     ...
     *     If znode not found.
     */
    public void incrementCversion(String path, long zxid) throws KeeperException.NoNodeException {
        if (path.endsWith("/")) {
