...
public class QuorumPeerConfig {
    ...
    public void parseProperties(Properties zkProp) throws IOException, ConfigException {
        ...
        for (Entry<Object, Object> entry : zkProp.entrySet()) {
            String var5 = entry.getKey().toString().trim();
            String value = entry.getValue().toString().trim();
            if (var5.equals("dataDir")) {
                dataDir = vff.create(value);
            } else if (var5.equals("dataLogDir")) {
                dataLogDir = vff.create(value);
            } else if (var5.equals("clientPort")) {
                clientPort = Integer.parseInt(value);
            } else if (var5.equals("clientPortAddress")) {
                clientPortAddress = value.trim();
            } else if (var5.equals("tickTime")) {
                tickTime = Integer.parseInt(value);
            } else if (var5.equals("maxClientCnxns")) {
                maxClientCnxns = Integer.parseInt(value);
            } else if (var5.equals("minSessionTimeout")) {
                minSessionTimeout = Integer.parseInt(value);
            } else if (var5.equals("maxSessionTimeout")) {
                maxSessionTimeout = Integer.parseInt(value);
            } else if (var5.equals("initLimit")) {
                initLimit = Integer.parseInt(value);
            } else if (var5.equals("syncLimit")) {
                syncLimit = Integer.parseInt(value);
            } else if (var5.equals("electionAlg")) {
                electionAlg = Integer.parseInt(value);
            } else if (var5.equals("peerType")) {
            ...
            } else if (var5.equals("autopurge.snapRetainCount")) {
                snapRetainCount = Integer.parseInt(value);
            } else if (var5.equals("autopurge.purgeInterval")) {
                purgeInterval = Integer.parseInt(value);
            } else if (var5.startsWith("server.")) {
                int dot = var5.indexOf('.');
                long sid = Long.parseLong(var5.substring(dot + 1));
            ...
            } else if (var5.startsWith("group")) {
                int dot = var5.indexOf('.');
                long gid = Long.parseLong(var5.substring(dot + 1));
                numGroups++;
                String[] parts = value.split(":");
                for (String s : parts) {
            ...
            } else if (var5.startsWith("weight")) {
                int dot = var5.indexOf('.');
                long sid = Long.parseLong(var5.substring(dot + 1));
                serverWeight.put(sid, Long.parseLong(value));
            } else {
                System.setProperty("zookeeper." + var5, value);
            }
        }
        // than 3.
