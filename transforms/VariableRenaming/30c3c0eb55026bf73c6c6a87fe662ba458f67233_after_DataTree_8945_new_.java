...
public class DataTree {
    ...
    public void setWatches(long relativeZxid, List<String> dataWatches, List<String> existWatches, List<String> childWatches, Watcher watcher) {
        for (String var11 : dataWatches) {
            DataNode node = getNode(var11);
            WatchedEvent e = null;
            if (node == null) {
                e = new WatchedEvent(EventType.NodeDeleted, KeeperState.SyncConnected, var11);
            } else if (node.stat.getCzxid() > relativeZxid) {
                e = new WatchedEvent(EventType.NodeCreated, KeeperState.SyncConnected, var11);
            } else if (node.stat.getMzxid() > relativeZxid) {
                e = new WatchedEvent(EventType.NodeDataChanged, KeeperState.SyncConnected, var11);
            }
            if (e != null) {
                watcher.process(e);
            } else {
                this.dataWatches.addWatch(var11, watcher);
            }
        }
        for (String var11 : existWatches) {
            DataNode node = getNode(var11);
            WatchedEvent e = null;
            if (node == null) {
            } else if (node.stat.getMzxid() > relativeZxid) {
        ...
        for (String var11 : childWatches) {
            DataNode node = getNode(var11);
            WatchedEvent e = null;
            if (node == null) {
                e = new WatchedEvent(EventType.NodeDeleted, KeeperState.SyncConnected, var11);
            } else if (node.stat.getPzxid() > relativeZxid) {
                e = new WatchedEvent(EventType.NodeChildrenChanged, KeeperState.SyncConnected, var11);
            }
            if (e != null) {
                watcher.process(e);
            } else {
                this.childWatches.addWatch(var11, watcher);
            }
        }
    }
}
