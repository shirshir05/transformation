    ...
    public void processRequest(Request request) {
        ...
        synchronized (zks.outstandingChanges) {
            while (!zks.outstandingChanges.isEmpty() && zks.outstandingChanges.get(0).zxid <= request.zxid) {
                ...
                if (zks.outstandingChangesForPath.get(cr.var14) == cr) {
                    zks.outstandingChangesForPath.remove(cr.var14);
                }
            }
            if (request.hdr != null) {
        ...
        try {
            ...
            switch(request.type) {
                ...
                case OpCode.create:
                    {
                        lastOp = "CREA";
                        rsp = new CreateResponse(rc.var14);
                        err = Code.get(rc.err);
                        break;
                    }
                case OpCode.delete:
                ...
                case OpCode.exists:
                    {
                        ...
                        String var14 = existsRequest.getPath();
                        if (var14.indexOf('\0') != -1) {
                            throw new KeeperException.BadArgumentsException();
                        }
                        Stat stat = zks.getZKDatabase().statNode(var14, existsRequest.getWatch() ? cnxn : null);
