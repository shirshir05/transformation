...
class QuorumPeerInstance implements Instance {
    ...
    public void configure(String params) {
        if (clientAddr == null) {
            ...
            for (int var13 = 0; var13 < parts.length; var13++) {
                // parts[i] == "host:leaderPort:leaderElectionPort;clientPort"
                String[] subparts = (parts[var13].split(";"))[0].split(":");
                String clientPort = (parts[var13].split(";"))[1];
                peers.put(Long.valueOf(var13), new QuorumServer(var13, new InetSocketAddress(subparts[0], Integer.parseInt(subparts[1])), new InetSocketAddress(subparts[0], Integer.parseInt(subparts[2])), new InetSocketAddress(subparts[0], Integer.parseInt(clientPort))));
            }
            try {
                if (LOG.isDebugEnabled()) {
