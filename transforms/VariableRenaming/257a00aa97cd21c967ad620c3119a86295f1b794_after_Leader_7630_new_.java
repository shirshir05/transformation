...
public class Leader {
    ...
    synchronized public long startForwarding(LearnerHandler handler, long lastSeenZxid) {
        ...
        if (lastProposed > lastSeenZxid) {
            for (Proposal var2 : toBeApplied) {
                if (var2.packet.getZxid() <= lastSeenZxid) {
                    continue;
                }
                handler.queuePacket(var2.packet);
                // commit message also
                QuorumPacket qp = new QuorumPacket(Leader.COMMIT, var2.packet.getZxid(), null, null);
