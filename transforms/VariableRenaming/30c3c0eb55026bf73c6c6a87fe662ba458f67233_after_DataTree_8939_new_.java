...
public class DataTree {
    ...
    public void setWatches(long relativeZxid, List<String> dataWatches, List<String> existWatches, List<String> childWatches, Watcher watcher) {
        for (String var5 : dataWatches) {
            DataNode node = getNode(var5);
            WatchedEvent e = null;
            if (node == null) {
                e = new WatchedEvent(EventType.NodeDeleted, KeeperState.SyncConnected, var5);
            } else if (node.stat.getCzxid() > relativeZxid) {
                e = new WatchedEvent(EventType.NodeCreated, KeeperState.SyncConnected, var5);
            } else if (node.stat.getMzxid() > relativeZxid) {
                e = new WatchedEvent(EventType.NodeDataChanged, KeeperState.SyncConnected, var5);
            }
            if (e != null) {
                watcher.process(e);
            } else {
                this.dataWatches.addWatch(var5, watcher);
            }
        }
        for (String var5 : existWatches) {
            DataNode node = getNode(var5);
            WatchedEvent e = null;
            if (node == null) {
            } else if (node.stat.getMzxid() > relativeZxid) {
        ...
        for (String var5 : childWatches) {
            DataNode node = getNode(var5);
            WatchedEvent e = null;
            if (node == null) {
                e = new WatchedEvent(EventType.NodeDeleted, KeeperState.SyncConnected, var5);
            } else if (node.stat.getPzxid() > relativeZxid) {
                e = new WatchedEvent(EventType.NodeChildrenChanged, KeeperState.SyncConnected, var5);
            }
            if (e != null) {
                watcher.process(e);
            } else {
                this.childWatches.addWatch(var5, watcher);
            }
        }
    }
}
