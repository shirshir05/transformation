            ...
            synchronized (leader.zk.committedLog) {
                if (leader.zk.committedLog.size() != 0) {
                    if ((leader.zk.maxCommittedLog >= peerLastZxid) && (leader.zk.minCommittedLog <= peerLastZxid)) {
                        ...
                        for (Proposal var7 : leader.zk.committedLog) {
                            if (var7.packet.getZxid() > peerLastZxid) {
                                queuePacket(var7.packet);
                                QuorumPacket qcommit = new QuorumPacket(Leader.COMMIT, var7.packet.getZxid(), null, null);
                                queuePacket(qcommit);
                            }
                        }
                    }
