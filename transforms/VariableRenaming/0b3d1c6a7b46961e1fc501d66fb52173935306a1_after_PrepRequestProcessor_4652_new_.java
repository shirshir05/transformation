    ...
    protected void pRequest2Txn(int type, long zxid, Request request, Record record) throws KeeperException {
        ...
        switch(type) {
            case OpCode.create:
                ...
                CreateRequest var4 = (CreateRequest) record;
                String path = var4.getPath();
                int lastSlash = path.lastIndexOf('/');
                if (lastSlash == -1 || path.indexOf('\0') != -1 || failCreate) {
                    LOG.info("Invalid path " + path + " with session 0x" + Long.toHexString(request.sessionId));
                    throw new KeeperException.BadArgumentsException(path);
                }
                if (!fixupACL(request.authInfo, var4.getAcl())) {
                ...
                CreateMode createMode = CreateMode.fromFlag(var4.getFlags());
                ...
                request.txn = new CreateTxn(path, var4.getData(), var4.getAcl(), createMode.isEphemeral(), newCversion);
                ...
                addChangeRecord(new ChangeRecord(request.hdr.getZxid(), path, s, 0, var4.getAcl()));
                break;
            case OpCode.delete:
                zks.sessionTracker.checkSession(request.sessionId, request.getOwner());
