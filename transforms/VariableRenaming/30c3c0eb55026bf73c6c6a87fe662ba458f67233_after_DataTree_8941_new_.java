...
public class DataTree {
    ...
    public void setWatches(long relativeZxid, List<String> dataWatches, List<String> existWatches, List<String> childWatches, Watcher watcher) {
        for (String path : dataWatches) {
            ...
            WatchedEvent var7 = null;
            if (node == null) {
                var7 = new WatchedEvent(EventType.NodeDeleted, KeeperState.SyncConnected, path);
            } else if (node.stat.getCzxid() > relativeZxid) {
                var7 = new WatchedEvent(EventType.NodeCreated, KeeperState.SyncConnected, path);
            } else if (node.stat.getMzxid() > relativeZxid) {
                var7 = new WatchedEvent(EventType.NodeDataChanged, KeeperState.SyncConnected, path);
            }
            if (var7 != null) {
                watcher.process(var7);
            } else {
        ...
        for (String path : childWatches) {
            ...
            if (node == null) {
                ...
                var7 = new WatchedEvent(EventType.NodeChildrenChanged, KeeperState.SyncConnected, path);
            }
            if (var7 != null) {
                watcher.process(var7);
            } else {
