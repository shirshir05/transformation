...
public class FileTxnSnapLog {
     ...
     */
    private void processTransaction(TxnHeader hdr, DataTree dt, Map<Long, Integer> sessions, Record var3) {
        switch(hdr.getType()) {
            case OpCode.createSession:
                sessions.put(hdr.getClientId(), ((CreateSessionTxn) var3).getTimeOut());
                ZooTrace.logTraceMessage(LOG, ZooTrace.SESSION_TRACE_MASK, "playLog --- create session in log: " + Long.toHexString(hdr.getClientId()) + " with timeout: " + ((CreateSessionTxn) var3).getTimeOut());
                // give dataTree a chance to sync its lastProcessedZxid
                dt.processTxn(hdr, var3);
                break;
