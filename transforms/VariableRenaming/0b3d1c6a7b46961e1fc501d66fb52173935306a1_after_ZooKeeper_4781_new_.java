...
public class ZooKeeper {
    ...
    protected List<OpResult> multiInternal(MultiTransactionRecord var0) throws InterruptedException, KeeperException {
        RequestHeader h = new RequestHeader();
        h.setType(ZooDefs.OpCode.multi);
        MultiResponse response = new MultiResponse();
        ReplyHeader r = cnxn.submitRequest(h, var0, response, null);
        if (r.getErr() != 0) {
     ...
     * @throws IllegalArgumentException if an invalid path is specified
     */
    public void deleteRecursive(final String pathRoot, VoidCallback cb, Object ctx) throws InterruptedException, KeeperException {
        PathUtils.validatePath(pathRoot);
        List<String> tree = this.listSubTreeBFS(pathRoot);
        LOG.debug("Deleting " + tree);
        LOG.debug("Deleting " + tree.size() + " subnodes ");
        for (int i = tree.size() - 1; i >= 0; --i) {
            // Delete all versions of the node with -1.
            this.delete(tree.get(i), -1, cb, ctx);
        }
    }
