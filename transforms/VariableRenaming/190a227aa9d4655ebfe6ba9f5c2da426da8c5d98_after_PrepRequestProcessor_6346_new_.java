    ...
    protected void pRequest2Txn(int type, long zxid, Request request, Record record, boolean deserialize) throws KeeperException, IOException, RequestProcessorException {
        ...
        switch(type) {
            ...
            case OpCode.deleteContainer:
                {
                    String var6 = new String(request.request.array(), UTF_8);
                    String parentPath = getParentPathAndValidate(var6);
                    ChangeRecord nodeRecord = getRecordForPath(var6);
                    if (nodeRecord.childCount > 0) {
                        throw new KeeperException.NotEmptyException(var6);
                    }
                    if (EphemeralType.get(nodeRecord.stat.getEphemeralOwner()) == EphemeralType.NORMAL) {
                        throw new KeeperException.BadVersionException(var6);
                    }
                    ChangeRecord parentRecord = getRecordForPath(parentPath);
                    request.setTxn(new DeleteTxn(var6));
                    ...
                    nodeRecord = new ChangeRecord(request.getHdr().getZxid(), var6, null, -1, null);
                    nodeRecord.precalculatedDigest = precalculateDigest(DigestOpCode.REMOVE, var6);
            ...
            case OpCode.delete:
                ...
                String var6 = deleteRequest.getPath();
                String parentPath = getParentPathAndValidate(var6);
                ChangeRecord parentRecord = getRecordForPath(parentPath);
                zks.checkACL(request.cnxn, parentRecord.acl, ZooDefs.Perms.DELETE, request.authInfo, var6, null);
                ChangeRecord nodeRecord = getRecordForPath(var6);
                checkAndIncVersion(nodeRecord.stat.getVersion(), deleteRequest.getVersion(), var6);
                if (nodeRecord.childCount > 0) {
                    throw new KeeperException.NotEmptyException(var6);
                }
                request.setTxn(new DeleteTxn(var6));
            ...
            case OpCode.setData:
                ...
                var6 = setDataRequest.getPath();
                validatePath(var6, request.sessionId);
                nodeRecord = getRecordForPath(var6);
                zks.checkACL(request.cnxn, nodeRecord.acl, ZooDefs.Perms.WRITE, request.authInfo, var6, null);
                zks.checkQuota(var6, setDataRequest.getData(), OpCode.setData);
                int newVersion = checkAndIncVersion(nodeRecord.stat.getVersion(), setDataRequest.getVersion(), var6);
                request.setTxn(new SetDataTxn(var6, setDataRequest.getData(), newVersion));
                ...
                nodeRecord.precalculatedDigest = precalculateDigest(DigestOpCode.UPDATE, var6, nodeRecord.data, nodeRecord.stat);
                setTxnDigest(request, nodeRecord.precalculatedDigest);
                addChangeRecord(nodeRecord);
                break;
            ...
            case OpCode.setACL:
                ...
                var6 = setAclRequest.getPath();
                validatePath(var6, request.sessionId);
                List<ACL> listACL = fixupACL(var6, request.authInfo, setAclRequest.getAcl());
                nodeRecord = getRecordForPath(var6);
                zks.checkACL(request.cnxn, nodeRecord.acl, ZooDefs.Perms.ADMIN, request.authInfo, var6, listACL);
                newVersion = checkAndIncVersion(nodeRecord.stat.getAversion(), setAclRequest.getVersion(), var6);
                request.setTxn(new SetACLTxn(var6, listACL, newVersion));
                nodeRecord = nodeRecord.duplicate(request.getHdr().getZxid());
            ...
            case OpCode.check:
                ...
                var6 = checkVersionRequest.getPath();
                validatePath(var6, request.sessionId);
                nodeRecord = getRecordForPath(var6);
                zks.checkACL(request.cnxn, nodeRecord.acl, ZooDefs.Perms.READ, request.authInfo, var6, null);
                request.setTxn(new CheckVersionTxn(var6, checkAndIncVersion(nodeRecord.stat.getVersion(), checkVersionRequest.getVersion(), var6)));
                break;
            default:
