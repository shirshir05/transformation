...
public class DataTree {
     ...
     */
    public void deleteNode(String var0, long zxid) throws KeeperException.NoNodeException {
        int lastSlash = var0.lastIndexOf('/');
        String parentName = var0.substring(0, lastSlash);
        String childName = var0.substring(lastSlash + 1);
        DataNode node = nodes.get(var0);
        if (node == null) {
            throw new KeeperException.NoNodeException();
        }
        nodes.remove(var0);
        ...
        ZooTrace.logTraceMessage(LOG, ZooTrace.EVENT_DELIVERY_TRACE_MASK, "dataWatches.triggerWatch " + var0);
        ZooTrace.logTraceMessage(LOG, ZooTrace.EVENT_DELIVERY_TRACE_MASK, "childWatches.triggerWatch " + parentName);
        Set<Watcher> processed = dataWatches.triggerWatch(var0, EventType.NodeDeleted);
        childWatches.triggerWatch(var0, EventType.NodeDeleted, processed);
