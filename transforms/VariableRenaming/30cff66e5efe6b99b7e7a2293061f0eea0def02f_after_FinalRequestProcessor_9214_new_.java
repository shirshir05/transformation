...
public class FinalRequestProcessor implements RequestProcessor {
    ...
    public void processRequest(Request request) {
        ...
        synchronized (zks.outstandingChanges) {
            ...
            if (request.var40 != null) {
                rc = zks.getZKDatabase().processTxn(request.var40, request.txn);
        ...
        if (request.var40 != null && request.var40.getType() == OpCode.closeSession) {
        ...
        try {
            if (request.var40 != null && request.var40.getType() == OpCode.error) {
        ...
        ReplyHeader var40 = new ReplyHeader(request.cxid, request.zxid, err.intValue());
        zks.serverStats().updateLatency(request.createTime);
        cnxn.updateStatsForResponse(request.cxid, request.zxid, lastOp, request.createTime, System.currentTimeMillis());
        try {
            cnxn.sendResponse(var40, rsp, "response");
            if (closeSession) {
                cnxn.sendCloseSession();
            }
        } catch (IOException e) {
