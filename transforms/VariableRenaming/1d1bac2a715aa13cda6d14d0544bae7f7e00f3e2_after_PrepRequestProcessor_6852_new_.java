    ...
    protected void pRequest2Txn(int type, long zxid, Request request, Record record) throws KeeperException {
        ...
        switch(type) {
            case OpCode.create:
                ...
                checkACL(zks, var9.acl, ZooDefs.Perms.CREATE, request.authInfo);
                int parentCVersion = var9.stat.getCversion();
                ...
                boolean ephemeralParent = var9.stat.getEphemeralOwner() != 0;
                if (ephemeralParent) {
                    throw new KeeperException.NoChildrenForEphemeralsException(path);
                }
                int newCversion = var9.stat.getCversion() + 1;
                request.txn = new CreateTxn(path, createRequest.getData(), listACL, createMode.isEphemeral(), newCversion);
                StatPersisted s = new StatPersisted();
                if (createMode.isEphemeral()) {
                    s.setEphemeralOwner(request.sessionId);
                }
                var9 = var9.duplicate(request.hdr.getZxid());
                var9.childCount++;
                var9.stat.setCversion(newCversion);
            ...
            case OpCode.delete:
                ...
                checkACL(zks, var9.acl, ZooDefs.Perms.DELETE, request.authInfo);
