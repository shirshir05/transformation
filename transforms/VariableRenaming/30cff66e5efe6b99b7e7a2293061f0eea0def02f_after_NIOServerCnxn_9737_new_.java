...
public class NIOServerCnxn extends ServerCnxn {
    ...
    private boolean readLength(SelectionKey k) throws IOException {
        ...
        int var1 = lenBuffer.getInt();
        if (!initialized && checkFourLetterWord(sk, var1)) {
            return false;
        }
        if (var1 < 0 || var1 > BinaryInputArchive.maxBuffer) {
            throw new IOException("Len error " + var1);
        }
        if (zkServer == null) {
            throw new IOException("ZooKeeperServer not running");
        }
        incomingBuffer = ByteBuffer.allocate(var1);
        return true;
    }
