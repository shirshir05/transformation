...
public class NIOServerCnxn implements Watcher, ServerCnxn {
    ...
    class CnxnStats implements ServerCnxn.Stats {
         ...
         */
        public synchronized void dumpConnectionInfo(PrintWriter var0, boolean brief) {
            Channel channel = sk.channel();
            if (channel instanceof SocketChannel) {
                var0.print(" ");
                var0.print(((SocketChannel) channel).socket().getRemoteSocketAddress());
                var0.print("[");
                var0.print(sk.isValid() ? Integer.toHexString(sk.interestOps()) : "0");
                var0.print("](queued=");
                var0.print(getOutstandingRequests());
                var0.print(",recved=");
                var0.print(getPacketsReceived());
                var0.print(",sent=");
                var0.print(getPacketsSent());
                if (!brief) {
                    long sessionId = getSessionId();
                    if (sessionId != 0) {
                        var0.print(",sid=0x");
                        var0.print(Long.toHexString(sessionId));
                        var0.print(",lop=");
                        var0.print(getLastOperation());
                        var0.print(",est=");
                        var0.print(getEstablished().getTime());
                        var0.print(",to=");
                        var0.print(getSessionTimeout());
                        long lastCxid = getLastCxid();
                        if (lastCxid >= 0) {
                            var0.print(",lcxid=0x");
                            var0.print(Long.toHexString(lastCxid));
                        }
                        var0.print(",lzxid=0x");
                        var0.print(Long.toHexString(getLastZxid()));
                        var0.print(",lresp=");
                        var0.print(getLastResponseTime());
                        var0.print(",llat=");
                        var0.print(getLastLatency());
                        var0.print(",minlat=");
                        var0.print(getMinLatency());
                        var0.print(",avglat=");
                        var0.print(getAvgLatency());
                        var0.print(",maxlat=");
                        var0.print(getMaxLatency());
                    }
                }
                var0.println(")");
            }
