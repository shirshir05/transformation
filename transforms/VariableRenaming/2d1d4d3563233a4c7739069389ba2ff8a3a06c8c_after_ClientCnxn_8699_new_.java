        ...
        public void run() {
            ...
            while (zooKeeper.state.isAlive()) {
                try {
                    ...
                    for (SelectionKey var5 : selected) {
                        SocketChannel sc = ((SocketChannel) var5.channel());
                        if ((var5.readyOps() & SelectionKey.OP_CONNECT) != 0) {
                            if (sc.finishConnect()) {
                                zooKeeper.state = States.CONNECTED;
                                timeout = readTimeout / 2;
                                lastHeard = now;
                                primeConnection(var5);
                            }
                        } else if ((var5.readyOps() & (SelectionKey.OP_READ | SelectionKey.OP_WRITE)) != 0) {
                            if (doIO()) {
                                lastHeard = now;
                            }
                        }
