            ...
            while (true) {
                ...
                switch(qp.getType()) {
                    ...
                    case Leader.REVALIDATE:
                        ...
                        long var38 = dis.readLong();
                        int to = dis.readInt();
                        ByteArrayOutputStream bos = new ByteArrayOutputStream();
                        DataOutputStream dos = new DataOutputStream(bos);
                        dos.writeLong(var38);
                        boolean valid = leader.zk.touch(var38, to);
                        if (valid) {
                            try {
                                // owns the session
                                leader.zk.setOwner(var38, this);
                            } catch (SessionExpiredException e) {
                                LOG.error("Somehow session " + Long.toHexString(var38) + " expired right after being renewed! (impossible)", e);
                            }
                        }
                        if (LOG.isTraceEnabled()) {
                            ZooTrace.logTraceMessage(LOG, ZooTrace.SESSION_TRACE_MASK, "Session 0x" + Long.toHexString(var38) + " is valid: " + valid);
                        }
