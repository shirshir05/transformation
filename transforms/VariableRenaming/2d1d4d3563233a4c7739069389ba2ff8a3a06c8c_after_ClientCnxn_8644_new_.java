                ...
                while (true) {
                    ...
                    if (event instanceof WatcherEvent) {
                        ...
                        if (p.var7 == null) {
                            ...
                            StatCallback var7 = (StatCallback) p.var7;
                            if (rc == 0) {
                                if (p.response instanceof ExistsResponse) {
                                    var7.processResult(rc, path, p.ctx, ((ExistsResponse) p.response).getStat());
                                } else if (p.response instanceof SetDataResponse) {
                                    var7.processResult(rc, path, p.ctx, ((SetDataResponse) p.response).getStat());
                                } else if (p.response instanceof SetACLResponse) {
                                    var7.processResult(rc, path, p.ctx, ((SetACLResponse) p.response).getStat());
                                }
                            } else {
                                var7.processResult(rc, path, p.ctx, null);
                            }
                        } else if (p.response instanceof GetDataResponse) {
                            DataCallback var7 = (DataCallback) p.var7;
                            GetDataResponse rsp = (GetDataResponse) p.response;
                            if (rc == 0) {
                                var7.processResult(rc, path, p.ctx, rsp.getData(), rsp.getStat());
                            } else {
                                var7.processResult(rc, path, p.ctx, null, null);
                            }
                        } else if (p.response instanceof GetACLResponse) {
                            ACLCallback var7 = (ACLCallback) p.var7;
                            GetACLResponse rsp = (GetACLResponse) p.response;
                            if (rc == 0) {
                                var7.processResult(rc, path, p.ctx, rsp.getAcl(), rsp.getStat());
                            } else {
                                var7.processResult(rc, path, p.ctx, null, null);
                            }
                        } else if (p.response instanceof GetChildrenResponse) {
                            ChildrenCallback var7 = (ChildrenCallback) p.var7;
                            GetChildrenResponse rsp = (GetChildrenResponse) p.response;
                            if (rc == 0) {
                                var7.processResult(rc, path, p.ctx, rsp.getChildren());
                            } else {
                                var7.processResult(rc, path, p.ctx, null);
                            }
                        } else if (p.response instanceof CreateResponse) {
                            StringCallback var7 = (StringCallback) p.var7;
                            CreateResponse rsp = (CreateResponse) p.response;
                            if (rc == 0) {
                                var7.processResult(rc, path, p.ctx, rsp.getPath());
                            } else {
                                var7.processResult(rc, path, p.ctx, null);
                            }
                        } else if (p.var7 instanceof VoidCallback) {
                            VoidCallback var7 = (VoidCallback) p.var7;
                            var7.processResult(rc, path, p.ctx);
                        }
                    }
                }
