...
public class LearnerHandler extends Thread {
    ...
    public void run() {
        try {
             ...
             */
            long var9 = leader.startForwarding(this, updates);
            // a special case when both the ids are the same
            if (peerLastZxid == var9) {
                packetToSend = Leader.DIFF;
                zxidToSend = var9;
            }
            QuorumPacket newLeaderQP = new QuorumPacket(Leader.NEWLEADER, var9, null, null);
            ...
            if (packetToSend == Leader.SNAP) {
                LOG.warn("Sending snapshot last zxid of peer is 0x" + Long.toHexString(peerLastZxid) + " " + " zxid of leader is 0x" + Long.toHexString(var9));
                // Dump data to peer
