...
public class NIOServerCnxn implements Watcher, ServerCnxn {
    ...
    static public class Factory extends Thread {
        ...
        public void run() {
                ...
                try {
                    ...
                    for (SelectionKey k : selected) {
                        if ((k.readyOps() & SelectionKey.OP_ACCEPT) != 0) {
                            ...
                            NIOServerCnxn cnxn = new NIOServerCnxn(zks, sc, sk, this);
                            sk.attach(cnxn);
                            addCnxn(cnxn);
            ...
            ZooLog.logTextTraceMessage("NIOServerCnxn factory exitedloop.", ZooLog.textTraceMask);
            clear();
    ...
    private void readRequest() throws IOException {
        ...
        InputStream var0 = new ByteBufferInputStream(incomingBuffer);
        BinaryInputArchive bia = BinaryInputArchive.getArchive(var0);
        ...
        if (h.getType() == OpCode.auth) {
            ...
            if (ap == null || ap.handleAuthentication(this, authPacket.getAuth()) != KeeperException.Code.Ok) {
                if (ap == null)
                    ZooLog.logError("No authentication provider for scheme: " + scheme);
                else
                    ZooLog.logError("Authentication failed for scheme: " + scheme);
                // send a response...
                ReplyHeader rh = new ReplyHeader(h.getXid(), 0, KeeperException.Code.AuthFailed);
                sendResponse(rh, null, null);
                // ... and close connection
                sendBuffer(NIOServerCnxn.closeConn);
                disableRecv();
            } else {
                ZooLog.logError("Authentication succeeded for scheme: " + scheme);
                ReplyHeader rh = new ReplyHeader(h.getXid(), 0, KeeperException.Code.Ok);
                sendResponse(rh, null, null);
            }
            return;
        } else {
            zk.submitRequest(this, sessionId, h.getType(), h.getXid(), incomingBuffer, authInfo);
        }
        if (h.getXid() >= 0) {
            synchronized (this) {
                outstandingRequests++;
    ...
    private void readConnectRequest() throws IOException, InterruptedException {
        ...
        ZooLog.logWarn("Connected to " + sock.socket().getRemoteSocketAddress() + " lastZxid " + connReq.getLastZxidSeen());
        if (zk == null) {
            throw new IOException("ZooKeeperServer not running");
        }
        if (connReq.getLastZxidSeen() > zk.dataTree.lastProcessedZxid) {
            ZooLog.logError("Client has seen " + Long.toHexString(connReq.getLastZxidSeen()) + " our last zxid is " + Long.toHexString(zk.dataTree.lastProcessedZxid));
        ...
        if (sessionId != 0) {
            ...
            ZooLog.logWarn("Creating new session " + Long.toHexString(sessionId));
        }
        initialized = true;
    }
            ...
            if (len == ruokCmd) {
                ...
                for (SelectionKey sk : factory.selector.keys()) {
                    ...
                    if (channel instanceof SocketChannel) {
                        ...
                        sb.append(" " + ((SocketChannel) channel).socket().getRemoteSocketAddress() + "[" + Integer.toHexString(sk.interestOps()) + "](queued=" + cnxn.outstandingRequests + ",recved=" + cnxn.packetsReceived + ",sent=" + cnxn.packetsSent + ")\n");
                    }
                }
                sb.append("\n");
                    ...
                    switch(factory.self.state) {
                        ...
                        case LEADING:
                            ...
                            for (FollowerHandler fh : factory.self.leader.followers) {
                                ...
                                if (factory.self.leader.forwardingFollowers.contains(fh)) {
                                    sb.append("*");
                                }
                            }
                            sb.append("\n");
    ...
    public void close() {
        ...
        ZooLog.logTextTraceMessage("close  NIOServerCnxn: " + sock, ZooLog.SESSION_TRACE_MASK);
        try {
            /*
    ...
    synchronized public void process(WatcherEvent event) {
        ...
        ZooLog.logTextTraceMessage("Deliver event " + event + " to " + this.sessionId + " through " + this, ZooLog.EVENT_DELIVERY_TRACE_MASK);
    ...
    public void finishSessionInit(boolean valid) {
        try {
            ...
            sendBuffer(bb);
            ZooLog.logWarn("Finished init of " + Long.toHexString(sessionId) + ": " + valid);
            if (!valid) {
