...
public class QuorumPeerConfig {
    ...
    public void checkValidity() throws IOException, ConfigException {
        if (quorumVerifier.getVotingMembers().size() > 1) {
            ...
            LearnerType var5 = quorumVerifier.getObservingMembers().containsKey(serverId) ? LearnerType.OBSERVER : LearnerType.PARTICIPANT;
            if (var5 != peerType) {
                LOG.warn("Peer type from servers list (" + var5 + ") doesn't match peerType (" + peerType + "). Defaulting to servers list.");
                peerType = var5;
            }
        }
    }
