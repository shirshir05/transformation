                ...
                try {
                    ...
                    if (closing) {
                        if (LOG.isDebugEnabled()) {
                            ...
                            LOG.debug("An exception was thrown while closing send thread for session 0x" + Long.toHexString(getSessionId()) + " : " + var2.getMessage());
                        }
                        break;
                    } else {
                        // this is ugly, you have a better way speak up
                        if (var2 instanceof SessionExpiredException) {
                            LOG.info(var2.getMessage() + ", closing socket connection");
                        } else if (var2 instanceof SessionTimeoutException) {
                            LOG.info(var2.getMessage() + RETRY_CONN_MSG);
                        } else if (var2 instanceof EndOfStreamException) {
                            LOG.info(var2.getMessage() + RETRY_CONN_MSG);
                        } else {
                            LOG.warn("Session 0x" + Long.toHexString(getSessionId()) + " for server " + clientCnxnSocket.getRemoteSocketAddress() + ", unexpected error" + RETRY_CONN_MSG, var2);
                        }
                        cleanup();
                        if (state.isAlive()) {
