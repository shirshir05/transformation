...
public final class StaticHostProvider implements HostProvider {
    ...
    private List<InetSocketAddress> resolveAndShuffle(Collection<InetSocketAddress> var0) throws UnknownHostException {
        List<InetSocketAddress> tmpList = new ArrayList<InetSocketAddress>(var0.size());
        for (InetSocketAddress address : var0) {
            ...
            for (InetAddress resolvedAddress : resolvedAddresses) {
                ...
                if (resolvedAddress.toString().startsWith("/") && resolvedAddress.getAddress() != null) {
                    tmpList.add(new InetSocketAddress(InetAddress.getByAddress(address.getHostName(), resolvedAddress.getAddress()), address.getPort()));
                } else {
