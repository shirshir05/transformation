        ...
        synchronized (this) {
            if (leader != null) {
                synchronized (leader.learners) {
                    for (LearnerHandler var1 : (Collection<LearnerHandler>) leader.learners) {
                        if (var1.getSocket() == null)
                            continue;
                        String s = var1.getSocket().getRemoteSocketAddress().toString();
                        if (leader.isLearnerSynced(var1))
                            s += "*";
                        l.add(s);
                    }
                }
