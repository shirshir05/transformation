    ...
    protected void pRequest2Txn(int type, long zxid, Request request, Record record, boolean deserialize) throws KeeperException, IOException, RequestProcessorException {
        ...
        switch(type) {
            ...
            case OpCode.deleteContainer:
                {
                    ...
                    ChangeRecord var14 = getRecordForPath(path);
                    if (var14.childCount > 0) {
                        throw new KeeperException.NotEmptyException(path);
                    }
                    if (EphemeralType.get(var14.stat.getEphemeralOwner()) == EphemeralType.NORMAL) {
                    ...
                    var14 = new ChangeRecord(request.getHdr().getZxid(), path, null, -1, null);
                    var14.precalculatedDigest = precalculateDigest(DigestOpCode.REMOVE, path);
                    setTxnDigest(request, var14.precalculatedDigest);
                    addChangeRecord(var14);
                    break;
                }
            case OpCode.delete:
                ...
                ChangeRecord var14 = getRecordForPath(path);
                checkAndIncVersion(var14.stat.getVersion(), deleteRequest.getVersion(), path);
                if (var14.childCount > 0) {
                    throw new KeeperException.NotEmptyException(path);
                }
                request.setTxn(new DeleteTxn(path));
            ...
            case OpCode.setData:
                ...
                var14 = getRecordForPath(path);
                zks.checkACL(request.cnxn, var14.acl, ZooDefs.Perms.WRITE, request.authInfo, path, null);
                zks.checkQuota(path, setDataRequest.getData(), OpCode.setData);
                int newVersion = checkAndIncVersion(var14.stat.getVersion(), setDataRequest.getVersion(), path);
                request.setTxn(new SetDataTxn(path, setDataRequest.getData(), newVersion));
                var14 = var14.duplicate(request.getHdr().getZxid());
                var14.stat.setVersion(newVersion);
                var14.stat.setMtime(request.getHdr().getTime());
                var14.stat.setMzxid(zxid);
                var14.data = setDataRequest.getData();
                var14.precalculatedDigest = precalculateDigest(DigestOpCode.UPDATE, path, var14.data, var14.stat);
                setTxnDigest(request, var14.precalculatedDigest);
                addChangeRecord(var14);
                break;
            case OpCode.reconfig:
                ...
                var14 = getRecordForPath(ZooDefs.CONFIG_NODE);
                zks.checkACL(request.cnxn, var14.acl, ZooDefs.Perms.WRITE, request.authInfo, null, null);
                SetDataTxn setDataTxn = new SetDataTxn(ZooDefs.CONFIG_NODE, request.qv.toString().getBytes(), -1);
                request.setTxn(setDataTxn);
                var14 = var14.duplicate(request.getHdr().getZxid());
                var14.stat.setVersion(-1);
                var14.stat.setMtime(request.getHdr().getTime());
                var14.stat.setMzxid(zxid);
                var14.data = setDataTxn.getData();
                // perspective since config node is not covered by the digests.
                var14.precalculatedDigest = precalculateDigest(DigestOpCode.NOOP, ZooDefs.CONFIG_NODE, var14.data, var14.stat);
                setTxnDigest(request, var14.precalculatedDigest);
                addChangeRecord(var14);
                break;
            case OpCode.setACL:
                ...
                var14 = getRecordForPath(path);
                zks.checkACL(request.cnxn, var14.acl, ZooDefs.Perms.ADMIN, request.authInfo, path, listACL);
                newVersion = checkAndIncVersion(var14.stat.getAversion(), setAclRequest.getVersion(), path);
                request.setTxn(new SetACLTxn(path, listACL, newVersion));
                var14 = var14.duplicate(request.getHdr().getZxid());
                var14.stat.setAversion(newVersion);
                var14.precalculatedDigest = precalculateDigest(DigestOpCode.UPDATE, path, var14.data, var14.stat);
                setTxnDigest(request, var14.precalculatedDigest);
                addChangeRecord(var14);
                break;
            ...
            case OpCode.closeSession:
                ...
                synchronized (zks.outstandingChanges) {
                    ...
                    for (String path2Delete : es) {
                        ...
                        var14 = new ChangeRecord(request.getHdr().getZxid(), path2Delete, null, 0, null);
                        var14.precalculatedDigest = precalculateDigest(DigestOpCode.REMOVE, path2Delete);
                        addChangeRecord(var14);
                    }
            ...
            case OpCode.check:
                ...
                validatePath(path, request.sessionId);
                var14 = getRecordForPath(path);
                zks.checkACL(request.cnxn, var14.acl, ZooDefs.Perms.READ, request.authInfo, path, null);
                request.setTxn(new CheckVersionTxn(path, checkAndIncVersion(var14.stat.getVersion(), checkVersionRequest.getVersion(), path)));
                break;
            default:
