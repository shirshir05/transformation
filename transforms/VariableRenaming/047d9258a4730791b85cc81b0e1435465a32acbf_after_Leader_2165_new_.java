...
public class Leader {
    ...
    Leader(QuorumPeer self) throws IOException {
        ...
        try {
            ...
            ZooLog.logError("Couldn't bind to port " + self.myQuorumAddr.getPort());
            throw e;
        }
    }
    ...
    void lead() throws IOException, InterruptedException {
        self.tick = 0;
        zk = new LeaderZooKeeperServer(self.getId(), self.dataDir, self.dataLogDir, this);
        zk.loadData();
        zk.startup();
        ...
        if ((newLeaderProposal.packet.getZxid() & 0xffffffffL) != 0) {
            ZooLog.logError("NEWLEADER proposal has Zxid of " + newLeaderProposal.packet.getZxid());
        }
        ...
        while (newLeaderProposal.ackCount <= self.quorumPeers.size() / 2) {
            if (self.tick > self.initLimit) {
                ...
                if (followers.size() >= self.quorumPeers.size() / 2) {
                    ZooLog.logWarn("Enough followers present. Perhaps the initTicks need to be increased.");
                }
    ...
    void shutdown(String reason) {
        ...
        ZooLog.logException(new Exception("shutdown Leader! reason: " + reason));
    ...
    synchronized public void processAck(long zxid, SocketAddress followerAddr) {
        ...
        for (Proposal p : outstandingProposals) {
            ...
            if (packetZxid == zxid) {
                ...
                if (p.ackCount > self.quorumPeers.size() / 2) {
                    if (!first) {
                        ZooLog.logError("Commiting " + Long.toHexString(zxid) + " from " + followerAddr + " not first!");
                        ZooLog.logError("First is " + outstandingProposals.element().packet);
                        System.exit(13);
                    }
        ...
        ZooLog.logError("Trying to commit future proposal: " + Long.toHexString(zxid) + " from " + followerAddr);
    }
     ...
     */
    public Proposal propose(Request var0) {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        BinaryOutputArchive boa = BinaryOutputArchive.getArchive(baos);
        try {
            var0.hdr.serialize(boa, "hdr");
            if (var0.txn != null) {
                var0.txn.serialize(boa, "txn");
            }
            baos.close();
        } catch (IOException e) {
        ...
        QuorumPacket pp = new QuorumPacket(Leader.PROPOSAL, var0.zxid, baos.toByteArray(), null);
     ...
     */
    synchronized public long startForwarding(FollowerHandler handler, long lastSeenZxid) {
        // new requests
