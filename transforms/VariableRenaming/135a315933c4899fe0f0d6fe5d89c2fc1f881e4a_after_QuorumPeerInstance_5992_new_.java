...
class QuorumPeerInstance implements Instance {
    ...
    public void configure(String params) {
        if (clientAddr == null) {
            ...
            String var8 = clientAddr.getHostName() + ':' + clientAddr.getPort() + ',' + quorumLeaderAddr.getHostName() + ':' + quorumLeaderAddr.getPort() + ':' + quorumLeaderElectionAddr.getPort();
            try {
                if (LOG.isDebugEnabled()) {
                    LOG.debug("Reporting " + var8);
                }
                r.var8(var8);
            } catch (Exception e) {
                e.printStackTrace();
