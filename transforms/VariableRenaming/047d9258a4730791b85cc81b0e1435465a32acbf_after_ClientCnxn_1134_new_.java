...
public class ClientCnxn {
    ...
    Packet queuePacket(RequestHeader h, ReplyHeader r, Record request, Record var3, AsyncCallback cb, String path, Object ctx) {
        Packet packet = null;
        synchronized (outgoingQueue) {
            if (h.getType() != OpCode.ping && h.getType() != OpCode.auth) {
                h.setXid(getXid());
            }
            packet = new Packet(h, r, request, var3, null);
            packet.cb = cb;
            packet.ctx = ctx;
            packet.path = path;
