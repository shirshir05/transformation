    ...
    protected void pRequest2Txn(int type, long zxid, Request request, Record record, boolean deserialize) throws KeeperException, IOException, RequestProcessorException {
        ...
        switch(type) {
            ...
            case OpCode.deleteContainer:
                {
                    String var10 = new String(request.request.array());
                    String parentPath = getParentPathAndValidate(var10);
                    ChangeRecord parentRecord = getRecordForPath(parentPath);
                    ChangeRecord nodeRecord = getRecordForPath(var10);
                    if (nodeRecord.childCount > 0) {
                        throw new KeeperException.NotEmptyException(var10);
                    }
                    if (EphemeralType.get(nodeRecord.stat.getEphemeralOwner()) == EphemeralType.NORMAL) {
                        throw new KeeperException.BadVersionException(var10);
                    }
                    request.setTxn(new DeleteTxn(var10));
                    parentRecord = parentRecord.duplicate(request.getHdr().getZxid());
                    parentRecord.childCount--;
                    addChangeRecord(parentRecord);
                    addChangeRecord(new ChangeRecord(request.getHdr().getZxid(), var10, null, -1, null));
                    break;
                }
            case OpCode.delete:
                ...
                String var10 = deleteRequest.getPath();
                String parentPath = getParentPathAndValidate(var10);
                ChangeRecord parentRecord = getRecordForPath(parentPath);
                ChangeRecord nodeRecord = getRecordForPath(var10);
                checkACL(zks, parentRecord.acl, ZooDefs.Perms.DELETE, request.authInfo);
                checkAndIncVersion(nodeRecord.stat.getVersion(), deleteRequest.getVersion(), var10);
                if (nodeRecord.childCount > 0) {
                    throw new KeeperException.NotEmptyException(var10);
                }
                request.setTxn(new DeleteTxn(var10));
            ...
            case OpCode.setData:
                ...
                var10 = setDataRequest.getPath();
                validatePath(var10, request.sessionId);
                nodeRecord = getRecordForPath(var10);
                checkACL(zks, nodeRecord.acl, ZooDefs.Perms.WRITE, request.authInfo);
                int newVersion = checkAndIncVersion(nodeRecord.stat.getVersion(), setDataRequest.getVersion(), var10);
                request.setTxn(new SetDataTxn(var10, setDataRequest.getData(), newVersion));
            ...
            case OpCode.setACL:
                ...
                var10 = setAclRequest.getPath();
                validatePath(var10, request.sessionId);
                List<ACL> listACL = fixupACL(var10, request.authInfo, setAclRequest.getAcl());
                nodeRecord = getRecordForPath(var10);
                checkACL(zks, nodeRecord.acl, ZooDefs.Perms.ADMIN, request.authInfo);
                newVersion = checkAndIncVersion(nodeRecord.stat.getAversion(), setAclRequest.getVersion(), var10);
                request.setTxn(new SetACLTxn(var10, listACL, newVersion));
                nodeRecord = nodeRecord.duplicate(request.getHdr().getZxid());
            ...
            case OpCode.check:
                ...
                var10 = checkVersionRequest.getPath();
                validatePath(var10, request.sessionId);
                nodeRecord = getRecordForPath(var10);
                checkACL(zks, nodeRecord.acl, ZooDefs.Perms.READ, request.authInfo);
                request.setTxn(new CheckVersionTxn(var10, checkAndIncVersion(nodeRecord.stat.getVersion(), checkVersionRequest.getVersion(), var10)));
                break;
            default:
