    ...
    public void run() {
        try {
            ...
            try {
                ...
                LOG.info("Synchronizing with Follower sid: " + this.sid + " maxCommittedLog =" + Long.toHexString(maxCommittedLog) + " minCommittedLog = " + Long.toHexString(minCommittedLog) + " peerLastZxid = " + Long.toHexString(var5));
                LinkedList<Proposal> proposals = leader.zk.getZKDatabase().getCommittedLog();
                if (proposals.size() != 0) {
                    if ((maxCommittedLog >= var5) && (minCommittedLog <= var5)) {
                        // proposal Id.
                        ...
                        for (Proposal propose : proposals) {
                            ...
                            if (propose.packet.getZxid() <= var5) {
            ...
            if (packetToSend == Leader.SNAP) {
                LOG.info("Sending snapshot last zxid of peer is 0x" + Long.toHexString(var5) + " " + " zxid of leader is 0x" + Long.toHexString(leaderLastZxid) + "sent zxid of db as 0x" + Long.toHexString(zxidToSend));
                // Dump data to peer
