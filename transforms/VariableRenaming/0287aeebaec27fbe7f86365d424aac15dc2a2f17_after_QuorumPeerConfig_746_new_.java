    ...
    public void parseProperties(Properties zkProp) throws IOException, ConfigException {
        ...
        for (Entry<Object, Object> entry : zkProp.entrySet()) {
            ...
            if (key.equals("dataDir")) {
                ...
                int dot = key.indexOf('.');
                long var20 = Long.parseLong(key.substring(dot + 1));
                ...
                if (parts.length == 2) {
                    servers.put(Long.valueOf(var20), new QuorumServer(var20, addr));
                } else if (parts.length == 3) {
                    InetSocketAddress electionAddr = new InetSocketAddress(parts[0], Integer.parseInt(parts[2]));
                    servers.put(Long.valueOf(var20), new QuorumServer(var20, addr, electionAddr));
                } else if (parts.length == 4) {
                    InetSocketAddress electionAddr = new InetSocketAddress(parts[0], Integer.parseInt(parts[2]));
                    LearnerType type = LearnerType.PARTICIPANT;
                    if (parts[3].equalsIgnoreCase("observer")) {
                        type = LearnerType.OBSERVER;
                        observers.put(Long.valueOf(var20), new QuorumServer(var20, addr, electionAddr, type));
                    } else if (parts[3].equalsIgnoreCase("participant")) {
                        type = LearnerType.PARTICIPANT;
                        servers.put(Long.valueOf(var20), new QuorumServer(var20, addr, electionAddr, type));
                    } else {
                        throw new ConfigException("Unrecognised peertype: " + value);
                    }
                }
                ...
                String[] parts = value.split(":");
                for (String s : parts) {
                    long var20 = Long.parseLong(s);
                    if (serverGroup.containsKey(var20))
                        throw new ConfigException("Server " + var20 + "is in multiple groups");
                    else
                        serverGroup.put(var20, gid);
                }
            } else if (key.startsWith("weight")) {
                int dot = key.indexOf('.');
                long var20 = Long.parseLong(key.substring(dot + 1));
                serverWeight.put(var20, Long.parseLong(value));
            } else {
