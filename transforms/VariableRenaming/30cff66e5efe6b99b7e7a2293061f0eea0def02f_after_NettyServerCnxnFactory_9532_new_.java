    ...
    class CnxnChannelHandler extends SimpleChannelHandler {
        ...
        private void processMessage(MessageEvent e, NettyServerCnxn cnxn) {
            ...
            if (e instanceof NettyServerCnxn.ResumeMessageEvent) {
                ...
                ChannelBuffer var2 = (ChannelBuffer) e.getMessage();
                if (LOG.isTraceEnabled()) {
                    LOG.trace(Long.toHexString(cnxn.sessionId) + " buf 0x" + ChannelBuffers.hexDump(var2));
                }
                if (cnxn.throttled) {
                    ...
                    if (cnxn.queuedBuffer == null) {
                        ...
                        cnxn.queuedBuffer = dynamicBuffer(var2.readableBytes());
                    }
                    cnxn.queuedBuffer.writeBytes(var2);
                    ...
                    if (cnxn.queuedBuffer != null) {
                        ...
                        cnxn.receiveMessage(var2);
                        if (var2.readable()) {
                            if (LOG.isTraceEnabled()) {
                                LOG.trace("Before copy " + var2);
                            }
                            cnxn.queuedBuffer = dynamicBuffer(var2.readableBytes());
                            cnxn.queuedBuffer.writeBytes(var2);
                            if (LOG.isTraceEnabled()) {
