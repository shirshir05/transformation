...
public class FileTxnLog implements TxnLog {
    ...
    public boolean truncate(long zxid) throws IOException {
        FileTxnIterator var1 = new FileTxnIterator(this.logDir, zxid);
        PositionInputStream input = var1.inputStream;
        long pos = input.getPosition();
        // now, truncate at the current position
        RandomAccessFile raf = new RandomAccessFile(var1.logFile, "rw");
        raf.setLength(pos);
        raf.close();
        while (var1.goToNextLog()) {
            if (!var1.logFile.delete()) {
                LOG.warn("Unable to truncate " + var1.logFile);
            }
        }
        return true;
    }
