...
class QuorumPeerInstance implements Instance {
    ...
    public void configure(String params) {
        if (clientAddr == null) {
            ...
            for (int var20 = 0; var20 < parts.length; var20++) {
                // parts[i] == "host:leaderPort:leaderElectionPort;clientPort"
                String[] subparts = (parts[var20].split(";"))[0].split(":");
                String clientPort = (parts[var20].split(";"))[1];
                peers.put(Long.valueOf(var20), new QuorumServer(var20, new InetSocketAddress(subparts[0], Integer.parseInt(subparts[1])), new InetSocketAddress(subparts[0], Integer.parseInt(subparts[2])), new InetSocketAddress(subparts[0], Integer.parseInt(clientPort))));
            }
            try {
                if (LOG.isDebugEnabled()) {
