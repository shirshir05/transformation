...
public class ClientCnxn {
    ...
    class SendThread extends Thread {
        ...
        void readResponse() throws IOException {
            ...
            ByteBufferInputStream bbis = new ByteBufferInputStream(incomingBuffer);
            BinaryInputArchive bbia = BinaryInputArchive.getArchive(bbis);
            ReplyHeader var2 = new ReplyHeader();
            var2.deserialize(bbia, "header");
            if (var2.getXid() == -2) {
            ...
            if (pendingQueue.size() == 0) {
                throw new IOException("Nothing in the queue, but got " + var2.getXid());
            }
            Packet p = null;
             ...
             */
            if (p.header.getXid() != var2.getXid()) {
                throw new IOException("Xid out of order. Got " + var2.getXid() + " expected " + p.header.getXid());
            }
            p.replyHeader.setXid(var2.getXid());
            p.replyHeader.setErr(var2.getErr());
            p.replyHeader.setZxid(var2.getZxid());
            lastZxid = var2.getZxid();
            if (p.response != null && var2.getErr() == 0) {
