...
class QuorumPeerInstance implements Instance {
    ...
    public void configure(String params) {
        if (clientAddr == null) {
            ...
            try {
                ServerSocket var4 = new ServerSocket(0, 1, InetAddress.getLocalHost());
                clientAddr = (InetSocketAddress) var4.getLocalSocketAddress();
                var4.close();
            } catch (IOException e) {
                e.printStackTrace();
            }
            try {
                ServerSocket var4 = new ServerSocket(0, 1, InetAddress.getLocalHost());
                quorumLeaderAddr = (InetSocketAddress) var4.getLocalSocketAddress();
                var4.close();
            } catch (IOException e) {
                e.printStackTrace();
            }
            try {
                ServerSocket var4 = new ServerSocket(0, 1, InetAddress.getLocalHost());
                quorumLeaderElectionAddr = (InetSocketAddress) var4.getLocalSocketAddress();
                var4.close();
            } catch (IOException e) {
                e.printStackTrace();
            }
