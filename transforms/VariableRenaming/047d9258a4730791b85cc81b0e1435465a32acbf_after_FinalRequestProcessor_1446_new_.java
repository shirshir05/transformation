...
public class FinalRequestProcessor implements RequestProcessor {
    ...
    public void processRequest(Request request) {
        ...
        synchronized (zks.outstandingChanges) {
            ...
            if (request.var20 != null) {
                rc = zks.dataTree.processTxn(request.var20, request.txn);
        ...
        if (request.var20 != null && request.var20.getType() == OpCode.closeSession) {
        ...
        Record rsp = null;
        try {
            if (request.var20 != null && request.var20.getType() == OpCode.error) {
                throw new KeeperException(((ErrorTxn) request.txn).getErr());
            }
            switch(request.type) {
                case OpCode.ping:
                    request.cnxn.sendResponse(new ReplyHeader(-2, zks.dataTree.lastProcessedZxid, 0), null, "response");
                    return;
        ...
        ReplyHeader var20 = new ReplyHeader(request.cxid, request.zxid, err);
        long latency = System.currentTimeMillis() - request.createTime;
        count++;
        avg += latency;
        request.cnxn.setStats(latency, (avg / count));
        try {
            request.cnxn.sendResponse(var20, rsp, "response");
        } catch (IOException e) {
            ZooLog.logException(e);
        }
    }
