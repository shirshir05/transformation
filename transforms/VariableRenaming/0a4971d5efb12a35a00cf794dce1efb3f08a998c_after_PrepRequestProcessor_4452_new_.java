...
public class PrepRequestProcessor extends ZooKeeperCriticalThread implements RequestProcessor {
    ...
    protected void pRequest(Request request) throws RequestProcessorException {
        ...
        request.setTxn(null);
        try {
            switch(request.var16) {
                case OpCode.createContainer:
                case OpCode.create:
                case OpCode.create2:
                    CreateRequest create2Request = new CreateRequest();
                    pRequest2Txn(request.var16, zks.getNextZxid(), request, create2Request, true);
                    break;
                case OpCode.createTTL:
                    CreateTTLRequest createTtlRequest = new CreateTTLRequest();
                    pRequest2Txn(request.var16, zks.getNextZxid(), request, createTtlRequest, true);
                    break;
                case OpCode.deleteContainer:
                case OpCode.delete:
                    DeleteRequest deleteRequest = new DeleteRequest();
                    pRequest2Txn(request.var16, zks.getNextZxid(), request, deleteRequest, true);
                    break;
                case OpCode.setData:
                    SetDataRequest setDataRequest = new SetDataRequest();
                    pRequest2Txn(request.var16, zks.getNextZxid(), request, setDataRequest, true);
                    break;
                case OpCode.reconfig:
                    ReconfigRequest reconfigRequest = new ReconfigRequest();
                    ByteBufferInputStream.byteBuffer2Record(request.request, reconfigRequest);
                    pRequest2Txn(request.var16, zks.getNextZxid(), request, reconfigRequest, true);
                    break;
                case OpCode.setACL:
                    SetACLRequest setAclRequest = new SetACLRequest();
                    pRequest2Txn(request.var16, zks.getNextZxid(), request, setAclRequest, true);
                    break;
                case OpCode.check:
                    CheckVersionRequest checkRequest = new CheckVersionRequest();
                    pRequest2Txn(request.var16, zks.getNextZxid(), request, checkRequest, true);
                    break;
                case OpCode.multi:
                     ...
                     */
                        ...
                        txns.add(new Txn(var16, bb.array()));
                    }
                    request.setHdr(new TxnHeader(request.sessionId, request.cxid, zxid, Time.currentWallTime(), request.var16));
                ...
                case OpCode.closeSession:
                    if (!request.isLocalSession()) {
                        pRequest2Txn(request.var16, zks.getNextZxid(), request, null, true);
                    }
                    break;
                ...
                case OpCode.removeWatches:
                    ...
                    break;
                default:
                    LOG.warn("unknown type " + request.var16);
                    break;
            }
        } catch (KeeperException e) {
