    ...
    void rollbackPendingChanges(long zxid, HashMap<String, ChangeRecord> pendingChangeRecords) {
        synchronized (zks.outstandingChanges) {
            ...
            while (iter.hasPrevious()) {
                ChangeRecord var7 = iter.previous();
                if (var7.zxid == zxid) {
                    iter.remove();
                    zks.outstandingChangesForPath.remove(var7.path);
                } else {
                    break;
                }
            }
            ...
            while (priorIter.hasNext()) {
                ChangeRecord var7 = priorIter.next();
                /* Don't apply any prior change records less than firstZxid */
                if (!empty && (var7.zxid < firstZxid)) {
                    continue;
                }
                zks.outstandingChangesForPath.put(var7.path, var7);
            }
        }
    }
