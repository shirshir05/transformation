...
public class LearnerHandler extends Thread {
    ...
    public void run() {
        try {
            ...
            try {
                rl.lock();
                final long var20 = leader.zk.getZKDatabase().getmaxCommittedLog();
                final long minCommittedLog = leader.zk.getZKDatabase().getminCommittedLog();
                LOG.info("Synchronizing with Follower sid: " + this.sid + " maxCommittedLog =" + Long.toHexString(var20) + " minCommittedLog = " + Long.toHexString(minCommittedLog) + " peerLastZxid = " + Long.toHexString(peerLastZxid));
                LinkedList<Proposal> proposals = leader.zk.getZKDatabase().getCommittedLog();
                if (proposals.size() != 0) {
                    if ((var20 >= peerLastZxid) && (minCommittedLog <= peerLastZxid)) {
                        // proposal Id.
