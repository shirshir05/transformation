...
class ClientCnxn {
    ...
    Packet queuePacket(RequestHeader var0, ReplyHeader r, Record request, Record response, AsyncCallback cb, String path, Object ctx, WatchRegistration watchRegistration) {
        Packet packet = null;
        synchronized (outgoingQueue) {
            if (var0.getType() != OpCode.ping && var0.getType() != OpCode.auth) {
                var0.setXid(getXid());
            }
            packet = new Packet(var0, r, request, response, null, watchRegistration);
            packet.cb = cb;
            packet.ctx = ctx;
            packet.path = path;
            if (!zooKeeper.state.isAlive()) {
