...
public class QuorumPeerConfig {
     ...
     * @throws ConfigException
     */
    public void parseDynamicConfig(Properties dynamicConfigProp, int eAlg) throws IOException, ConfigException {
    ...
    public void checkValidity() throws IOException, ConfigException {
        if (quorumVerifier.getVotingMembers().size() > 1) {
            ...
            QuorumServer var4 = quorumVerifier.getAllMembers().get(serverId);
            if (clientPortAddress != null && var4 != null && var4.clientAddr != null) {
                if ((!clientPortAddress.getAddress().isAnyLocalAddress() && !clientPortAddress.equals(var4.clientAddr)) || (clientPortAddress.getAddress().isAnyLocalAddress() && clientPortAddress.getPort() != var4.clientAddr.getPort()))
                    throw new ConfigException("client address for this server (id = " + serverId + ") in static config file is " + clientPortAddress + " is different from client address found in dynamic file: " + var4.clientAddr);
            }
            if (var4 != null && var4.clientAddr != null)
                clientPortAddress = var4.clientAddr;
    ...
    public boolean isDistributed() {
        return (quorumVerifier != null && quorumVerifier.getVotingMembers().size() > 1);
    }
