...
public class Follower {
    ...
    void followLeader() throws InterruptedException {
        ...
        if (addr == null) {
            ZooLog.logError("Couldn't find the leader with id = " + self.currentVote.id);
        }
        ...
        try {
            ...
            zk = new FollowerZooKeeperServer(self.getId(), self.dataDir, self.dataLogDir, this);
            synchronized (zk) {
            ...
            while (self.running) {
                ...
                switch(qp.getType()) {
                    case Leader.PING:
                        ...
                        HashMap<Long, Integer> touchTable = ((FollowerZooKeeperServer) zk).getTouchSnapshot();
                    ...
                    case Leader.PROPOSAL:
                        ...
                        BinaryInputArchive var13 = BinaryInputArchive.getArchive(new ByteArrayInputStream(qp.getData()));
                        Record txn = ZooKeeperServer.deserializeTxn(var13, hdr);
                        if (hdr.getZxid() != lastQueued + 1) {
                            ZooLog.logWarn("Got zxid " + Long.toHexString(hdr.getZxid()) + " expected " + Long.toHexString(lastQueued + 1));
                        }
                    ...
                    case Leader.REVALIDATE:
                        ...
                        synchronized (pendingRevalidations) {
                            ...
                            if (cnxn == null) {
                                ZooLog.logWarn("Missing " + Long.toHexString(sessionId) + " for validation");
                            } else {
                                cnxn.finishSessionInit(valid);
                            }
                        }
                        ZooLog.logTextTraceMessage("Session " + sessionId + " is valid: " + valid, ZooLog.SESSION_TRACE_MASK);
                }
            }
        } catch (IOException e) {
            try {
    ...
    void validateSession(ServerCnxn cnxn, long clientId, int timeout) throws IOException, InterruptedException {
        ...
        ZooLog.logTextTraceMessage("To validate session " + clientId, ZooLog.SESSION_TRACE_MASK);
        writePacket(qp);
    }
