...
public class PrepRequestProcessor extends ZooKeeperCriticalThread implements RequestProcessor {
    ...
    private void pRequest2TxnCreate(int type, Request request, Record record, boolean deserialize) throws IOException, KeeperException {
        ...
        ChangeRecord var14 = getRecordForPath(parentPath);
        zks.checkACL(request.cnxn, var14.acl, ZooDefs.Perms.CREATE, request.authInfo, path, listACL);
        int parentCVersion = var14.stat.getCversion();
        ...
        boolean ephemeralParent = EphemeralType.get(var14.stat.getEphemeralOwner()) == EphemeralType.NORMAL;
        if (ephemeralParent) {
            throw new KeeperException.NoChildrenForEphemeralsException(path);
        }
        int newCversion = var14.stat.getCversion() + 1;
        ...
        var14.childCount++;
        var14.stat.setCversion(newCversion);
        var14.stat.setPzxid(request.getHdr().getZxid());
        var14.precalculatedDigest = precalculateDigest(DigestOpCode.UPDATE, parentPath, var14.data, var14.stat);
