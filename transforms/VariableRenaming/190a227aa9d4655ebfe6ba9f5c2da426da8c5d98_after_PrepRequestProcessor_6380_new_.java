        ...
        switch(type) {
            ...
            case OpCode.reconfig:
                ...
                if (configId != -1 && configId != lzks.self.getLastSeenQuorumVerifier().getVersion()) {
                    String var40 = "Reconfiguration from version " + configId + " failed -- last seen version is " + lzks.self.getLastSeenQuorumVerifier().getVersion();
                    throw new KeeperException.BadVersionException(var40);
                }
                ...
                if (newMembers != null) {
                    ...
                    if (!(lastSeenQV instanceof QuorumMaj)) {
                        String var40 = "Incremental reconfiguration requested but last configuration seen has a non-majority quorum system";
                        LOG.warn(var40);
                        throw new KeeperException.BadArgumentsException(var40);
                    }
                ...
                if (QuorumPeerConfig.isStandaloneEnabled() && request.qv.getVotingMembers().size() < 2) {
                    String var40 = "Reconfig failed - new configuration must include at least 2 followers";
                    LOG.warn(var40);
                    throw new KeeperException.BadArgumentsException(var40);
                } else if (request.qv.getVotingMembers().size() < 1) {
                    String var40 = "Reconfig failed - new configuration must include at least 1 follower";
                    LOG.warn(var40);
                    throw new KeeperException.BadArgumentsException(var40);
                }
