...
public class ContainerManager {
    ...
    protected Collection<String> getCandidates() {
        ...
        for (String containerPath : zkDb.getDataTree().getContainers()) {
            DataNode var4 = zkDb.getDataTree().getNode(containerPath);
            /*
             ...
             */
            if ((var4 != null) && (var4.stat.getCversion() > 0) && (var4.getChildren().size() == 0)) {
                candidates.add(containerPath);
            }
        }
        for (String ttlPath : zkDb.getDataTree().getTtls()) {
            DataNode var4 = zkDb.getDataTree().getNode(ttlPath);
            if (var4 != null) {
                Set<String> children = var4.getChildren();
                if ((children == null) || (children.size() == 0)) {
                    long elapsed = getElapsed(var4);
                    long ttl = EphemeralType.getTTL(var4.stat.getEphemeralOwner());
