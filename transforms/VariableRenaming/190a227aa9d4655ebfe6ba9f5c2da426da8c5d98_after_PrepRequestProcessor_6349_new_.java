    ...
    protected void pRequest2Txn(int type, long zxid, Request request, Record record, boolean deserialize) throws KeeperException, IOException, RequestProcessorException {
        ...
        switch(type) {
            ...
            case OpCode.deleteContainer:
                {
                    ...
                    ChangeRecord var9 = getRecordForPath(parentPath);
                    request.setTxn(new DeleteTxn(path));
                    var9 = var9.duplicate(request.getHdr().getZxid());
                    var9.childCount--;
                    var9.stat.setPzxid(request.getHdr().getZxid());
                    var9.precalculatedDigest = precalculateDigest(DigestOpCode.UPDATE, parentPath, var9.data, var9.stat);
                    addChangeRecord(var9);
            ...
            case OpCode.delete:
                ...
                String parentPath = getParentPathAndValidate(path);
                ChangeRecord var9 = getRecordForPath(parentPath);
                zks.checkACL(request.cnxn, var9.acl, ZooDefs.Perms.DELETE, request.authInfo, path, null);
                ChangeRecord nodeRecord = getRecordForPath(path);
                ...
                synchronized (zks.outstandingChanges) {
                    ...
                    for (String path2Delete : es) {
                        if (digestEnabled) {
                            ...
                            var9 = getRecordForPath(parentPath);
                            var9 = var9.duplicate(request.getHdr().getZxid());
                            var9.stat.setPzxid(request.getHdr().getZxid());
                            var9.precalculatedDigest = precalculateDigest(DigestOpCode.UPDATE, parentPath, var9.data, var9.stat);
                            addChangeRecord(var9);
