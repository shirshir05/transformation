...
class ClientCnxn {
    ...
    private void conLossPacket(Packet var0) {
        if (var0.replyHeader == null) {
            return;
        }
        switch(zooKeeper.state) {
            case AUTH_FAILED:
                var0.replyHeader.setErr(KeeperException.Code.AuthFailed);
                break;
            case CLOSED:
                var0.replyHeader.setErr(KeeperException.Code.SessionExpired);
                break;
            default:
                var0.replyHeader.setErr(KeeperException.Code.ConnectionLoss);
        }
        finishPacket(var0);
    }
