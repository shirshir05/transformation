...
public class QuorumCnxManager {
    ...
    public boolean receiveConnection(Socket sock) {
        ...
        try {
            ...
            var1 = din.readLong();
            if (var1 == QuorumPeer.OBSERVER_ID) {
                var1 = observerCounter--;
                LOG.info("Setting arbitrary identifier to observer: " + var1);
            }
        } catch (IOException e) {
            closeSocket(sock);
        ...
        if (var1 < self.getId()) {
             ...
             * new connection.
             */
            SendWorker sw = senderWorkerMap.get(var1);
            if (sw != null) {
                sw.finish();
            }
            /*
             ...
             */
            LOG.debug("Create new connection to server: " + var1);
            closeSocket(sock);
            connectOne(var1);
        } else {
            SendWorker sw = new SendWorker(sock, var1);
            RecvWorker rw = new RecvWorker(sock, var1, sw);
            sw.setRecv(rw);
            ...
            if (!queueSendMap.containsKey(var1)) {
                queueSendMap.put(var1, new ArrayBlockingQueue<ByteBuffer>(SEND_CAPACITY));
            }
            sw.start();
            rw.start();
            return true;
    ...
    public class Listener extends Thread {
        ...
        public void run() {
            ...
            if (!shutdown) {
                LOG.error("As I'm leaving the listener thread, " + "I won't be able to participate in leader " + "election any longer: " + self.quorumPeers.get(self.getId()).electionAddr);
            }
        }
