    ...
    protected void pRequest(Request request) throws RequestProcessorException {
        ...
        try {
                     ...
                     */
                        ...
                        } else /* Prep the request and convert to a Txn */
                        {
                            try {
                                ...
                                txn = request.getTxn();
                            } catch (KeeperException var23) {
                                ke = var23;
                                type = OpCode.error;
                                txn = new ErrorTxn(var23.code().intValue());
                                LOG.info("Got user-level KeeperException when processing " + request.toString() + " aborting remaining multi ops." + " Error Path:" + var23.getPath() + " Error:" + var23.getMessage());
                                request.setException(var23);
            ...
            if (request.getHdr() != null) {
                ...
                request.setTxn(new ErrorTxn(var23.code().intValue()));
            }
            LOG.info("Got user-level KeeperException when processing " + request.toString() + " Error Path:" + var23.getPath() + " Error:" + var23.getMessage());
            request.setException(var23);
        } catch (Exception var23) {
            // error to the user
            LOG.error("Failed to process " + request, var23);
