    ...
    public void run() {
        try {
            ...
            new Thread() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        LOG.warn("Unexpected interruption", var28);
                    }
                }
            }.start();
            while (true) {
                ...
                switch(qp.getType()) {
                    ...
                    case Leader.REVALIDATE:
                        ...
                        if (valid) {
                            ...
                            } catch (SessionExpiredException var28) {
                                LOG.error("Somehow session " + Long.toHexString(id) + " expired right after being renewed! (impossible)", var28);
                            }
                        }
                        if (LOG.isTraceEnabled()) {
            ...
            if (sock != null && !sock.isClosed()) {
                LOG.error("Unexpected exception causing shutdown while sock " + "still open", var28);
            }
        } catch (InterruptedException var28) {
            LOG.error("Unexpected exception causing shutdown", var28);
        } finally {
            ...
            try {
                ...
                LOG.warn("Ignoring unexpected exception", var28);
            }
            shutdown();
        }
    }
