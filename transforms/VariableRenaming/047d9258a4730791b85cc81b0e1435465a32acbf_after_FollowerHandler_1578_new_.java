...
public class FollowerHandler extends Thread {
    ...
    static public String packetToString(QuorumPacket p) {
        ...
        switch(p.getType()) {
            ...
            case Leader.PROPOSAL:
                ...
                TxnHeader var5 = new TxnHeader();
                try {
                    txn = ZooKeeperServer.deserializeTxn(ia, var5);
                } catch (IOException e) {
                    ZooLog.logException(e);
                }
                break;
    ...
    public void run() {
        try {
            ia = BinaryInputArchive.getArchive(new BufferedInputStream(s.getInputStream()));
            bufferedOutput = new BufferedOutputStream(s.getOutputStream());
            oa = BinaryOutputArchive.getArchive(bufferedOutput);
            QuorumPacket qp = new QuorumPacket();
            ia.readRecord(qp, "packet");
            if (qp.getType() != Leader.LASTZXID) {
                ZooLog.logError("First packet " + qp.toString() + " is not LASTZXID!");
                return;
            }
            ...
            new Thread() {
                ...
                public void run() {
                    Thread.currentThread().setName("Sender-" + s.getRemoteSocketAddress());
                    try {
                        sendPackets();
            ...
            while (true) {
                ...
                switch(qp.getType()) {
                    ...
                    case Leader.REVALIDATE:
                        ...
                        ZooLog.logTextTraceMessage("Session " + id + " is valid: " + valid, ZooLog.SESSION_TRACE_MASK);
                    ...
                    case Leader.REQUEST:
                        ...
                        int type = bb.getInt();
                        bb = bb.slice();
                        leader.zk.submitRequest(null, sessionId, type, cxid, bb, null);
                        break;
                    default:
                }
            }
     ...
     */
    public void ping() {
        QuorumPacket ping = new QuorumPacket(Leader.PING, leader.lastProposed, null, null);
        queuePacket(ping);
    }
