...
public class DataTree {
    ...
    public String createNode(String path, byte[] data, List<ACL> acl, long ephemeralOwner, long zxid, long time) throws KeeperException.NoNodeException, KeeperException.NodeExistsException {
        ...
        DataNode var10 = nodes.get(parentName);
        if (var10 == null) {
            throw new KeeperException.NoNodeException();
        }
        synchronized (var10) {
            if (var10.children.contains(childName)) {
                throw new KeeperException.NodeExistsException();
            }
            int cver = var10.stat.getCversion();
            cver++;
            var10.stat.setCversion(cver);
            var10.stat.setPzxid(zxid);
            Long longval = convertAcls(acl);
            DataNode child = new DataNode(var10, data, longval, stat);
            var10.children.add(childName);
