...
public class PrepRequestProcessor extends ZooKeeperCriticalThread implements RequestProcessor {
    ...
    private void pRequest2TxnCreate(int type, Request var1, Record record, boolean deserialize) throws IOException, KeeperException {
        if (deserialize) {
            ByteBufferInputStream.byteBuffer2Record(var1.var1, record);
        }
        int flags;
        String path;
        List<ACL> acl;
        ...
        validateCreateRequest(path, createMode, var1, ttl);
        String parentPath = validatePathForCreate(path, var1.sessionId);
        List<ACL> listACL = fixupACL(path, var1.authInfo, acl);
        ChangeRecord parentRecord = getRecordForPath(parentPath);
        zks.checkACL(var1.cnxn, parentRecord.acl, ZooDefs.Perms.CREATE, var1.authInfo, path, listACL);
        int parentCVersion = parentRecord.stat.getCversion();
        if (createMode.isSequential()) {
            path = path + String.format(Locale.ENGLISH, "%010d", parentCVersion);
        }
        validatePath(path, var1.sessionId);
        try {
        ...
        if (type == OpCode.createContainer) {
            var1.setTxn(new CreateContainerTxn(path, data, listACL, newCversion));
        } else if (type == OpCode.createTTL) {
            var1.setTxn(new CreateTTLTxn(path, data, listACL, newCversion, ttl));
        } else {
            var1.setTxn(new CreateTxn(path, data, listACL, createMode.isEphemeral(), newCversion));
        }
        TxnHeader hdr = var1.getHdr();
        ...
        if (createMode.isContainer()) {
            ...
            ephemeralOwner = var1.sessionId;
        }
        StatPersisted s = DataTree.createStat(hdr.getZxid(), hdr.getTime(), ephemeralOwner);
        parentRecord = parentRecord.duplicate(var1.getHdr().getZxid());
        parentRecord.childCount++;
        parentRecord.stat.setCversion(newCversion);
        parentRecord.stat.setPzxid(var1.getHdr().getZxid());
        parentRecord.precalculatedDigest = precalculateDigest(DigestOpCode.UPDATE, parentPath, parentRecord.data, parentRecord.stat);
        addChangeRecord(parentRecord);
        ChangeRecord nodeRecord = new ChangeRecord(var1.getHdr().getZxid(), path, s, 0, listACL);
        nodeRecord.data = data;
        nodeRecord.precalculatedDigest = precalculateDigest(DigestOpCode.ADD, path, nodeRecord.data, s);
        setTxnDigest(var1, nodeRecord.precalculatedDigest);
        addChangeRecord(nodeRecord);
    }
