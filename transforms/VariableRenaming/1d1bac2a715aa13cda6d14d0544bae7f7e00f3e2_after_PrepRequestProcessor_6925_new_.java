...
public class PrepRequestProcessor extends Thread implements RequestProcessor {
    ...
    private boolean fixupACL(List<Id> authInfo, List<ACL> acl) {
        ...
        while (it.hasNext()) {
            ACL a = it.next();
            Id var5 = a.getId();
            if (var5.getScheme().equals("world") && var5.getId().equals("anyone")) {
            } else if (var5.getScheme().equals("auth")) {
                ...
                AuthenticationProvider ap = ProviderRegistry.getProvider(var5.getScheme());
                if (ap == null) {
                    return false;
                }
                if (!ap.isValid(var5.getId())) {
                    return false;
                }
            }
        }
