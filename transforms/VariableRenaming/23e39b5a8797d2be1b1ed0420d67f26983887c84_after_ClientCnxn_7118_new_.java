...
class ClientCnxn {
    ...
    Packet queuePacket(RequestHeader h, ReplyHeader r, Record var2, Record response, AsyncCallback cb, String path, Object ctx, WatchRegistration watchRegistration) {
        Packet packet = null;
        synchronized (outgoingQueue) {
            if (h.getType() != OpCode.ping && h.getType() != OpCode.auth) {
                h.setXid(getXid());
            }
            packet = new Packet(h, r, var2, response, null, watchRegistration);
            packet.cb = cb;
            packet.ctx = ctx;
            packet.path = path;
            if (!zooKeeper.state.isAlive()) {
