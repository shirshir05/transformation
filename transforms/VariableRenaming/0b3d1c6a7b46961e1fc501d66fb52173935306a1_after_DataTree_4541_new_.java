        ...
        try {
            ...
            switch(header.getType()) {
                ...
                case OpCode.multi:
                    ...
                    for (Txn var15 : txns) {
                        ByteBuffer bb = ByteBuffer.wrap(var15.getData());
                        Record record = null;
                        switch(var15.getType()) {
                            ...
                            case OpCode.check:
                                ...
                                break;
                            default:
                                throw new IOException("Invalid type of op: " + var15.getType());
                        }
                        assert (record != null);
                        ZooKeeperServer.byteBuffer2Record(bb, record);
                        if (failed && var15.getType() != OpCode.error) {
                            int ec = post_failed ? Code.RUNTIMEINCONSISTENCY.intValue() : Code.OK.intValue();
                            var15.setType(OpCode.error);
                            record = new ErrorTxn(ec);
                        }
                        if (failed) {
                            assert (var15.getType() == OpCode.error);
                        }
                        TxnHeader subHdr = new TxnHeader(header.getClientId(), header.getCxid(), header.getZxid(), header.getTime(), var15.getType());
