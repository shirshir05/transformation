...
public class PrepRequestProcessor extends Thread implements RequestProcessor {
    ...
    protected void pRequest2Txn(int type, long zxid, Request request, Record record) throws KeeperException {
        ...
        switch(type) {
            ...
            case OpCode.setACL:
                zks.sessionTracker.checkSession(request.sessionId, request.getOwner());
                SetACLRequest var21 = (SetACLRequest) record;
                path = var21.getPath();
                if (!fixupACL(request.authInfo, var21.getAcl())) {
                    throw new KeeperException.InvalidACLException(path);
                }
                nodeRecord = getRecordForPath(path);
                checkACL(zks, nodeRecord.acl, ZooDefs.Perms.ADMIN, request.authInfo);
                version = var21.getVersion();
                currentVersion = nodeRecord.stat.getAversion();
                if (version != -1 && version != currentVersion) {
                    throw new KeeperException.BadVersionException(path);
                }
                version = currentVersion + 1;
                request.txn = new SetACLTxn(path, var21.getAcl(), version);
                nodeRecord = nodeRecord.duplicate(request.hdr.getZxid());
