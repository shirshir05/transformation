    ...
    public void processRequest(Request request) {
        ...
        synchronized (zks.outstandingChanges) {
            while (!zks.outstandingChanges.isEmpty() && zks.outstandingChanges.get(0).zxid <= request.zxid) {
                ChangeRecord var3 = zks.outstandingChanges.remove(0);
                if (var3.zxid < request.zxid) {
                    LOG.warn("Zxid outstanding " + var3.zxid + " is less than current " + request.zxid);
                }
                if (zks.outstandingChangesForPath.get(var3.path) == var3) {
                    zks.outstandingChangesForPath.remove(var3.path);
                }
            }
            if (request.hdr != null) {
