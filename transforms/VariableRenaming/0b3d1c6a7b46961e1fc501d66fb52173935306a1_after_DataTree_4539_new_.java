        ...
        try {
            ...
            switch(header.getType()) {
                ...
                case OpCode.multi:
                    ...
                    for (Txn var13 : txns) {
                        ByteBuffer bb = ByteBuffer.wrap(var13.getData());
                        Record record = null;
                        switch(var13.getType()) {
                            ...
                            case OpCode.check:
                                ...
                                break;
                            default:
                                throw new IOException("Invalid type of op: " + var13.getType());
                        }
                        assert (record != null);
                        ZooKeeperServer.byteBuffer2Record(bb, record);
                        if (failed && var13.getType() != OpCode.error) {
                            int ec = post_failed ? Code.RUNTIMEINCONSISTENCY.intValue() : Code.OK.intValue();
                            var13.setType(OpCode.error);
                            record = new ErrorTxn(ec);
                        }
                        if (failed) {
                            assert (var13.getType() == OpCode.error);
                        }
                        TxnHeader subHdr = new TxnHeader(header.getClientId(), header.getCxid(), header.getZxid(), header.getTime(), var13.getType());
