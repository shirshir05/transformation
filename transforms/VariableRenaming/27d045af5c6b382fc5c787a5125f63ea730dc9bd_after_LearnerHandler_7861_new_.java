            ...
            try {
                ...
                if (proposals.size() != 0) {
                    if ((maxCommittedLog >= peerLastZxid) && (minCommittedLog <= peerLastZxid)) {
                        ...
                        for (Proposal var25 : proposals) {
                            // skip the proposals the peer already has
                            if (var25.packet.getZxid() <= peerLastZxid) {
                                prevProposalZxid = var25.packet.getZxid();
                                continue;
                            } else {
                                ...
                                queuePacket(var25.packet);
                                QuorumPacket qcommit = new QuorumPacket(Leader.COMMIT, var25.packet.getZxid(), null, null);
                                queuePacket(qcommit);
                            }
                        }
