...
public final class StaticHostProvider implements HostProvider {
    ...
    private List<InetSocketAddress> resolveAndShuffle(Collection<InetSocketAddress> serverAddresses) throws UnknownHostException {
        ...
        for (InetSocketAddress address : serverAddresses) {
            ...
            for (InetAddress var5 : resolvedAddresses) {
                // the toString() method.
                if (var5.toString().startsWith("/") && var5.getAddress() != null) {
                    tmpList.add(new InetSocketAddress(InetAddress.getByAddress(address.getHostName(), var5.getAddress()), address.getPort()));
                } else {
                    tmpList.add(new InetSocketAddress(var5.getHostAddress(), address.getPort()));
                }
            }
        }
