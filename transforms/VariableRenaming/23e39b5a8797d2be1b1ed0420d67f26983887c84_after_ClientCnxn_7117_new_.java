...
class ClientCnxn {
    ...
    Packet queuePacket(RequestHeader h, ReplyHeader var1, Record request, Record response, AsyncCallback cb, String path, Object ctx, WatchRegistration watchRegistration) {
        Packet packet = null;
        synchronized (outgoingQueue) {
            if (h.getType() != OpCode.ping && h.getType() != OpCode.auth) {
                h.setXid(getXid());
            }
            packet = new Packet(h, var1, request, response, null, watchRegistration);
            packet.cb = cb;
            packet.ctx = ctx;
            packet.path = path;
            if (!zooKeeper.state.isAlive()) {
