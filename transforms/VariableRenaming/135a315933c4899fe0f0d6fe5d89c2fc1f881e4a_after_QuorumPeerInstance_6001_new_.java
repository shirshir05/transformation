...
class QuorumPeerInstance implements Instance {
    ...
    public void configure(String params) {
        if (clientAddr == null) {
            ...
            for (int var17 = 0; var17 < parts.length; var17++) {
                // parts[i] == "host:leaderPort:leaderElectionPort;clientPort"
                String[] subparts = (parts[var17].split(";"))[0].split(":");
                String clientPort = (parts[var17].split(";"))[1];
                peers.put(Long.valueOf(var17), new QuorumServer(var17, new InetSocketAddress(subparts[0], Integer.parseInt(subparts[1])), new InetSocketAddress(subparts[0], Integer.parseInt(subparts[2])), new InetSocketAddress(subparts[0], Integer.parseInt(clientPort))));
            }
            try {
                if (LOG.isDebugEnabled()) {
