...
public class PrepRequestProcessor extends Thread implements RequestProcessor {
    ...
    protected void pRequest(Request request) {
        ...
        try {
                     ...
                     */
                        ...
                        } else /* Prep the request and convert to a Txn */
                        {
                            try {
                                ...
                                request.hdr.setType(OpCode.error);
                                request.txn = new ErrorTxn(var19.code().intValue());
                                LOG.error(">>>> Got user-level KeeperException when processing " + request.toString() + " Error Path:" + var19.getPath() + " Error:" + var19.getMessage());
                                LOG.error(">>>> ABORTING remaing MultiOp ops");
                                request.setException(var19);
            ...
            LOG.info("Got user-level KeeperException when processing " + request.toString() + " Error Path:" + var19.getPath() + " Error:" + var19.getMessage());
            request.setException(var19);
        } catch (Exception var19) {
            // error to the user
            LOG.error("Failed to process " + request, var19);
