    ...
    protected void pRequest2Txn(int type, long zxid, Request request, Record record, boolean deserialize) throws KeeperException, IOException, RequestProcessorException {
        ...
        switch(type) {
            ...
            case OpCode.deleteContainer:
                {
                    ...
                    ChangeRecord var13 = getRecordForPath(parentPath);
                    request.setTxn(new DeleteTxn(path));
                    var13 = var13.duplicate(request.getHdr().getZxid());
                    var13.childCount--;
                    var13.stat.setPzxid(request.getHdr().getZxid());
                    var13.precalculatedDigest = precalculateDigest(DigestOpCode.UPDATE, parentPath, var13.data, var13.stat);
            ...
            case OpCode.delete:
                ...
                String parentPath = getParentPathAndValidate(path);
                ChangeRecord var13 = getRecordForPath(parentPath);
                zks.checkACL(request.cnxn, var13.acl, ZooDefs.Perms.DELETE, request.authInfo, path, null);
                ChangeRecord nodeRecord = getRecordForPath(path);
                ...
                synchronized (zks.outstandingChanges) {
                    ...
                    for (String path2Delete : es) {
                        if (digestEnabled) {
                            ...
                            var13 = getRecordForPath(parentPath);
                            var13 = var13.duplicate(request.getHdr().getZxid());
                            var13.stat.setPzxid(request.getHdr().getZxid());
                            var13.precalculatedDigest = precalculateDigest(DigestOpCode.UPDATE, parentPath, var13.data, var13.stat);
                            addChangeRecord(var13);
