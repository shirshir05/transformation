...
public class ClientCnxnSocketNIO extends ClientCnxnSocket {
    ...
    @Override
    void doTransport(int waitTimeOut, List<Packet> pendingQueue, LinkedList<Packet> outgoingQueue) throws IOException, InterruptedException {
        ...
        updateNow();
        for (SelectionKey var4 : selected) {
            SocketChannel sc = ((SocketChannel) var4.channel());
            if ((var4.readyOps() & SelectionKey.OP_CONNECT) != 0) {
                if (sc.finishConnect()) {
                    updateLastSendAndHeard();
                    sendThread.primeConnection();
                }
            } else if ((var4.readyOps() & (SelectionKey.OP_READ | SelectionKey.OP_WRITE)) != 0) {
