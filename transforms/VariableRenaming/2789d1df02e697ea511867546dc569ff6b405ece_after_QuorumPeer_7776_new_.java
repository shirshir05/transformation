...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    class ResponderThread extends ZooKeeperThread {
        ...
        public void run() {
            try {
                byte[] var0 = new byte[36];
                ByteBuffer responseBuffer = ByteBuffer.wrap(var0);
                DatagramPacket packet = new DatagramPacket(var0, var0.length);
                while (running) {
    ...
    public QuorumPeer(Map<Long, QuorumServer> quorumPeers, File snapDir, File logDir, int clientPort, int electionAlg, long myid, int tickTime, int initLimit, int syncLimit) throws IOException {
        this(quorumPeers, snapDir, logDir, electionAlg, myid, tickTime, initLimit, syncLimit, false, ServerCnxnFactory.createFactory(new InetSocketAddress(clientPort), -1), new QuorumMaj(quorumPeers));
    }
    ...
    public synchronized void setLastSeenQuorumVerifier(QuorumVerifier qv, boolean writeToDisk) {
        ...
        if (writeToDisk) {
            try {
                QuorumPeerConfig.writeDynamicConfig(getNextDynamicConfigFilename(), qv, true);
            } catch (IOException e) {
