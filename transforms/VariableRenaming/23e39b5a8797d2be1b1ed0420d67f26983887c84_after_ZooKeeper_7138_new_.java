    ...
    public void processWatchEvent(WatcherEvent event) {
        ...
        if (event.getState() != Watcher.Event.KeeperStateSyncConnected) {
            synchronized (dataWatches) {
                for (Set<Watcher> var3 : dataWatches.values()) {
                    for (Watcher watcher : var3) {
            ...
            synchronized (childWatches) {
                for (Set<Watcher> var3 : childWatches.values()) {
                    for (Watcher watcher : var3) {
                        watcher.process(event);
                    }
                }
        ...
        switch(event.getType()) {
            ...
            case Watcher.Event.EventNodeCreated:
                synchronized (dataWatches) {
                    var3 = dataWatches.remove(event.getPath());
                }
                break;
            case Watcher.Event.EventNodeChildrenChanged:
                synchronized (childWatches) {
                    var3 = childWatches.remove(event.getPath());
                }
                break;
