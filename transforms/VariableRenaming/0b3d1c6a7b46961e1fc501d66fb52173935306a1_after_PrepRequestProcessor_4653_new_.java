    ...
    protected void pRequest2Txn(int type, long zxid, Request request, Record record) throws KeeperException {
        ...
        switch(type) {
            case OpCode.create:
                ...
                String var5 = createRequest.getPath();
                int lastSlash = var5.lastIndexOf('/');
                if (lastSlash == -1 || var5.indexOf('\0') != -1 || failCreate) {
                    LOG.info("Invalid path " + var5 + " with session 0x" + Long.toHexString(request.sessionId));
                    throw new KeeperException.BadArgumentsException(var5);
                }
                if (!fixupACL(request.authInfo, createRequest.getAcl())) {
                    throw new KeeperException.InvalidACLException(var5);
                }
                String parentPath = var5.substring(0, lastSlash);
                ChangeRecord parentRecord = getRecordForPath(parentPath);
                checkACL(zks, parentRecord.acl, ZooDefs.Perms.CREATE, request.authInfo);
                int parentCVersion = parentRecord.stat.getCversion();
                CreateMode createMode = CreateMode.fromFlag(createRequest.getFlags());
                if (createMode.isSequential()) {
                    var5 = var5 + String.format("%010d", parentCVersion);
                }
                try {
                    PathUtils.validatePath(var5);
                } catch (IllegalArgumentException ie) {
                    LOG.info("Invalid path " + var5 + " with session 0x" + Long.toHexString(request.sessionId));
                    throw new KeeperException.BadArgumentsException(var5);
                }
                try {
                    if (getRecordForPath(var5) != null) {
                        throw new KeeperException.NodeExistsException(var5);
                    }
                } catch (KeeperException.NoNodeException e) {
                }
                boolean ephemeralParent = parentRecord.stat.getEphemeralOwner() != 0;
                if (ephemeralParent) {
                    throw new KeeperException.NoChildrenForEphemeralsException(var5);
                }
                int newCversion = parentRecord.stat.getCversion() + 1;
                request.txn = new CreateTxn(var5, createRequest.getData(), createRequest.getAcl(), createMode.isEphemeral(), newCversion);
                ...
                addChangeRecord(new ChangeRecord(request.hdr.getZxid(), var5, s, 0, createRequest.getAcl()));
                break;
            case OpCode.delete:
                zks.sessionTracker.checkSession(request.sessionId, request.getOwner());
                DeleteRequest deleteRequest = (DeleteRequest) record;
                var5 = deleteRequest.getPath();
                lastSlash = var5.lastIndexOf('/');
                if (lastSlash == -1 || var5.indexOf('\0') != -1 || zks.getZKDatabase().isSpecialPath(var5)) {
                    throw new KeeperException.BadArgumentsException(var5);
                }
                parentPath = var5.substring(0, lastSlash);
                parentRecord = getRecordForPath(parentPath);
                ChangeRecord nodeRecord = getRecordForPath(var5);
                checkACL(zks, parentRecord.acl, ZooDefs.Perms.DELETE, request.authInfo);
                int version = deleteRequest.getVersion();
                if (version != -1 && nodeRecord.stat.getVersion() != version) {
                    throw new KeeperException.BadVersionException(var5);
                }
                if (nodeRecord.childCount > 0) {
                    throw new KeeperException.NotEmptyException(var5);
                }
                request.txn = new DeleteTxn(var5);
                parentRecord = parentRecord.duplicate(request.hdr.getZxid());
                parentRecord.childCount--;
                addChangeRecord(parentRecord);
                addChangeRecord(new ChangeRecord(request.hdr.getZxid(), var5, null, -1, null));
                break;
            case OpCode.setData:
                zks.sessionTracker.checkSession(request.sessionId, request.getOwner());
                SetDataRequest setDataRequest = (SetDataRequest) record;
                var5 = setDataRequest.getPath();
                nodeRecord = getRecordForPath(var5);
                ...
                request.txn = new SetDataTxn(var5, setDataRequest.getData(), version);
            ...
            case OpCode.setACL:
                ...
                version = currentVersion + 1;
                request.txn = new SetACLTxn(var5, setAclRequest.getAcl(), version);
                nodeRecord = nodeRecord.duplicate(request.hdr.getZxid());
            ...
            case OpCode.check:
                ...
                var5 = checkVersionRequest.getPath();
                nodeRecord = getRecordForPath(var5);
                ...
                version = currentVersion + 1;
                request.txn = new CheckVersionTxn(var5, version);
                break;
        }
    }
