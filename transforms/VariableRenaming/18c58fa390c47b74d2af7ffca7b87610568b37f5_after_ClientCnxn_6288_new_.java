            ...
            while (state.isAlive()) {
                try {
                    ...
                    if (state == States.CONNECTED) {
                        var0 = readTimeout - clientCnxnSocket.getIdleRecv();
                    } else {
                        var0 = connectTimeout - clientCnxnSocket.getIdleRecv();
                    }
                    if (var0 <= 0) {
                    ...
                    if (state == States.CONNECTED) {
                        ...
                        if (timeToNextPing <= 0) {
                            ...
                            if (timeToNextPing < var0) {
                                var0 = timeToNextPing;
                            }
                        }
                    }
                    clientCnxnSocket.doTransport(var0, pendingQueue, outgoingQueue);
                } catch (Exception e) {
                    if (closing) {
                        if (LOG.isDebugEnabled()) {
