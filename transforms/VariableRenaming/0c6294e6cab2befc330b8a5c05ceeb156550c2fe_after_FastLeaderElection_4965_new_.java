...
public class FastLeaderElection implements Election {
    ...
    static public class ToSend {
        ...
        ToSend(mType type, long leader, long zxid, long epoch, ServerState state, InetAddress addr) {
            this.leader = leader;
            this.zxid = zxid;
            this.epoch = epoch;
            this.state = state;
            ...
            public void run() {
                ...
                while (true) {
                    ...
                    try {
            			 ...
            			 */
                        if ((messenger.lastEpoch <= n.epoch) && ((n.zxid > messenger.lastProposedZxid) || ((n.zxid == messenger.lastProposedZxid) && (n.leader > messenger.lastProposedLeader)))) {
                        ...
                        if (self.getPeerState() == QuorumPeer.ServerState.LOOKING) {
                            ...
                            if ((ackstate == QuorumPeer.ServerState.LOOKING) && (n.epoch < logicalclock)) {
                                ToSend notmsg = new ToSend(ToSend.mType.notification, proposedLeader, proposedZxid, logicalclock, self.getPeerState(), response.addr);
                            ...
                            if (ackstate == QuorumPeer.ServerState.LOOKING) {
                                ToSend notmsg = new ToSend(ToSend.mType.notification, current.id, current.zxid, logicalclock, self.getPeerState(), response.addr);
                                sendqueue.offer(notmsg);
                            }
                        }
    ...
    private void sendNotifications() {
        for (QuorumServer server : self.quorumPeers) {
            ...
            ToSend notmsg = new ToSend(ToSend.mType.notification, proposedLeader, proposedZxid, logicalclock, QuorumPeer.ServerState.LOOKING, saddr);
            sendqueue.offer(notmsg);
        }
    }
     ...
     */
    private boolean termPredicate(HashMap<InetAddress, Vote> votes, long l, long zxid) {
        int count = 0;
                ...
                switch(n.state) {
                    case LOOKING:
                        ...
                        if (self.quorumPeers.size() == recvset.size()) {
                            ...
                            while ((!recvqueue.isEmpty()) && !totalOrderPredicate(recvqueue.peek().leader, recvqueue.peek().zxid)) {
                                recvqueue.poll();
                            }
