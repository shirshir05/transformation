...
public class ZooKeeper {
     ...
     */
    public void processWatchEvent(WatcherEvent var0) {
        // clear the watches if we are not connected
        if (var0.getState() != Watcher.Event.KeeperStateSyncConnected) {
            synchronized (dataWatches) {
                for (Set<Watcher> watchers : dataWatches.values()) {
                    for (Watcher watcher : watchers) {
                        watcher.process(var0);
                    }
                }
                dataWatches.clear();
            }
        ...
        switch(var0.getType()) {
            ...
            case Watcher.Event.EventNodeDeleted:
                ...
                synchronized (childWatches) {
                    cwatches = childWatches.remove(var0.getPath());
                }
                if (cwatches != null) {
                    if (watchers == null) {
                        watchers = cwatches;
                ...
                break;
            default:
                String msg = "Unhandled watch event type " + var0.getType();
                LOG.error(msg);
