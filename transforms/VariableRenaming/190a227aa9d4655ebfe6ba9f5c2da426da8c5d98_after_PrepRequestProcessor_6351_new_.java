    ...
    protected void pRequest2Txn(int type, long zxid, Request request, Record record, boolean deserialize) throws KeeperException, IOException, RequestProcessorException {
        ...
        switch(type) {
            ...
            case OpCode.deleteContainer:
                {
                    String var11 = new String(request.request.array(), UTF_8);
                    String parentPath = getParentPathAndValidate(var11);
                    ChangeRecord nodeRecord = getRecordForPath(var11);
                    if (nodeRecord.childCount > 0) {
                        throw new KeeperException.NotEmptyException(var11);
                    }
                    if (EphemeralType.get(nodeRecord.stat.getEphemeralOwner()) == EphemeralType.NORMAL) {
                        throw new KeeperException.BadVersionException(var11);
                    }
                    ChangeRecord parentRecord = getRecordForPath(parentPath);
                    request.setTxn(new DeleteTxn(var11));
                    ...
                    nodeRecord = new ChangeRecord(request.getHdr().getZxid(), var11, null, -1, null);
                    nodeRecord.precalculatedDigest = precalculateDigest(DigestOpCode.REMOVE, var11);
            ...
            case OpCode.delete:
                ...
                String var11 = deleteRequest.getPath();
                String parentPath = getParentPathAndValidate(var11);
                ChangeRecord parentRecord = getRecordForPath(parentPath);
                zks.checkACL(request.cnxn, parentRecord.acl, ZooDefs.Perms.DELETE, request.authInfo, var11, null);
                ChangeRecord nodeRecord = getRecordForPath(var11);
                checkAndIncVersion(nodeRecord.stat.getVersion(), deleteRequest.getVersion(), var11);
                if (nodeRecord.childCount > 0) {
                    throw new KeeperException.NotEmptyException(var11);
                }
                request.setTxn(new DeleteTxn(var11));
            ...
            case OpCode.setData:
                ...
                var11 = setDataRequest.getPath();
                validatePath(var11, request.sessionId);
                nodeRecord = getRecordForPath(var11);
                zks.checkACL(request.cnxn, nodeRecord.acl, ZooDefs.Perms.WRITE, request.authInfo, var11, null);
                zks.checkQuota(var11, setDataRequest.getData(), OpCode.setData);
                int newVersion = checkAndIncVersion(nodeRecord.stat.getVersion(), setDataRequest.getVersion(), var11);
                request.setTxn(new SetDataTxn(var11, setDataRequest.getData(), newVersion));
                ...
                nodeRecord.precalculatedDigest = precalculateDigest(DigestOpCode.UPDATE, var11, nodeRecord.data, nodeRecord.stat);
                setTxnDigest(request, nodeRecord.precalculatedDigest);
                addChangeRecord(nodeRecord);
                break;
            ...
            case OpCode.setACL:
                ...
                var11 = setAclRequest.getPath();
                validatePath(var11, request.sessionId);
                List<ACL> listACL = fixupACL(var11, request.authInfo, setAclRequest.getAcl());
                nodeRecord = getRecordForPath(var11);
                zks.checkACL(request.cnxn, nodeRecord.acl, ZooDefs.Perms.ADMIN, request.authInfo, var11, listACL);
                newVersion = checkAndIncVersion(nodeRecord.stat.getAversion(), setAclRequest.getVersion(), var11);
                request.setTxn(new SetACLTxn(var11, listACL, newVersion));
                nodeRecord = nodeRecord.duplicate(request.getHdr().getZxid());
            ...
            case OpCode.check:
                ...
                var11 = checkVersionRequest.getPath();
                validatePath(var11, request.sessionId);
                nodeRecord = getRecordForPath(var11);
                zks.checkACL(request.cnxn, nodeRecord.acl, ZooDefs.Perms.READ, request.authInfo, var11, null);
                request.setTxn(new CheckVersionTxn(var11, checkAndIncVersion(nodeRecord.stat.getVersion(), checkVersionRequest.getVersion(), var11)));
                break;
            default:
