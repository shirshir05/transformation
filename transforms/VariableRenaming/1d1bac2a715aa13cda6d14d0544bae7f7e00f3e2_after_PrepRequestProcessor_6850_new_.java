    ...
    protected void pRequest2Txn(int type, long zxid, Request request, Record record) throws KeeperException {
        ...
        switch(type) {
            case OpCode.create:
                ...
                List<ACL> var7 = removeDuplicates(createRequest.getAcl());
                if (!fixupACL(request.authInfo, var7)) {
                ...
                request.txn = new CreateTxn(path, createRequest.getData(), var7, createMode.isEphemeral(), newCversion);
                ...
                addChangeRecord(new ChangeRecord(request.hdr.getZxid(), path, s, 0, var7));
                break;
            case OpCode.delete:
                zks.sessionTracker.checkSession(request.sessionId, request.getOwner());
            ...
            case OpCode.setACL:
                ...
                var7 = removeDuplicates(setAclRequest.getAcl());
                if (!fixupACL(request.authInfo, var7)) {
                    throw new KeeperException.InvalidACLException(path);
                }
                nodeRecord = getRecordForPath(path);
                ...
                version = currentVersion + 1;
                request.txn = new SetACLTxn(path, var7, version);
                nodeRecord = nodeRecord.duplicate(request.hdr.getZxid());
