...
public class Bookie extends Thread {
    ...
    public void addEntry(ByteBuffer entry, AddCallback cb, Object ctx, byte[] masterKey) throws IOException, BookieException {
        long var4 = entry.getLong();
        LedgerDescriptor handle = getHandle(var4, false, masterKey);
        if (!masterKeys.get(var4).equals(ByteBuffer.wrap(masterKey))) {
        ...
        try {
            ...
            if (LOG.isTraceEnabled()) {
                LOG.trace("Adding " + entryId + "@" + var4);
            }
            queue.add(new QueueEntry(entry, var4, entryId, cb, ctx));
        } finally {
            putHandle(handle);
        }
    }
