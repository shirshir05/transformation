...
class ClientCnxn {
    ...
    public void addAuthInfo(String var0, byte[] auth) {
        authInfo.add(new AuthData(var0, auth));
        if (zooKeeper.state == States.CONNECTED) {
            queuePacket(new RequestHeader(-4, OpCode.auth), null, new AuthPacket(0, var0, auth), null, null, null, null, null);
        }
    }
}
