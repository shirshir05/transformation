        ...
        public void run() {
            while (!ss.socket().isClosed()) {
                try {
                    ...
                    for (SelectionKey var2 : selectedList) {
                        if ((var2.readyOps() & SelectionKey.OP_ACCEPT) != 0) {
                            SocketChannel sc = ((ServerSocketChannel) var2.channel()).accept();
                        ...
                        } else if ((var2.readyOps() & (SelectionKey.OP_READ | SelectionKey.OP_WRITE)) != 0) {
                            NIOServerCnxn c = (NIOServerCnxn) var2.attachment();
                            c.doIO(var2);
                        } else {
                            if (LOG.isDebugEnabled()) {
                                LOG.debug("Unexpected ops in select " + var2.readyOps());
                            }
                        }
                    }
                    selected.clear();
