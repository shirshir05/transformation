...
public class FileTxnSnapLog {
    ...
    public long restore(DataTree dt, Map<Long, Integer> sessions, PlayBackListener listener) throws IOException {
        ...
        while (true) {
            ...
            if (var6.getZxid() <= highestZxid && highestZxid != 0) {
                LOG.error(highestZxid + "(higestZxid) >= " + var6.getZxid() + "(next log) for type " + var6.getType());
            } else {
                highestZxid = var6.getZxid();
            }
            processTransaction(var6, dt, sessions, itr.getTxn());
            if (!itr.next())
                break;
        }
        return highestZxid;
