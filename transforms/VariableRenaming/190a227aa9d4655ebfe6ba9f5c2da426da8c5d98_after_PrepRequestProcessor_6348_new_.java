    ...
    protected void pRequest2Txn(int type, long zxid, Request request, Record record, boolean deserialize) throws KeeperException, IOException, RequestProcessorException {
        ...
        switch(type) {
            ...
            case OpCode.deleteContainer:
                {
                    ...
                    ChangeRecord var8 = getRecordForPath(path);
                    if (var8.childCount > 0) {
                        throw new KeeperException.NotEmptyException(path);
                    }
                    if (EphemeralType.get(var8.stat.getEphemeralOwner()) == EphemeralType.NORMAL) {
                    ...
                    var8 = new ChangeRecord(request.getHdr().getZxid(), path, null, -1, null);
                    var8.precalculatedDigest = precalculateDigest(DigestOpCode.REMOVE, path);
                    setTxnDigest(request, var8.precalculatedDigest);
                    addChangeRecord(var8);
                    break;
                }
            case OpCode.delete:
                ...
                ChangeRecord var8 = getRecordForPath(path);
                checkAndIncVersion(var8.stat.getVersion(), deleteRequest.getVersion(), path);
                if (var8.childCount > 0) {
                    throw new KeeperException.NotEmptyException(path);
                }
                request.setTxn(new DeleteTxn(path));
            ...
            case OpCode.setData:
                ...
                var8 = getRecordForPath(path);
                zks.checkACL(request.cnxn, var8.acl, ZooDefs.Perms.WRITE, request.authInfo, path, null);
                zks.checkQuota(path, setDataRequest.getData(), OpCode.setData);
                int newVersion = checkAndIncVersion(var8.stat.getVersion(), setDataRequest.getVersion(), path);
                request.setTxn(new SetDataTxn(path, setDataRequest.getData(), newVersion));
                var8 = var8.duplicate(request.getHdr().getZxid());
                var8.stat.setVersion(newVersion);
                var8.stat.setMtime(request.getHdr().getTime());
                var8.stat.setMzxid(zxid);
                var8.data = setDataRequest.getData();
                var8.precalculatedDigest = precalculateDigest(DigestOpCode.UPDATE, path, var8.data, var8.stat);
                setTxnDigest(request, var8.precalculatedDigest);
                addChangeRecord(var8);
                break;
            case OpCode.reconfig:
                ...
                var8 = getRecordForPath(ZooDefs.CONFIG_NODE);
                zks.checkACL(request.cnxn, var8.acl, ZooDefs.Perms.WRITE, request.authInfo, null, null);
                SetDataTxn setDataTxn = new SetDataTxn(ZooDefs.CONFIG_NODE, request.qv.toString().getBytes(), -1);
                request.setTxn(setDataTxn);
                var8 = var8.duplicate(request.getHdr().getZxid());
                var8.stat.setVersion(-1);
                var8.stat.setMtime(request.getHdr().getTime());
                var8.stat.setMzxid(zxid);
                var8.data = setDataTxn.getData();
                // perspective since config node is not covered by the digests.
                var8.precalculatedDigest = precalculateDigest(DigestOpCode.NOOP, ZooDefs.CONFIG_NODE, var8.data, var8.stat);
                setTxnDigest(request, var8.precalculatedDigest);
                addChangeRecord(var8);
                break;
            case OpCode.setACL:
                ...
                var8 = getRecordForPath(path);
                zks.checkACL(request.cnxn, var8.acl, ZooDefs.Perms.ADMIN, request.authInfo, path, listACL);
                newVersion = checkAndIncVersion(var8.stat.getAversion(), setAclRequest.getVersion(), path);
                request.setTxn(new SetACLTxn(path, listACL, newVersion));
                var8 = var8.duplicate(request.getHdr().getZxid());
                var8.stat.setAversion(newVersion);
                var8.precalculatedDigest = precalculateDigest(DigestOpCode.UPDATE, path, var8.data, var8.stat);
                setTxnDigest(request, var8.precalculatedDigest);
                addChangeRecord(var8);
                break;
            ...
            case OpCode.closeSession:
                ...
                synchronized (zks.outstandingChanges) {
                    ...
                    for (String path2Delete : es) {
                        ...
                        var8 = new ChangeRecord(request.getHdr().getZxid(), path2Delete, null, 0, null);
                        var8.precalculatedDigest = precalculateDigest(DigestOpCode.REMOVE, path2Delete);
                        addChangeRecord(var8);
                    }
            ...
            case OpCode.check:
                ...
                validatePath(path, request.sessionId);
                var8 = getRecordForPath(path);
                zks.checkACL(request.cnxn, var8.acl, ZooDefs.Perms.READ, request.authInfo, path, null);
                request.setTxn(new CheckVersionTxn(path, checkAndIncVersion(var8.stat.getVersion(), checkVersionRequest.getVersion(), path)));
                break;
            default:
