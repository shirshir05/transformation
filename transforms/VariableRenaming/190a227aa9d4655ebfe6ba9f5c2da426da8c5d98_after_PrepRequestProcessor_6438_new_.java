...
public class PrepRequestProcessor extends ZooKeeperCriticalThread implements RequestProcessor {
    ...
    private void pRequest2TxnCreate(int type, Request request, Record record, boolean deserialize) throws IOException, KeeperException {
        ...
        CreateMode var11 = CreateMode.fromFlag(flags);
        validateCreateRequest(path, var11, request, ttl);
        String parentPath = validatePathForCreate(path, request.sessionId);
        List<ACL> listACL = fixupACL(path, request.authInfo, acl);
        ChangeRecord parentRecord = getRecordForPath(parentPath);
        zks.checkACL(request.cnxn, parentRecord.acl, ZooDefs.Perms.CREATE, request.authInfo, path, listACL);
        int parentCVersion = parentRecord.stat.getCversion();
        if (var11.isSequential()) {
        ...
        if (type == OpCode.createContainer) {
            ...
            request.setTxn(new CreateTxn(path, data, listACL, var11.isEphemeral(), newCversion));
        }
        TxnHeader hdr = request.getHdr();
        long ephemeralOwner = 0;
        if (var11.isContainer()) {
            ephemeralOwner = EphemeralType.CONTAINER_EPHEMERAL_OWNER;
        } else if (var11.isTTL()) {
            ephemeralOwner = EphemeralType.TTL.toEphemeralOwner(ttl);
        } else if (var11.isEphemeral()) {
