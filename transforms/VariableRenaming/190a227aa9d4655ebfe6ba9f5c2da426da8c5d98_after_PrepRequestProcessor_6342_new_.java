...
public class PrepRequestProcessor extends ZooKeeperCriticalThread implements RequestProcessor {
     ...
     * @param record
     */
    protected void pRequest2Txn(int type, long zxid, Request var2, Record record, boolean deserialize) throws KeeperException, IOException, RequestProcessorException {
        if (var2.getHdr() == null) {
            var2.setHdr(new TxnHeader(var2.sessionId, var2.cxid, zxid, Time.currentWallTime(), type));
        }
        ...
        switch(type) {
            ...
            case OpCode.createContainer:
                {
                    pRequest2TxnCreate(type, var2, record, deserialize);
                    break;
                }
            case OpCode.deleteContainer:
                {
                    String path = new String(var2.var2.array(), UTF_8);
                    ...
                    var2.setTxn(new DeleteTxn(path));
                    parentRecord = parentRecord.duplicate(var2.getHdr().getZxid());
                    parentRecord.childCount--;
                    parentRecord.stat.setPzxid(var2.getHdr().getZxid());
                    ...
                    nodeRecord = new ChangeRecord(var2.getHdr().getZxid(), path, null, -1, null);
                    nodeRecord.precalculatedDigest = precalculateDigest(DigestOpCode.REMOVE, path);
                    setTxnDigest(var2, nodeRecord.precalculatedDigest);
            ...
            case OpCode.delete:
                ...
                if (deserialize) {
                    ByteBufferInputStream.byteBuffer2Record(var2.var2, deleteRequest);
                }
                String path = deleteRequest.getPath();
                String parentPath = getParentPathAndValidate(path);
                ChangeRecord parentRecord = getRecordForPath(parentPath);
                zks.checkACL(var2.cnxn, parentRecord.acl, ZooDefs.Perms.DELETE, var2.authInfo, path, null);
                ChangeRecord nodeRecord = getRecordForPath(path);
            ...
            case OpCode.setData:
                ...
                if (deserialize) {
                    ByteBufferInputStream.byteBuffer2Record(var2.var2, setDataRequest);
                }
                path = setDataRequest.getPath();
                validatePath(path, var2.sessionId);
                nodeRecord = getRecordForPath(path);
                zks.checkACL(var2.cnxn, nodeRecord.acl, ZooDefs.Perms.WRITE, var2.authInfo, path, null);
                zks.checkQuota(path, setDataRequest.getData(), OpCode.setData);
                int newVersion = checkAndIncVersion(nodeRecord.stat.getVersion(), setDataRequest.getVersion(), path);
                var2.setTxn(new SetDataTxn(path, setDataRequest.getData(), newVersion));
                nodeRecord = nodeRecord.duplicate(var2.getHdr().getZxid());
                nodeRecord.stat.setVersion(newVersion);
                nodeRecord.stat.setMtime(var2.getHdr().getTime());
            ...
            case OpCode.reconfig:
                ...
                if (newMembers != null) {
                    ...
                    try {
                        ...
                        var2.qv = QuorumPeerConfig.parseDynamicConfig(props, lzks.self.getElectionType(), true, false);
                        var2.qv.setVersion(var2.getHdr().getZxid());
                    ...
                    var2.qv = new QuorumMaj(nextServers);
                    var2.qv.setVersion(var2.getHdr().getZxid());
                }
                if (QuorumPeerConfig.isStandaloneEnabled() && var2.qv.getVotingMembers().size() < 2) {
                    String msg = "Reconfig failed - new configuration must include at least 2 followers";
                    LOG.warn(msg);
                    throw new KeeperException.BadArgumentsException(msg);
                } else if (var2.qv.getVotingMembers().size() < 1) {
                    String msg = "Reconfig failed - new configuration must include at least 1 follower";
                    LOG.warn(msg);
                    throw new KeeperException.BadArgumentsException(msg);
                }
                if (!lzks.getLeader().isQuorumSynced(var2.qv)) {
                ...
                zks.checkACL(var2.cnxn, nodeRecord.acl, ZooDefs.Perms.WRITE, var2.authInfo, null, null);
                SetDataTxn setDataTxn = new SetDataTxn(ZooDefs.CONFIG_NODE, var2.qv.toString().getBytes(), -1);
                var2.setTxn(setDataTxn);
                nodeRecord = nodeRecord.duplicate(var2.getHdr().getZxid());
            ...
            case OpCode.setACL:
                ...
                if (deserialize) {
                    ByteBufferInputStream.byteBuffer2Record(var2.var2, setAclRequest);
                }
                path = setAclRequest.getPath();
                validatePath(path, var2.sessionId);
                List<ACL> listACL = fixupACL(path, var2.authInfo, setAclRequest.getAcl());
                nodeRecord = getRecordForPath(path);
                zks.checkACL(var2.cnxn, nodeRecord.acl, ZooDefs.Perms.ADMIN, var2.authInfo, path, listACL);
                newVersion = checkAndIncVersion(nodeRecord.stat.getAversion(), setAclRequest.getVersion(), path);
                var2.setTxn(new SetACLTxn(path, listACL, newVersion));
                nodeRecord = nodeRecord.duplicate(var2.getHdr().getZxid());
            ...
            case OpCode.createSession:
                ...
                var2.setTxn(new CreateSessionTxn(to));
                var2.var2.rewind();
                // only add the global session tracker but not to ZKDb
                zks.sessionTracker.trackSession(var2.sessionId, to);
            ...
            case OpCode.closeSession:
                ...
                synchronized (zks.outstandingChanges) {
                    ...
                    Set<String> es = zks.getZKDatabase().getEphemerals(var2.sessionId);
                    ...
                    for (String path2Delete : es) {
                        ...
                        nodeRecord = new ChangeRecord(var2.getHdr().getZxid(), path2Delete, null, 0, null);
                        nodeRecord.precalculatedDigest = precalculateDigest(DigestOpCode.REMOVE, path2Delete);
                        addChangeRecord(nodeRecord);
                    }
                    if (ZooKeeperServer.isCloseSessionTxnEnabled()) {
                        var2.setTxn(new CloseSessionTxn(new ArrayList<String>(es)));
                    }
                    zks.sessionTracker.setSessionClosing(var2.sessionId);
                }
            ...
            case OpCode.check:
                ...
                if (deserialize) {
                    ByteBufferInputStream.byteBuffer2Record(var2.var2, checkVersionRequest);
                }
                path = checkVersionRequest.getPath();
                validatePath(path, var2.sessionId);
                nodeRecord = getRecordForPath(path);
                zks.checkACL(var2.cnxn, nodeRecord.acl, ZooDefs.Perms.READ, var2.authInfo, path, null);
                var2.setTxn(new CheckVersionTxn(path, checkAndIncVersion(nodeRecord.stat.getVersion(), checkVersionRequest.getVersion(), path)));
                break;
            default:
        ...
        if (var2.getTxnDigest() == null && digestEnabled) {
            setTxnDigest(var2);
        }
    }
    ...
    private PrecalculatedDigest precalculateDigest(DigestOpCode type, String path) throws KeeperException.NoNodeException {
