    ...
    public void run() {
        try {
            ...
            new Thread() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        LOG.warn("Unexpected interruption", var11);
                    }
                }
            }.start();
            while (true) {
                ...
                switch(qp.getType()) {
                    ...
                    case Leader.REVALIDATE:
                        ...
                        if (valid) {
                            ...
                            } catch (SessionExpiredException var11) {
                                LOG.error("Somehow session " + Long.toHexString(id) + " expired right after being renewed! (impossible)", var11);
                            }
                        }
                        if (LOG.isTraceEnabled()) {
            ...
            if (sock != null && !sock.isClosed()) {
                LOG.error("Unexpected exception causing shutdown while sock " + "still open", var11);
            }
        } catch (InterruptedException var11) {
            LOG.error("Unexpected exception causing shutdown", var11);
        } finally {
            ...
            try {
                ...
                LOG.warn("Ignoring unexpected exception", var11);
            }
            shutdown();
        }
    }
