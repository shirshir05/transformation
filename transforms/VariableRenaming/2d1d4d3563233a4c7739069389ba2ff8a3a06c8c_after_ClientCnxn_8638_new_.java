...
public class ClientCnxn {
    ...
    static class Packet {
        ...
        Packet(RequestHeader header, ReplyHeader replyHeader, Record record, Record response, ByteBuffer bb) {
            this.header = header;
    ...
    public ClientCnxn(String hosts, int sessionTimeout, ZooKeeper zooKeeper) throws KeeperException, IOException {
     ...
     * @throws IOException
     */
    public ClientCnxn(String hosts, int sessionTimeout, ZooKeeper zooKeeper, long sessionId, byte[] sessionPasswd) throws KeeperException, IOException {
            ...
            try {
                while (true) {
                    ...
                    if (event instanceof WatcherEvent) {
                        ...
                        Packet var1 = (Packet) event;
                        int rc = 0;
                        String path = var1.path;
                        if (var1.replyHeader.getErr() != 0) {
                            rc = var1.replyHeader.getErr();
                        }
                        if (var1.cb == null) {
                            ZooLog.logError("Somehow a null cb got to EventThread!");
                        } else if (var1.response instanceof ExistsResponse || var1.response instanceof SetDataResponse || var1.response instanceof SetACLResponse) {
                            StatCallback cb = (StatCallback) var1.cb;
                            if (rc == 0) {
                                if (var1.response instanceof ExistsResponse) {
                                    cb.processResult(rc, path, var1.ctx, ((ExistsResponse) var1.response).getStat());
                                } else if (var1.response instanceof SetDataResponse) {
                                    cb.processResult(rc, path, var1.ctx, ((SetDataResponse) var1.response).getStat());
                                } else if (var1.response instanceof SetACLResponse) {
                                    cb.processResult(rc, path, var1.ctx, ((SetACLResponse) var1.response).getStat());
                                }
                            } else {
                                cb.processResult(rc, path, var1.ctx, null);
                            }
                        } else if (var1.response instanceof GetDataResponse) {
                            DataCallback cb = (DataCallback) var1.cb;
                            GetDataResponse rsp = (GetDataResponse) var1.response;
                            if (rc == 0) {
                                cb.processResult(rc, path, var1.ctx, rsp.getData(), rsp.getStat());
                            } else {
                                cb.processResult(rc, path, var1.ctx, null, null);
                            }
                        } else if (var1.response instanceof GetACLResponse) {
                            ACLCallback cb = (ACLCallback) var1.cb;
                            GetACLResponse rsp = (GetACLResponse) var1.response;
                            if (rc == 0) {
                                cb.processResult(rc, path, var1.ctx, rsp.getAcl(), rsp.getStat());
                            } else {
                                cb.processResult(rc, path, var1.ctx, null, null);
                            }
                        } else if (var1.response instanceof GetChildrenResponse) {
                            ChildrenCallback cb = (ChildrenCallback) var1.cb;
                            GetChildrenResponse rsp = (GetChildrenResponse) var1.response;
                            if (rc == 0) {
                                cb.processResult(rc, path, var1.ctx, rsp.getChildren());
                            } else {
                                cb.processResult(rc, path, var1.ctx, null);
                            }
                        } else if (var1.response instanceof CreateResponse) {
                            StringCallback cb = (StringCallback) var1.cb;
                            CreateResponse rsp = (CreateResponse) var1.response;
                            if (rc == 0) {
                                cb.processResult(rc, path, var1.ctx, rsp.getPath());
                            } else {
                                cb.processResult(rc, path, var1.ctx, null);
                            }
                        } else if (var1.cb instanceof VoidCallback) {
                            VoidCallback cb = (VoidCallback) var1.cb;
                            cb.processResult(rc, path, var1.ctx);
                        }
                    }
                }
    ...
    class SendThread extends Thread {
        ...
        void readConnectResult() throws IOException {
            ...
            if (sessionTimeout <= 0) {
                ...
                waitingEvents.add(new WatcherEvent(Watcher.Event.EventNone, Watcher.Event.KeeperStateExpired, null));
            ...
            waitingEvents.add(new WatcherEvent(Watcher.Event.EventNone, Watcher.Event.KeeperStateSyncConnected, null));
        }
        ...
        void readResponse() throws IOException {
            ...
            if (pendingQueue.size() == 0) {
                throw new IOException("Nothing in the queue, but got " + r.getXid());
            }
            Packet p = null;
            ...
            if (p.header.getXid() != r.getXid()) {
                throw new IOException("Xid out of order. Got " + r.getXid() + " expected " + p.header.getXid());
            }
        ...
        private void primeConnection(SelectionKey k) throws IOException {
            ZooLog.logWarn("Priming connection to " + ((SocketChannel) sockKey.channel()));
    ...
    public void close() throws IOException {
        ZooLog.logTextTraceMessage("Close ClientCnxn for session: " + sessionId + "!", ZooLog.SESSION_TRACE_MASK);
        sendThread.close();
    ...
    public ReplyHeader submitRequest(RequestHeader h, Record request, Record response) throws InterruptedException {
        ReplyHeader r = new ReplyHeader();
    ...
    Packet queuePacket(RequestHeader h, ReplyHeader r, Record request, Record response, AsyncCallback cb, String path, Object ctx) {
        Packet packet = null;
        synchronized (outgoingQueue) {
