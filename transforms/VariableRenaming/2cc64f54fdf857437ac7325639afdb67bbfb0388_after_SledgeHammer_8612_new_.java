...
public class SledgeHammer extends Thread implements Watcher {
    ...
    public SledgeHammer(String hosts, int count, int readsPerWrite) throws KeeperException, IOException {
    ...
    public void run() {
        try {
            Stat stat = new Stat();
            String path = zk.create("/hammers/hammer-", new byte[0], Ids.OPEN_ACL_UNSAFE, CreateFlags.SEQUENCE);
            ...
            for (int i = 0; i < count; i++) {
                try {
                    ...
                    ArrayList<String> childs = zk.getChildren("/hammers", false);
                    Collections.shuffle(childs);
                    for (String var6 : childs) {
                        if (var6.startsWith("hammer-")) {
                            var6 = "/hammers/" + var6;
                            zk.setData(var6, tag, -1);
                            for (int j = 0; j < readsPerWrite; j++) {
                                zk.getData(var6, false, stat);
                            }
                            break;
                        }
                    }
     ...
     */
    public static void main(String[] args) throws NumberFormatException, KeeperException, IOException {
        if (args.length != 3) {
