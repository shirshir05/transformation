...
public class QuorumCnxManager {
    ...
    synchronized void connectOne(long var0) {
        if (senderWorkerMap.get(var0) == null) {
            ...
            if (self.quorumPeers.containsKey(var0)) {
                electionAddr = self.quorumPeers.get(var0).electionAddr;
            } else {
            ...
            try {
                ...
                Socket sock = new Socket();
                setSockOpts(sock);
                sock.connect(self.getView().get(var0).electionAddr, cnxTO);
                if (LOG.isDebugEnabled()) {
                ...
                // detail.
                LOG.warn("Cannot open channel to " + var0 + " at election address " + electionAddr, e);
                throw e;
            } catch (IOException e) {
                LOG.warn("Cannot open channel to " + var0 + " at election address " + electionAddr, e);
            }
    ...
    public class Listener extends Thread {
        ...
        public void run() {
            ...
            if (!shutdown) {
                LOG.fatal("As I'm leaving the listener thread, " + "I won't be able to participate in leader " + "election any longer: " + self.quorumPeers.get(self.getId()).electionAddr);
            }
        }
