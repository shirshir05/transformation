    ...
    class SendThread extends Thread {
        ...
        void readResponse() throws IOException {
            ...
            synchronized (pendingQueue) {
                var4 = pendingQueue.remove();
            }
            /*
             * Since requests are processed in order, we better get a response
             * to the first request!
             */
            if (var4.header.getXid() != r.getXid()) {
                throw new IOException("Xid out of order. Got " + r.getXid() + " expected " + var4.header.getXid());
            }
            var4.replyHeader.setXid(r.getXid());
            var4.replyHeader.setErr(r.getErr());
            var4.replyHeader.setZxid(r.getZxid());
            lastZxid = r.getZxid();
            if (var4.response != null && r.getErr() == 0) {
                var4.response.deserialize(bbia, "response");
            }
            var4.finished = true;
            finishPacket(var4);
        }
