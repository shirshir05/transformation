...
public class NIOServerCnxn implements Watcher, ServerCnxn {
     ...
     */
    synchronized public void sendResponse(ReplyHeader h, Record var1, String tag) {
        try {
            if (closed) {
                if (LOG.isTraceEnabled()) {
                    LOG.trace("send called on closed session 0x" + Long.toHexString(sessionId) + " with record " + var1);
                }
                return;
            }
