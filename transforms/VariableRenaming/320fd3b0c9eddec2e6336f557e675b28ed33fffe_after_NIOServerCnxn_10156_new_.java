...
public class NIOServerCnxn implements Watcher, ServerCnxn {
    ...
    public void close() {
        ...
        LOG.info("closing session:0x" + Long.toHexString(sessionId) + " NIOServerCnxn: " + sock);
        try {
            /*
        ...
        try {
            ...
            LOG.warn("ignoring exception during socket close", var4);
        }
        try {
            sock.close();
        } catch (IOException var4) {
            LOG.warn("ignoring exception during socketchannel close", var4);
        }
        sock = null;
        if (sk != null) {
            try {
                ...
                LOG.warn("ignoring exception during selectionkey cancel", var4);
            }
        }
    }
    ...
    public void finishSessionInit(boolean valid) {
        ...
        try {
            ...
            sendBuffer(bb);
            LOG.info("Finished init of 0x" + Long.toHexString(sessionId) + " valid:" + valid);
            if (!valid) {
    ...
    private class CnxnStats implements ServerCnxn.Stats {
        ...
        public String toString() {
            ...
            if (channel instanceof SocketChannel) {
                sb.append(" ").append(((SocketChannel) channel).socket().getRemoteSocketAddress()).append("[").append(Integer.toHexString(sk.interestOps())).append("](queued=").append(getOutstandingRequests()).append(",recved=").append(getPacketsReceived()).append(",sent=").append(getPacketsSent()).append(")\n");
            }
            return sb.toString();
        }
    }
