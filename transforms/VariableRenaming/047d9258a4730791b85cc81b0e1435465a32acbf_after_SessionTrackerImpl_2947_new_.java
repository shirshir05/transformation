...
public class SessionTrackerImpl extends Thread implements SessionTracker {
    ...
    synchronized public void run() {
            ...
            while (running) {
                ...
                if (set != null) {
                    for (Session s : set.sessions) {
                        ...
                        ZooLog.logWarn("Expiring " + Long.toHexString(s.sessionId));
        ...
        ZooLog.logTextTraceMessage("SessionTrackerImpl exited loop!", ZooLog.textTraceMask);
    }

    synchronized public boolean touchSession(long sessionId, int timeout) {
        ZooLog.logTextTraceMessage("SessionTrackerImpl --- Touch session: " + sessionId + " with timeout " + timeout, ZooLog.CLIENT_PING_TRACE_MASK);
    ...
    synchronized public void removeSession(long sessionId) {
        ...
        ZooLog.logTextTraceMessage("SessionTrackerImpl --- Removing " + sessionId, ZooLog.SESSION_TRACE_MASK);
        if (s != null) {
    ...
    synchronized public void addSession(long id, int sessionTimeout) {
        ...
        if (sessionsById.get(id) == null) {
            Session var2 = new Session(id, 0);
            sessionsById.put(id, var2);
            ZooLog.logTextTraceMessage("SessionTrackerImpl --- Adding " + id + " " + sessionTimeout, ZooLog.SESSION_TRACE_MASK);
        } else {
            ZooLog.logTextTraceMessage("SessionTrackerImpl --- Existing session " + id + " " + sessionTimeout, ZooLog.SESSION_TRACE_MASK);
        }
