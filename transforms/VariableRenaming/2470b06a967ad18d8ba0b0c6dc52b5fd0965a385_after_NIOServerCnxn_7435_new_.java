...
public class NIOServerCnxn implements Watcher, ServerCnxn {
    ...
    void doIO(SelectionKey k) throws InterruptedException {
        try {
            ...
            LOG.warn("Exception causing close of session 0x" + Long.toHexString(sessionId) + " due to " + var10);
            if (LOG.isDebugEnabled()) {
                LOG.debug("CancelledKeyException stack trace", var10);
            }
            close();
        } catch (CloseRequestException var10) {
            // expecting close to log session closure
            close();
        } catch (EndOfStreamException var10) {
            // tell user why
            LOG.warn(var10);
            // expecting close to log session closure
            close();
            ...
            LOG.warn("Exception causing close of session 0x" + Long.toHexString(sessionId) + " due to " + var10);
            if (LOG.isDebugEnabled()) {
                LOG.debug("IOException stack trace", var10);
            }
            close();
        }
    }
