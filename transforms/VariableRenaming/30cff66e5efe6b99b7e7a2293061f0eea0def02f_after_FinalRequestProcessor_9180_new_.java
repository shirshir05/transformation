...
public class FinalRequestProcessor implements RequestProcessor {
    ...
    public void processRequest(Request request) {
        ...
        if (request.hdr != null && request.hdr.getType() == OpCode.closeSession) {
            ...
            if (scxn != null && request.var6 == null) {
                // in the switch block below
                scxn.closeSession(request.sessionId);
                return;
            }
        }
        if (request.var6 == null) {
            return;
        }
        ServerCnxn var6 = request.var6;
        String lastOp = "NA";
        zks.decInProcess();
        Code err = Code.OK;
        Record rsp = null;
                ...
                case OpCode.ping:
                    {
                        ...
                        lastOp = "PING";
                        var6.updateStatsForResponse(request.cxid, request.zxid, lastOp, request.createTime, System.currentTimeMillis());
                        var6.sendResponse(new ReplyHeader(-2, zks.getZKDatabase().getDataTreeLastProcessedZxid(), 0), null, "response");
                        return;
                    }
                case OpCode.createSession:
                    {
                        ...
                        lastOp = "SESS";
                        var6.updateStatsForResponse(request.cxid, request.zxid, lastOp, request.createTime, System.currentTimeMillis());
                        zks.finishSessionInit(request.var6, true);
                        return;
                    }
                case OpCode.create:
                    {
                ...
                case OpCode.exists:
                    {
                        ...
                        Stat stat = zks.getZKDatabase().statNode(path, existsRequest.getWatch() ? var6 : null);
                ...
                case OpCode.getData:
                    {
                        ...
                        Stat stat = new Stat();
                        byte[] b = zks.getZKDatabase().getData(getDataRequest.getPath(), stat, getDataRequest.getWatch() ? var6 : null);
                ...
                case OpCode.setWatches:
                    {
                        ...
                        zks.getZKDatabase().setWatches(relativeZxid, setWatches.getDataWatches(), setWatches.getExistWatches(), setWatches.getChildWatches(), var6);
                        break;
                    }
                case OpCode.getACL:
                    {
                ...
                case OpCode.getChildren:
                    {
                        ...
                        List<String> children = zks.getZKDatabase().getChildren(getChildrenRequest.getPath(), null, getChildrenRequest.getWatch() ? var6 : null);
                ...
                case OpCode.getChildren2:
                    {
                        ...
                        PrepRequestProcessor.checkACL(zks, zks.getZKDatabase().convertLong(aclG), ZooDefs.Perms.READ, request.authInfo);
                        List<String> children = zks.getZKDatabase().getChildren(getChildren2Request.getPath(), stat, getChildren2Request.getWatch() ? var6 : null);
        ...
        var6.updateStatsForResponse(request.cxid, request.zxid, lastOp, request.createTime, System.currentTimeMillis());
        try {
            var6.sendResponse(hdr, rsp, "response");
            if (closeSession) {
                var6.sendCloseSession();
            }
        } catch (IOException e) {
