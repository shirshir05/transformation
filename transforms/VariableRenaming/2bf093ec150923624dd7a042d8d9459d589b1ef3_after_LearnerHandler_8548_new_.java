    ...
    public void run() {
        try {
            ...
            new Thread() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        LOG.warn("Unexpected interruption", var32);
                    }
                }
            }.start();
            /*
                ...
                switch(qp.getType()) {
                    ...
                    case Leader.REVALIDATE:
                        ...
                        if (valid) {
                            ...
                            } catch (SessionExpiredException var32) {
                                LOG.error("Somehow session " + Long.toHexString(id) + " expired right after being renewed! (impossible)", var32);
                            }
                        }
                        if (LOG.isTraceEnabled()) {
            ...
            if (sock != null && !sock.isClosed()) {
                LOG.error("Unexpected exception causing shutdown while sock " + "still open", var32);
            ...
            LOG.error("Unexpected exception causing shutdown", var32);
        } finally {
            ...
            try {
                ...
                LOG.warn("Ignoring unexpected exception", var32);
            }
            shutdown();
        }
    }
