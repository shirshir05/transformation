...
public class FileTxnSnapLog {
    ...
    public void save(DataTree dataTree, ConcurrentHashMap<Long, Integer> sessionsWithTimeouts, boolean syncSnap) throws IOException {
        ...
        File var4 = new File(snapDir, Util.makeSnapshotName(lastZxid));
        LOG.info("Snapshotting: 0x{} to {}", Long.toHexString(lastZxid), var4);
        try {
            snapLog.serialize(dataTree, sessionsWithTimeouts, var4, syncSnap);
        } catch (IOException e) {
            if (var4.length() == 0) {
                ...
                if (var4.delete()) {
                    LOG.info("Deleted empty snapshot file: {}", var4.getAbsolutePath());
                } else {
                    LOG.warn("Could not delete empty snapshot file: {}", var4.getAbsolutePath());
                }
            } else {
            }
            throw e;
