        ...
        try {
            ...
            switch(request.type) {
                ...
                case OpCode.exists:
                    ...
                    ExistsRequest var6 = new ExistsRequest();
                    ZooKeeperServer.byteBuffer2Record(request.request, var6);
                    String path = var6.getPath();
                    if (path.indexOf('\0') != -1) {
                        throw new KeeperException(Code.BadArguments);
                    }
                    Stat stat = zks.dataTree.statNode(path, var6.getWatch() ? request.cnxn : null);
