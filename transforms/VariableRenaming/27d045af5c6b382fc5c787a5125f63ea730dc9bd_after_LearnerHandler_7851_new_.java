    ...
    public void run() {
        try {
                            ...
                            if (propose.packet.getZxid() <= peerLastZxid) {
                                ...
                                if (firstPacket) {
                                    ...
                                    if (prevProposalZxid < peerLastZxid) {
                                        ...
                                        var15 = prevProposalZxid;
                                        updates = var15;
                                    } else {
                                        // Just send the diff
                                        packetToSend = Leader.DIFF;
                                        LOG.info("Sending diff");
                                        var15 = maxCommittedLog;
                                    }
                                }
            ...
            if (packetToSend == Leader.SNAP) {
                var15 = leader.zk.getZKDatabase().getDataTreeLastProcessedZxid();
            }
            oa.writeRecord(new QuorumPacket(packetToSend, var15, null, null), "packet");
            bufferedOutput.flush();
            /* if we are not truncating or sending a diff just send a snapshot */
            if (packetToSend == Leader.SNAP) {
                LOG.info("Sending snapshot last zxid of peer is 0x" + Long.toHexString(peerLastZxid) + " " + " zxid of leader is 0x" + Long.toHexString(leaderLastZxid) + "sent zxid of db as 0x" + Long.toHexString(var15));
                // Dump data to peer
