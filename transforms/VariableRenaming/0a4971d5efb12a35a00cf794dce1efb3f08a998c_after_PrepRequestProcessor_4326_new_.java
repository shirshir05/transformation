    ...
    protected void pRequest2Txn(int type, long zxid, Request request, Record record, boolean deserialize) throws KeeperException, IOException, RequestProcessorException {
        ...
        switch(type) {
            ...
            case OpCode.deleteContainer:
                {
                    String var5 = new String(request.request.array());
                    String parentPath = getParentPathAndValidate(var5);
                    ...
                    request.setTxn(new DeleteTxn(var5));
                    parentRecord = parentRecord.duplicate(request.getHdr().getZxid());
                    parentRecord.childCount--;
                    addChangeRecord(parentRecord);
                    addChangeRecord(new ChangeRecord(request.getHdr().getZxid(), var5, null, -1, null));
                    break;
                }
            case OpCode.delete:
                ...
                String var5 = deleteRequest.getPath();
                String parentPath = getParentPathAndValidate(var5);
                ChangeRecord parentRecord = getRecordForPath(parentPath);
                ChangeRecord nodeRecord = getRecordForPath(var5);
                checkACL(zks, parentRecord.acl, ZooDefs.Perms.DELETE, request.authInfo);
                checkAndIncVersion(nodeRecord.stat.getVersion(), deleteRequest.getVersion(), var5);
                if (nodeRecord.childCount > 0) {
                    throw new KeeperException.NotEmptyException(var5);
                }
                request.setTxn(new DeleteTxn(var5));
            ...
            case OpCode.setData:
                ...
                validatePath(var5, request.sessionId);
                nodeRecord = getRecordForPath(var5);
                checkACL(zks, nodeRecord.acl, ZooDefs.Perms.WRITE, request.authInfo);
                int newVersion = checkAndIncVersion(nodeRecord.stat.getVersion(), setDataRequest.getVersion(), var5);
                request.setTxn(new SetDataTxn(var5, setDataRequest.getData(), newVersion));
            ...
            case OpCode.setACL:
                ...
                validatePath(var5, request.sessionId);
                List<ACL> listACL = fixupACL(var5, request.authInfo, setAclRequest.getAcl());
                nodeRecord = getRecordForPath(var5);
                checkACL(zks, nodeRecord.acl, ZooDefs.Perms.ADMIN, request.authInfo);
                newVersion = checkAndIncVersion(nodeRecord.stat.getAversion(), setAclRequest.getVersion(), var5);
                request.setTxn(new SetACLTxn(var5, listACL, newVersion));
                nodeRecord = nodeRecord.duplicate(request.getHdr().getZxid());
            ...
            case OpCode.check:
                ...
                request.setTxn(new CheckVersionTxn(var5, checkAndIncVersion(nodeRecord.stat.getVersion(), checkVersionRequest.getVersion(), var5)));
                break;
            default:
