...
public class PrepRequestProcessor extends Thread implements RequestProcessor {
    ...
    protected void pRequest(Request request) {
        ...
        try {
                ...
                case OpCode.multi:
                     ...
                     */
                        ...
                        } else /* Prep the request and convert to a Txn */
                        {
                            ...
                            } catch (KeeperException var14) {
                                if (ke == null) {
                                    ke = var14;
                                }
                                request.hdr.setType(OpCode.error);
                                request.txn = new ErrorTxn(var14.code().intValue());
                                LOG.error(">>>> Got user-level KeeperException when processing " + request.toString() + " Error Path:" + var14.getPath() + " Error:" + var14.getMessage());
                                LOG.error(">>>> ABORTING remaing MultiOp ops");
                                request.setException(var14);
            ...
            LOG.info("Got user-level KeeperException when processing " + request.toString() + " Error Path:" + var14.getPath() + " Error:" + var14.getMessage());
            request.setException(var14);
        } catch (Exception var14) {
            // error to the user
            LOG.error("Failed to process " + request, var14);
