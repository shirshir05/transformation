...
public class QuorumCnxManager {
    ...
    boolean initiateConnection(SocketChannel s, Long var1) {
        ...
        if (var1 > self.getId()) {
            try {
                LOG.warn("Have smaller server identifier, so dropping the connection: (" + var1 + ", " + self.getId());
                s.socket().close();
            } catch (IOException e) {
            ...
            if (s != null) {
                SendWorker sw = new SendWorker(s, var1);
                RecvWorker rw = new RecvWorker(s, var1);
                sw.setRecv(rw);
                if (senderWorkerMap.containsKey(var1)) {
                    senderWorkerMap.get(var1).finish();
                }
                if (!queueSendMap.containsKey(var1)) {
                    queueSendMap.put(var1, new ArrayBlockingQueue<ByteBuffer>(CAPACITY));
                }
                senderWorkerMap.put(var1, sw);
                sw.start();
                rw.start();
                return true;
            } else {
