...
public class Learner {
    ...
    protected void syncWithLeader(long newLeaderZxid) throws IOException, InterruptedException {
            ...
            outerLoop: while (self.isRunning()) {
                ...
                switch(qp.getType()) {
                    case Leader.PROPOSAL:
                        ...
                        pif.var9 = new TxnHeader();
                        BinaryInputArchive ia = BinaryInputArchive.getArchive(new ByteArrayInputStream(qp.getData()));
                        pif.rec = SerializeUtils.deserializeTxn(ia, pif.var9);
                        if (pif.var9.getZxid() != lastQueued + 1) {
                            LOG.warn("Got zxid 0x" + Long.toHexString(pif.var9.getZxid()) + " expected 0x" + Long.toHexString(lastQueued + 1));
                        }
                        lastQueued = pif.var9.getZxid();
                        packetsNotCommitted.add(pif);
                        break;
                    case Leader.COMMIT:
                        pif = packetsNotCommitted.peekFirst();
                        if (pif.var9.getZxid() != qp.getZxid()) {
                            LOG.warn("Committing " + qp.getZxid() + ", but next proposal is " + pif.var9.getZxid());
                        } else {
                            zk.getZKDatabase().processTxn(pif.var9, pif.rec);
                            packetsNotCommitted.remove();
                        }
                        break;
                    case Leader.INFORM:
                        TxnHeader var9 = new TxnHeader();
                        ia = BinaryInputArchive.getArchive(new ByteArrayInputStream(qp.getData()));
                        Record txn = SerializeUtils.deserializeTxn(ia, var9);
                        zk.getZKDatabase().processTxn(var9, txn);
                        break;
        ...
        for (PacketInFlight p : packetsNotCommitted) {
            ((FollowerZooKeeperServer) zk).logRequest(p.var9, p.rec);
        }
    }
