...
public class DataTree {
    ...
    public void setWatches(long relativeZxid, List<String> dataWatches, List<String> existWatches, List<String> childWatches, Watcher watcher) {
        for (String var8 : dataWatches) {
            DataNode node = getNode(var8);
            WatchedEvent e = null;
            if (node == null) {
                e = new WatchedEvent(EventType.NodeDeleted, KeeperState.SyncConnected, var8);
            } else if (node.stat.getCzxid() > relativeZxid) {
                e = new WatchedEvent(EventType.NodeCreated, KeeperState.SyncConnected, var8);
            } else if (node.stat.getMzxid() > relativeZxid) {
                e = new WatchedEvent(EventType.NodeDataChanged, KeeperState.SyncConnected, var8);
            }
            if (e != null) {
                watcher.process(e);
            } else {
                this.dataWatches.addWatch(var8, watcher);
            }
        }
        for (String var8 : existWatches) {
            DataNode node = getNode(var8);
            WatchedEvent e = null;
            if (node == null) {
            } else if (node.stat.getMzxid() > relativeZxid) {
        ...
        for (String var8 : childWatches) {
            DataNode node = getNode(var8);
            WatchedEvent e = null;
            if (node == null) {
                e = new WatchedEvent(EventType.NodeDeleted, KeeperState.SyncConnected, var8);
            } else if (node.stat.getPzxid() > relativeZxid) {
                e = new WatchedEvent(EventType.NodeChildrenChanged, KeeperState.SyncConnected, var8);
            }
            if (e != null) {
                watcher.process(e);
            } else {
                this.childWatches.addWatch(var8, watcher);
            }
        }
    }
}
