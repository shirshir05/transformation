    ...
    protected void pRequest(Request request) throws RequestProcessorException {
        ...
        try {
                     ...
                     */
                        ...
                        } else /* Prep the request and convert to a Txn */
                        {
                            try {
                                ...
                                ke = var9;
                                type = OpCode.error;
                                txn = new ErrorTxn(var9.code().intValue());
                                LOG.info("Got user-level KeeperException when processing " + request.toString() + " aborting remaining multi ops." + " Error Path:" + var9.getPath() + " Error:" + var9.getMessage());
                                request.setException(var9);
            ...
            if (request.getHdr() != null) {
                ...
                request.setTxn(new ErrorTxn(var9.code().intValue()));
            }
            LOG.info("Got user-level KeeperException when processing " + request.toString() + " Error Path:" + var9.getPath() + " Error:" + var9.getMessage());
            request.setException(var9);
        } catch (Exception var9) {
            // error to the user
            LOG.error("Failed to process " + request, var9);
