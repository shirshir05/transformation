...
public class DataTree {
    ...
    public void setWatches(long relativeZxid, List<String> dataWatches, List<String> existWatches, List<String> childWatches, Watcher watcher) {
        for (String path : dataWatches) {
            ...
            WatchedEvent var10 = null;
            if (node == null) {
                var10 = new WatchedEvent(EventType.NodeDeleted, KeeperState.SyncConnected, path);
            } else if (node.stat.getCzxid() > relativeZxid) {
                var10 = new WatchedEvent(EventType.NodeCreated, KeeperState.SyncConnected, path);
            } else if (node.stat.getMzxid() > relativeZxid) {
                var10 = new WatchedEvent(EventType.NodeDataChanged, KeeperState.SyncConnected, path);
            }
            if (var10 != null) {
                watcher.process(var10);
            } else {
        ...
        for (String path : childWatches) {
            ...
            if (node == null) {
                ...
                var10 = new WatchedEvent(EventType.NodeChildrenChanged, KeeperState.SyncConnected, path);
            }
            if (var10 != null) {
                watcher.process(var10);
            } else {
