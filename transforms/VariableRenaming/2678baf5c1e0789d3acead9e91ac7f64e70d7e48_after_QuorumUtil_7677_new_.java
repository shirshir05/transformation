...
public class QuorumUtil {
    ...
    public QuorumUtil(int n) throws RuntimeException {
        try {
            ...
            for (int i = 1; i <= ALL; ++i) {
                ...
                peers.put(i, ps);
                peersView.put(Long.valueOf(i), new QuorumServer(i, new InetSocketAddress("127.0.0.1", ps.clientPort + 1000), new InetSocketAddress("127.0.0.1", PortAssignment.unique() + 1000), LearnerType.PARTICIPANT));
    ...
    public void startQuorum() throws IOException {
        ...
        for (int var1 = 1; var1 <= N + 1; ++var1) {
            Assert.assertTrue("Waiting for server up", ClientBase.waitForServerUp("127.0.0.1:" + getPeer(var1).clientPort, ClientBase.CONNECTION_TIMEOUT));
        }
    }
