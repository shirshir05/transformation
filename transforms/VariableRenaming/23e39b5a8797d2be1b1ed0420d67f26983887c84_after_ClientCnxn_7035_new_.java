...
class ClientCnxn {
    ...
    public ClientCnxn(String hosts, int sessionTimeout, ZooKeeper zooKeeper) throws IOException {
     ...
     * @throws IOException
     */
    public ClientCnxn(String hosts, int sessionTimeout, ZooKeeper zooKeeper, long sessionId, byte[] sessionPasswd) throws IOException {
    ...
    class SendThread extends Thread {
        ...
        private void primeConnection(SelectionKey k) throws IOException {
            LOG.info("Priming connection to " + ((SocketChannel) sockKey.channel()));
    ...
    public void close() throws IOException {
        ...
        if (ZooTrace.isTraceEnabled(LOG, traceMask)) {
            ZooTrace.logTraceMessage(LOG, traceMask, "Close ClientCnxn for session: " + sessionId + "!");
        }
        sendThread.close();
    ...
    Packet queuePacket(RequestHeader h, ReplyHeader r, Record request, Record response, AsyncCallback cb, String path, Object ctx, WatchRegistration watchRegistration) {
        Packet packet = null;
        synchronized (outgoingQueue) {
    ...
    public void addAuthInfo(String scheme, byte[] auth) {
        ...
        if (zooKeeper.state == States.CONNECTED) {
            queuePacket(new RequestHeader(-4, OpCode.auth), null, new AuthPacket(0, scheme, auth), null, null, null, null, null);
        }
    }
}
