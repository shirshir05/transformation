    ...
    protected void pRequest(Request request) throws RequestProcessorException {
        ...
        try {
                     ...
                     */
                        ...
                        } else /* Prep the request and convert to a Txn */
                        {
                            try {
                                ...
                                txn = request.getTxn();
                            } catch (KeeperException var18) {
                                ke = var18;
                                type = OpCode.error;
                                txn = new ErrorTxn(var18.code().intValue());
                                LOG.info("Got user-level KeeperException when processing " + request.toString() + " aborting remaining multi ops." + " Error Path:" + var18.getPath() + " Error:" + var18.getMessage());
                                request.setException(var18);
            ...
            if (request.getHdr() != null) {
                ...
                request.setTxn(new ErrorTxn(var18.code().intValue()));
            }
            LOG.info("Got user-level KeeperException when processing " + request.toString() + " Error Path:" + var18.getPath() + " Error:" + var18.getMessage());
            request.setException(var18);
        } catch (Exception var18) {
            // error to the user
            LOG.error("Failed to process " + request, var18);
