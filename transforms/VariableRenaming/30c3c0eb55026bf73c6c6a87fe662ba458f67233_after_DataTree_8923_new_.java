...
public class DataTree {
    ...
    void killSession(long var0, long zxid) {
        // are again called from FinalRequestProcessor in sequence.
        HashSet<String> list = ephemerals.remove(var0);
        if (list != null) {
            for (String path : list) {
                try {
                    deleteNode(path, zxid);
                    ZooTrace.logTraceMessage(LOG, ZooTrace.SESSION_TRACE_MASK, "Deleting ephemeral node " + path + " for session 0x" + Long.toHexString(var0));
                } catch (KeeperException e) {
                    LOG.error("FIXMSG", e);
                }
            }
