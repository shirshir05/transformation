...
public class NIOServerCnxn implements Watcher, ServerCnxn {
    ...
    void doIO(SelectionKey k) throws InterruptedException {
        try {
        ...
        } catch (CancelledKeyException var9) {
            LOG.warn("Exception causing close of session 0x" + Long.toHexString(sessionId) + " due to " + var9);
            if (LOG.isDebugEnabled()) {
                LOG.debug("CancelledKeyException stack trace", var9);
            }
            close();
        } catch (CloseRequestException var9) {
            ...
            LOG.warn("Exception causing close of session 0x" + Long.toHexString(sessionId) + " due to " + var9);
            if (LOG.isDebugEnabled()) {
                LOG.debug("IOException stack trace", var9);
            }
            close();
        }
    }
