...
public class PrepRequestProcessor extends ZooKeeperCriticalThread implements RequestProcessor {
    ...
    private void pRequest2TxnCreate(int type, Request var1, Record record, boolean deserialize) throws IOException, KeeperException {
        ...
        if (createMode.isEphemeral()) {
            s.setEphemeralOwner(var1.sessionId);
        }
        parentRecord = parentRecord.duplicate(var1.getHdr().getZxid());
        parentRecord.childCount++;
        parentRecord.stat.setCversion(newCversion);
        addChangeRecord(parentRecord);
        addChangeRecord(new ChangeRecord(var1.getHdr().getZxid(), path, s, 0, listACL));
    }
