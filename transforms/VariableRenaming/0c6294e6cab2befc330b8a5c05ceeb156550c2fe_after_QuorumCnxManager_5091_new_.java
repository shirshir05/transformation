...
public class QuorumCnxManager {
     ...
     * election uses it.
     */
    void toSend(Long var0, ByteBuffer b) {
        /*
        ...
        if (self.getId() == var0) {
            try {
                b.position(0);
                recvQueue.put(new Message(b.duplicate(), var0));
            ...
            try {
                 ...
                 */
                if (!queueSendMap.containsKey(var0)) {
                    queueSendMap.put(var0, new ArrayBlockingQueue<ByteBuffer>(CAPACITY));
                    queueSendMap.get(var0).put(b);
                } else {
                    if (queueSendMap.get(var0).remainingCapacity() == 0) {
                        queueSendMap.get(var0).take();
                    }
                    queueSendMap.get(var0).put(b);
                }
                // synchronized (senderWorkerMap) {
                if ((senderWorkerMap.get(var0) == null)) {
                    SocketChannel channel;
                    try {
                        channel = SocketChannel.open(self.quorumPeers.get(var0).electionAddr);
                        channel.socket().setTcpNoDelay(true);
                        initiateConnection(channel, var0);
                    } catch (IOException e) {
                        LOG.warn("Cannot open channel to " + var0 + "( " + e.toString() + ")");
                    }
                }
            } catch (InterruptedException e) {
