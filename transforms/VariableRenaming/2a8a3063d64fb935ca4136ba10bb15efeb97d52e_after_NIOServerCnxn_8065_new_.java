...
public class NIOServerCnxn implements Watcher, ServerCnxn {
     ...
     */
    private boolean checkFourLetterWord(final SelectionKey k, final int var1) throws IOException {
        // for cmds. They are all 4-bytes which fits inside of an int
        String cmd = cmd2String.get(var1);
        if (cmd == null) {
            return false;
        }
        ...
        if (var1 == ruokCmd) {
            ...
            ruok.start();
            return true;
        } else if (var1 == getTraceMaskCmd) {
            TraceMaskCommand tmask = new TraceMaskCommand(pwriter);
            tmask.start();
            return true;
        } else if (var1 == setTraceMaskCmd) {
            int rc = sock.read(incomingBuffer);
            if (rc < 0) {
            ...
            setMask.start();
            return true;
        } else if (var1 == enviCmd) {
            EnvCommand env = new EnvCommand(pwriter);
            env.start();
            return true;
        } else if (var1 == confCmd) {
            ConfCommand ccmd = new ConfCommand(pwriter);
            ccmd.start();
            return true;
        } else if (var1 == srstCmd) {
            StatResetCommand strst = new StatResetCommand(pwriter);
            strst.start();
            return true;
        } else if (var1 == crstCmd) {
            CnxnStatResetCommand crst = new CnxnStatResetCommand(pwriter);
            crst.start();
            return true;
        } else if (var1 == dumpCmd) {
            DumpCommand dump = new DumpCommand(pwriter);
            dump.start();
            return true;
        } else if (var1 == statCmd || var1 == srvrCmd) {
            StatCommand stat = new StatCommand(pwriter, var1);
            stat.start();
            return true;
        } else if (var1 == consCmd) {
            ConsCommand cons = new ConsCommand(pwriter);
            cons.start();
            return true;
        } else if (var1 == wchpCmd || var1 == wchcCmd || var1 == wchsCmd) {
            WatchCommand wcmd = new WatchCommand(pwriter, var1);
            wcmd.start();
            return true;
        } else if (var1 == mntrCmd) {
