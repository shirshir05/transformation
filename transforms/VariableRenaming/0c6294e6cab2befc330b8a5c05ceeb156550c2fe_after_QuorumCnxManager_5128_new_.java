    ...
    class SendWorker extends Thread {
        ...
        public void run() {
            while (running && !shutdown) {
                ...
                try {
                    var0 = queueSendMap.get(sid).take();
                } catch (InterruptedException e) {
                    LOG.warn("Interrupted while waiting for message on queue (" + e.toString() + ")");
                    continue;
                }
                try {
                    byte[] msgBytes = new byte[var0.capacity() + (Integer.SIZE / 8)];
                    ByteBuffer msgBuffer = ByteBuffer.wrap(msgBytes);
                    msgBuffer.putInt(var0.capacity());
                    msgBuffer.put(var0.array(), 0, var0.capacity());
                    msgBuffer.position(0);
                    ...
                    synchronized (senderWorkerMap) {
                        ...
                        if (queueSendMap.get(sid).size() == 0)
                            queueSendMap.get(sid).offer(var0);
                    }
                }
            }
