    ...
    protected void pRequest2Txn(int type, long zxid, Request request, Record record) throws KeeperException {
        ...
        switch(type) {
            ...
            case OpCode.delete:
                ...
                ChangeRecord var17 = getRecordForPath(path);
                checkACL(zks, parentRecord.acl, ZooDefs.Perms.DELETE, request.authInfo);
                int version = deleteRequest.getVersion();
                if (version != -1 && var17.stat.getVersion() != version) {
                    throw new KeeperException.BadVersionException(path);
                }
                if (var17.childCount > 0) {
            ...
            case OpCode.setData:
                ...
                var17 = getRecordForPath(path);
                checkACL(zks, var17.acl, ZooDefs.Perms.WRITE, request.authInfo);
                version = setDataRequest.getVersion();
                int currentVersion = var17.stat.getVersion();
                if (version != -1 && version != currentVersion) {
                    throw new KeeperException.BadVersionException(path);
                }
                version = currentVersion + 1;
                request.txn = new SetDataTxn(path, setDataRequest.getData(), version);
                var17 = var17.duplicate(request.hdr.getZxid());
                var17.stat.setVersion(version);
                addChangeRecord(var17);
                break;
            case OpCode.setACL:
                zks.sessionTracker.checkSession(request.sessionId, request.getOwner());
                ...
                var17 = getRecordForPath(path);
                checkACL(zks, var17.acl, ZooDefs.Perms.ADMIN, request.authInfo);
                version = setAclRequest.getVersion();
                currentVersion = var17.stat.getAversion();
                if (version != -1 && version != currentVersion) {
                    throw new KeeperException.BadVersionException(path);
                }
                version = currentVersion + 1;
                ...
                var17 = var17.duplicate(request.hdr.getZxid());
                var17.stat.setAversion(version);
                addChangeRecord(var17);
                break;
            ...
            case OpCode.check:
                ...
                var17 = getRecordForPath(path);
                checkACL(zks, var17.acl, ZooDefs.Perms.READ, request.authInfo);
                version = checkVersionRequest.getVersion();
                currentVersion = var17.stat.getVersion();
                if (version != -1 && version != currentVersion) {
                    throw new KeeperException.BadVersionException(path);
                }
                version = currentVersion + 1;
