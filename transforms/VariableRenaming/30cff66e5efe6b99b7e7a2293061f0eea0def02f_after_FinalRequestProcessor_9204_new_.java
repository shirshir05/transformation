        ...
        try {
            ...
            switch(request.type) {
                ...
                case OpCode.getChildren2:
                    {
                        lastOp = "GETC";
                        GetChildren2Request var30 = new GetChildren2Request();
                        ZooKeeperServer.byteBuffer2Record(request.request, var30);
                        Stat stat = new Stat();
                        DataNode n = zks.getZKDatabase().getNode(var30.getPath());
                        if (n == null) {
                            throw new KeeperException.NoNodeException();
                        }
                        Long aclG;
                        ...
                        PrepRequestProcessor.checkACL(zks, zks.getZKDatabase().convertLong(aclG), ZooDefs.Perms.READ, request.authInfo);
                        List<String> children = zks.getZKDatabase().getChildren(var30.getPath(), stat, var30.getWatch() ? cnxn : null);
