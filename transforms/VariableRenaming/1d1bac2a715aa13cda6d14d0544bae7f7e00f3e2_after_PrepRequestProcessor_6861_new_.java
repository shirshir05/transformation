    ...
    protected void pRequest2Txn(int type, long zxid, Request request, Record record) throws KeeperException {
        ...
        switch(type) {
            ...
            case OpCode.delete:
                ...
                ChangeRecord var18 = getRecordForPath(path);
                checkACL(zks, parentRecord.acl, ZooDefs.Perms.DELETE, request.authInfo);
                int version = deleteRequest.getVersion();
                if (version != -1 && var18.stat.getVersion() != version) {
                    throw new KeeperException.BadVersionException(path);
                }
                if (var18.childCount > 0) {
            ...
            case OpCode.setData:
                ...
                var18 = getRecordForPath(path);
                checkACL(zks, var18.acl, ZooDefs.Perms.WRITE, request.authInfo);
                version = setDataRequest.getVersion();
                int currentVersion = var18.stat.getVersion();
                if (version != -1 && version != currentVersion) {
                    throw new KeeperException.BadVersionException(path);
                }
                version = currentVersion + 1;
                request.txn = new SetDataTxn(path, setDataRequest.getData(), version);
                var18 = var18.duplicate(request.hdr.getZxid());
                var18.stat.setVersion(version);
                addChangeRecord(var18);
                break;
            case OpCode.setACL:
                zks.sessionTracker.checkSession(request.sessionId, request.getOwner());
                ...
                var18 = getRecordForPath(path);
                checkACL(zks, var18.acl, ZooDefs.Perms.ADMIN, request.authInfo);
                version = setAclRequest.getVersion();
                currentVersion = var18.stat.getAversion();
                if (version != -1 && version != currentVersion) {
                    throw new KeeperException.BadVersionException(path);
                }
                version = currentVersion + 1;
                ...
                var18 = var18.duplicate(request.hdr.getZxid());
                var18.stat.setAversion(version);
                addChangeRecord(var18);
                break;
            ...
            case OpCode.check:
                ...
                var18 = getRecordForPath(path);
                checkACL(zks, var18.acl, ZooDefs.Perms.READ, request.authInfo);
                version = checkVersionRequest.getVersion();
                currentVersion = var18.stat.getVersion();
                if (version != -1 && version != currentVersion) {
                    throw new KeeperException.BadVersionException(path);
                }
                version = currentVersion + 1;
