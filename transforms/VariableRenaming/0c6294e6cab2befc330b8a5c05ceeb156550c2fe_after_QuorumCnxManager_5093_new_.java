...
class QuorumCnxManager {
    ...
    void genChallenge() {
        Random rand = new Random(System.currentTimeMillis() + localIP.hashCode());
        ...
        if (!wins) {
            ...
            synchronized (senderWorkerMap) {
                ...
                if (s != null) {
                    ...
                    sw.setRecv(rw);
                    if (senderWorkerMap.containsKey(s.socket().getInetAddress())) {
        ...
        if (addr.equals(localIP)) {
            ...
            try {
                ...
                if (!queueSendMap.containsKey(addr)) {
                    queueSendMap.put(addr, new ArrayBlockingQueue<ByteBuffer>(CAPACITY));
                ...
                synchronized (senderWorkerMap) {
                    if (senderWorkerMap.get(addr) == null) {
                        ...
                        try {
                            ...
                            LOG.warn("Cannot open channel to " + addr.toString() + "( " + e.toString() + ")");
                        }
                    }
                }
            } catch (InterruptedException e) {
        ...
        public void run() {
            while (running && !shutdown) {
                ...
                try {
                     ...
                     */
                    LOG.warn("Exception when using channel: " + addr + ")" + e.toString());
                    running = false;
                    synchronized (senderWorkerMap) {
                        recvWorker.finish();
                        recvWorker = null;
                        senderWorkerMap.remove(channel.socket().getInetAddress());
                        if (queueSendMap.get(channel.socket().getInetAddress()).size() == 0)
                            queueSendMap.get(channel.socket().getInetAddress()).offer(b);
                    }
                }
            }
                ...
                while (running && !shutdown && channel.isConnected()) {
                    ...
                    if (length > 0) {
                        ...
                        synchronized (recvQueue) {
                            recvQueue.put(new Message(message.duplicate(), addr));
                        }
                        msgLength.position(0);
                    }
                }
