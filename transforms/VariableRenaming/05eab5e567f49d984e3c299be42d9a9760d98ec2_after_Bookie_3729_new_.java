...
public class Bookie extends Thread {
    ...
    private LedgerDescriptor getHandle(long var0, boolean readonly, byte[] masterKey) throws IOException {
        LedgerDescriptor handle = null;
        synchronized (ledgers) {
            handle = ledgers.get(var0);
            if (handle == null) {
                handle = createHandle(var0, readonly);
                ledgers.put(var0, handle);
                masterKeys.put(var0, ByteBuffer.wrap(masterKey));
            }
            handle.incRef();
        }
        return handle;
    ...
    static class QueueEntry {
        ...
        QueueEntry(ByteBuffer entry, long ledgerId, long entryId, AddCallback cb, Object ctx) {
    ...
    public void run() {
        ...
        try {
            FileChannel logFile = new RandomAccessFile(new File(journalDirectory, Long.toHexString(System.currentTimeMillis()) + ".txn"), "rw").getChannel();
            zeros.clear();
    ...
    public void addEntry(ByteBuffer entry, AddCallback cb, Object ctx, byte[] masterKey) throws IOException, BookieException {
