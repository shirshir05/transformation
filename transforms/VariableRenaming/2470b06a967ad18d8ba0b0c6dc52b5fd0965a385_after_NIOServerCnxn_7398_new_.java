                ...
                try {
                    ...
                    for (SelectionKey k : selectedList) {
                        if ((k.readyOps() & SelectionKey.OP_ACCEPT) != 0) {
                            SocketChannel var3 = ((ServerSocketChannel) k.channel()).accept();
                            InetAddress ia = var3.socket().getInetAddress();
                            int cnxncount = getClientCnxnCount(ia);
                            if (maxClientCnxns > 0 && cnxncount >= maxClientCnxns) {
                                LOG.warn("Too many connections from " + ia + " - max is " + maxClientCnxns);
                                var3.close();
                            } else {
                                LOG.info("Accepted socket connection from " + var3.socket().getRemoteSocketAddress());
                                var3.configureBlocking(false);
                                SelectionKey sk = var3.register(selector, SelectionKey.OP_READ);
                                NIOServerCnxn cnxn = createConnection(var3, sk);
                                sk.attach(cnxn);
                                addCnxn(cnxn);
                            }
