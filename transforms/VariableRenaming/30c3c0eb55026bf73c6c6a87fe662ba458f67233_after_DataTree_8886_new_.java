...
public class DataTree {
    ...
    public void deleteNode(String path, long zxid) throws KeeperException.NoNodeException {
        int lastSlash = path.lastIndexOf('/');
        String var3 = path.substring(0, lastSlash);
        String childName = path.substring(lastSlash + 1);
        ...
        nodes.remove(path);
        DataNode parent = nodes.get(var3);
        if (parent == null) {
            throw new KeeperException.NoNodeException();
        }
        synchronized (parent) {
        ...
        ZooTrace.logTraceMessage(LOG, ZooTrace.EVENT_DELIVERY_TRACE_MASK, "childWatches.triggerWatch " + var3);
        Set<Watcher> processed = dataWatches.triggerWatch(path, EventType.NodeDeleted);
        childWatches.triggerWatch(path, EventType.NodeDeleted, processed);
        childWatches.triggerWatch(var3.equals("") ? "/" : var3, EventType.NodeChildrenChanged);
    }
