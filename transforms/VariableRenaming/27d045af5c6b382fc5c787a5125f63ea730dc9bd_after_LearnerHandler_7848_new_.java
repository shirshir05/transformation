        ...
        try {
            ...
            if (learnerType == LearnerType.PARTICIPANT) {
                ...
                if (this.getVersion() < 0x10000) {
                    ...
                    bufferedOutput.flush();
                    QuorumPacket var12 = new QuorumPacket();
                    ia.readRecord(var12, "packet");
                    if (var12.getType() != Leader.ACKEPOCH) {
                        LOG.error(var12.toString() + " is not ACKEPOCH");
                        return;
                    }
                    ByteBuffer bbepoch = ByteBuffer.wrap(var12.getData());
                    ss = new StateSummary(bbepoch.getInt(), var12.getZxid());
                    leader.waitForEpochAck(this.getSid(), ss);
                }
