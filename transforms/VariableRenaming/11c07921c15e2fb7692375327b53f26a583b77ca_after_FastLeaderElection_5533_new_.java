    ...
    public Vote lookForLeader() throws InterruptedException {
        ...
        try {
            ...
            while ((self.getPeerState() == ServerState.LOOKING) && (!stop)) {
                 ...
                 */
                Notification var5 = recvqueue.poll(notTimeout, TimeUnit.MILLISECONDS);
                /*
                ...
                if (var5 == null) {
                ...
                } else if (validVoter(var5.sid) && validVoter(var5.leader)) {
                    /*
                    ...
                    switch(var5.state) {
                        case LOOKING:
                            ...
                            if (var5.zxid == -1) {
                                LOG.debug("Ignoring notification from member with -1 zxid {}", var5.sid);
                                break;
                            }
                            // If notification > current, replace and send messages out
                            if (var5.electionEpoch > logicalclock.get()) {
                                logicalclock.set(var5.electionEpoch);
                                recvset.clear();
                                if (totalOrderPredicate(var5.leader, var5.zxid, var5.peerEpoch, getInitId(), getInitLastLoggedZxid(), getPeerEpoch())) {
                                    updateProposal(var5.leader, var5.zxid, var5.peerEpoch);
                                } else {
                                    updateProposal(getInitId(), getInitLastLoggedZxid(), getPeerEpoch());
                                }
                                sendNotifications();
                            } else if (var5.electionEpoch < logicalclock.get()) {
                                LOG.debug("Notification election epoch is smaller than logicalclock. n.electionEpoch = 0x{}, logicalclock=0x{}", Long.toHexString(var5.electionEpoch), Long.toHexString(logicalclock.get()));
                                break;
                            } else if (totalOrderPredicate(var5.leader, var5.zxid, var5.peerEpoch, proposedLeader, proposedZxid, proposedEpoch)) {
                                updateProposal(var5.leader, var5.zxid, var5.peerEpoch);
                                sendNotifications();
                            }
                            LOG.debug("Adding vote: from={}, proposed leader={}, proposed zxid=0x{}, proposed election epoch=0x{}", var5.sid, var5.leader, Long.toHexString(var5.zxid), Long.toHexString(var5.electionEpoch));
                            // don't care about the version if it's in LOOKING state
                            recvset.put(var5.sid, new Vote(var5.leader, var5.zxid, var5.electionEpoch, var5.peerEpoch));
                            voteSet = getVoteTracker(recvset, new Vote(proposedLeader, proposedZxid, logicalclock.get(), proposedEpoch));
                            if (voteSet.hasAllQuorums()) {
                                // Verify if there is any change in the proposed leader
                                while ((var5 = recvqueue.poll(finalizeWait, TimeUnit.MILLISECONDS)) != null) {
                                    if (totalOrderPredicate(var5.leader, var5.zxid, var5.peerEpoch, proposedLeader, proposedZxid, proposedEpoch)) {
                                        recvqueue.put(var5);
                                        break;
                                    }
                                }
                            ...
                            break;
                        case OBSERVING:
                            LOG.debug("Notification from observer: {}", var5.sid);
                            break;
                        case FOLLOWING:
                        case LEADING:
                            /*
                         ...
                         */
                            if (var5.electionEpoch == logicalclock.get()) {
                                recvset.put(var5.sid, new Vote(var5.leader, var5.zxid, var5.electionEpoch, var5.peerEpoch, var5.state));
                                voteSet = getVoteTracker(recvset, new Vote(var5.version, var5.leader, var5.zxid, var5.electionEpoch, var5.peerEpoch, var5.state));
                                if (voteSet.hasAllQuorums() && checkLeader(recvset, var5.leader, var5.electionEpoch)) {
                                    setPeerState(var5.leader, voteSet);
                                    Vote endVote = new Vote(var5.leader, var5.zxid, var5.electionEpoch, var5.peerEpoch);
                                    leaveInstance(endVote);
                                    return endVote;
                                }
                            }
                         ...
                         */
                            outofelection.put(var5.sid, new Vote(var5.version, var5.leader, var5.zxid, var5.electionEpoch, var5.peerEpoch, var5.state));
                            voteSet = getVoteTracker(outofelection, new Vote(var5.version, var5.leader, var5.zxid, var5.electionEpoch, var5.peerEpoch, var5.state));
                            if (voteSet.hasAllQuorums() && checkLeader(outofelection, var5.leader, var5.electionEpoch)) {
                                synchronized (this) {
                                    logicalclock.set(var5.electionEpoch);
                                    setPeerState(var5.leader, voteSet);
                                }
                            ...
                            break;
                        default:
                            LOG.warn("Notification state unrecoginized: {} (n.state), {}(n.sid)", var5.state, var5.sid);
                            break;
                    }
                } else {
                    if (!validVoter(var5.leader)) {
                        LOG.warn("Ignoring notification for non-cluster member sid {} from sid {}", var5.leader, var5.sid);
                    }
                    if (!validVoter(var5.sid)) {
                        LOG.warn("Ignoring notification for sid {} from non-quorum member sid {}", var5.leader, var5.sid);
                    }
                }
            }
            return null;
