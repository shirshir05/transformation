        ...
        public void run() {
            ...
            while (zooKeeper.state.isAlive()) {
                try {
                    ...
                    for (SelectionKey var8 : selected) {
                        SocketChannel sc = ((SocketChannel) var8.channel());
                        if ((var8.readyOps() & SelectionKey.OP_CONNECT) != 0) {
                            if (sc.finishConnect()) {
                                zooKeeper.state = States.CONNECTED;
                                lastHeard = now;
                                lastSend = now;
                                primeConnection(var8);
                            }
                        } else if ((var8.readyOps() & (SelectionKey.OP_READ | SelectionKey.OP_WRITE)) != 0) {
                            if (outgoingQueue.size() > 0) {
