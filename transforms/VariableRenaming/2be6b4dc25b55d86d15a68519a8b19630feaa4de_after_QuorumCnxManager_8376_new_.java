...
public class QuorumCnxManager {
     ...
     */
    public void toSend(Long var0, ByteBuffer b) {
        /*
         * If sending message to myself, then simply enqueue it (loopback).
         */
        if (self.getId() == var0) {
            b.position(0);
            addToRecvQueue(new Message(b.duplicate(), var0));
        } else {
            /*
            ...
            if (!queueSendMap.containsKey(var0)) {
                ...
                queueSendMap.put(var0, bq);
                addToSendQueue(bq, b);
            } else {
                ArrayBlockingQueue<ByteBuffer> bq = queueSendMap.get(var0);
                if (bq != null) {
                    addToSendQueue(bq, b);
                } else {
                    LOG.error("No queue for server " + var0);
                }
            }
            connectOne(var0);
        }
