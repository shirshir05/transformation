...
public class Leader {
     ...
     */
    synchronized public long startForwarding(LearnerHandler var0, long lastSeenZxid) {
        // new requests
        if (lastProposed > lastSeenZxid) {
            ...
            for (Long zxid : zxids) {
                ...
                var0.queuePacket(outstandingProposals.get(zxid).packet);
            }
        }
        if (var0.getLearnerType() == LearnerType.PARTICIPANT) {
            addForwardingFollower(var0);
        } else {
            addObserverLearnerHandler(var0);
        }
        return lastProposed;
    }
