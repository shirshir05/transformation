...
class QuorumPeerInstance implements Instance {
    ...
    public void configure(String var0) {
        if (clientAddr == null) {
            String[] parts = var0.split(" ");
            ...
            int spaceIndex = var0.indexOf(' ');
            if (spaceIndex == -1) {
                LOG.warn("looking for host:port,... start|stop, but found " + var0);
                return;
            }
            String quorumSpecs = var0.substring(0, spaceIndex);
            String cmd = var0.substring(spaceIndex + 1);
            if (LOG.isDebugEnabled()) {
