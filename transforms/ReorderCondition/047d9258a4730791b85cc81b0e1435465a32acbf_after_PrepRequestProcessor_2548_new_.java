...
public class PrepRequestProcessor extends Thread implements RequestProcessor {
    ...
    public void run() {
        ...
        ZooLog.logTextTraceMessage("PrepRequestProcessor exited loop!", ZooLog.textTraceMask);
    }
    ...
    ChangeRecord getRecordForPath(String path) throws KeeperException {
        ...
        synchronized (zks.outstandingChanges) {
            ...
            if (lastChange == null) {
                ...
                if (n != null) {
                    lastChange = new ChangeRecord(-1, path, n.stat, n.children.size(), n.acl);
                }
            }
        }
    ...
    static void checkACL(ZooKeeperServer zks, ArrayList<ACL> acl, int perm, ArrayList<Id> ids) throws KeeperException {
        if (skipACL) {
            return;
        }
        ...
        for (ACL a : acl) {
            ...
            if ((a.getPerms() & perm) != 0) {
                ...
                AuthenticationProvider ap = zks.authenticationProviders.get(id.getScheme());
                if (ap != null) {
                    for (Id authId : ids) {
                        if (authId.getScheme().equals("super")) {
                            return;
                        }
                        if (ap.matches(authId.getId(), id.getId()) && authId.getScheme().equals(id.getScheme())) {
                            return;
                        }
                    }
                }
        ...
        try {
            switch(request.type) {
                case OpCode.create:
                    ...
                    txn = new CreateTxn(path, createRequest.getData(), createRequest.getAcl(), (createRequest.getFlags() & CreateFlags.EPHEMERAL) != 0);
                    Stat s = new Stat();
                ...
                case OpCode.closeSession:
                    ...
                    ZooLog.logWarn("Processed session termination request for id: " + Long.toHexString(request.sessionId));
                    break;
                case OpCode.exists:
                case OpCode.getData:
                case OpCode.getACL:
    ...
    private boolean fixupACL(ArrayList<Id> authInfo, ArrayList<ACL> acl) {
        ...
        while (it.hasNext()) {
            ...
            if (id.getScheme().equals("world") && id.getId().equals("anyone")) {
                ...
                for (Id cid : authInfo) {
                    AuthenticationProvider ap = zks.authenticationProviders.get(cid.getScheme());
                    if (ap == null) {
                        ZooLog.logError("Missing AuthenticationProvider for " + cid.getScheme());
