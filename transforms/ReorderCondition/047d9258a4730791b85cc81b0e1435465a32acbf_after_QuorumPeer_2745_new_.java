...
public class QuorumPeer extends Thread {
            ...
            try {
                ...
                while (true) {
                    ...
                    if (packet.getLength() != 4) {
                        ZooLog.logError("Got more than just an xid! Len = " + packet.getLength());
                    } else {
    ...
    public QuorumPeer(ArrayList<QuorumServer> quorumPeers, File dataDir, File dataLogDir, int clientPort, int electionAlg, long myid, int tickTime, int initLimit, int syncLimit) throws IOException {
        super("QuorumPeer");
    ...
    long getLastLoggedZxid() {
        ...
        for (File f : list) {
            ...
            if (name.startsWith("log.")) {
                ...
                long zxid = ZooKeeperServer.getZxidFromName(f.getName(), "snapshot");
                if (zxid > maxLog) {
                    maxSnapShot = zxid;
                }
            }
        ...
        try {
            logStream = new FileInputStream(new File(dataLogDir, "log." + Long.toHexString(maxLog)));
            ...
            while (true) {
                ...
                InputArchive bia = BinaryInputArchive.getArchive(new ByteArrayInputStream(bytes));
        ...
        try {
            ...
            if (servers.size() > 1) {
                ...
                try {
                    ...
                    ZooLog.logError(" is not a number" + myIdString);
                    System.exit(2);
                    // makes the id not initialized warning go away...
                    return;
                }
                QuorumPeer self = new QuorumPeer(servers, dataDir, dataLogDir, clientPort, electionAlg, id, tickTime, initLimit, syncLimit);
                self.start();
                self.join();
            } else {
                int port = clientPort;
