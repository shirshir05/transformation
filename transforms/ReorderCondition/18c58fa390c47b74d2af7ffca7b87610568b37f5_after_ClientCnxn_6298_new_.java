        ...
        public void run() {
            ...
            while (state.isAlive()) {
                try {
                    ...
                    if (to <= 0) {
                        throw new SessionTimeoutException("ms" + "Client session timed out, have not heard from server in " + clientCnxnSocket.getIdleRecv() + " for sessionid 0x" + Long.toHexString(sessionId));
                    }
                    if (state == States.CONNECTED) {
