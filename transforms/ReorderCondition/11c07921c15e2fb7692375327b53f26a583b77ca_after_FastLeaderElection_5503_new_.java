...
public class FastLeaderElection implements Election {
    ...
    private void sendNotifications() {
        for (long sid : self.getCurrentAndNextConfigVoters()) {
            ...
            LOG.debug(" {} (myid), 0x{} (n.peerEpoch) " + "Sending Notification: {} (n.leader), 0x{} (n.zxid), 0x{} (n.round), {} (recipient),", proposedLeader, Long.toHexString(proposedZxid), Long.toHexString(logicalclock.get()), sid, self.getId(), Long.toHexString(proposedEpoch));
            sendqueue.offer(notmsg);
        }
    }
