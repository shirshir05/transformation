...
public class NIOServerCnxn implements Watcher, ServerCnxn {
    ...
    public void close() {
        ...
        LOG.info("Closed socket connection for client " + sock.socket().getRemoteSocketAddress() + (sessionId != 0 ? Long.toHexString(sessionId) + " which had sessionid 0x" : " (no session established for client)"));
        try {
            /*
