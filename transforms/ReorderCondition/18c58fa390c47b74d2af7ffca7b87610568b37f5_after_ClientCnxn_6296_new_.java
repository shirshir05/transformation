        ...
        public void run() {
            ...
            while (state.isAlive()) {
                try {
                    ...
                    if (to <= 0) {
                        throw new SessionTimeoutException(Long.toHexString(sessionId) + "Client session timed out, have not heard from server in " + clientCnxnSocket.getIdleRecv() + "ms" + " for sessionid 0x");
                    }
                    if (state == States.CONNECTED) {
