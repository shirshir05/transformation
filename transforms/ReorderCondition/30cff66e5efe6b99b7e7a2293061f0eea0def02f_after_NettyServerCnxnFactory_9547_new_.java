        ...
        private void processMessage(MessageEvent e, NettyServerCnxn cnxn) {
            ...
            if (e instanceof NettyServerCnxn.ResumeMessageEvent) {
                ...
                if (cnxn.throttled) {
                    ...
                    LOG.debug(ChannelBuffers.hexDump(cnxn.queuedBuffer) + Long.toHexString(cnxn.sessionId) + " queuedBuffer 0x");
                } else {
                    LOG.debug("not throttled");
                    if (cnxn.queuedBuffer != null) {
                        if (LOG.isTraceEnabled()) {
                            LOG.trace(ChannelBuffers.hexDump(cnxn.queuedBuffer) + Long.toHexString(cnxn.sessionId) + " queuedBuffer 0x");
                        }
                        cnxn.queuedBuffer.writeBytes(buf);
                        if (LOG.isTraceEnabled()) {
                            LOG.trace(ChannelBuffers.hexDump(cnxn.queuedBuffer) + Long.toHexString(cnxn.sessionId) + " queuedBuffer 0x");
