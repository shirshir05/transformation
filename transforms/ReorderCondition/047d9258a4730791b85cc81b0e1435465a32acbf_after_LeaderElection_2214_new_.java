    ...
    private ElectionResult countVotes(HashMap<InetSocketAddress, Vote> votes) {
        ...
        for (Vote v : votesCast) {
            ...
            if (v.id == result.vote.id) {
                result.count++;
            } else if (v.zxid > result.vote.zxid || (v.zxid == result.vote.zxid && v.id > result.vote.id)) {
                result.vote = v;
                result.count = 1;
            }
        }
        ...
        while (self.running) {
            ...
            for (QuorumServer server : self.quorumPeers) {
                ...
                try {
                    ...
                    if (responsePacket.getLength() != responseBytes.length) {
                        ZooLog.logError("Got a short response: " + responsePacket.getLength());
                        continue;
                    }
                    responseBuffer.clear();
                    int recvedXid = responseBuffer.getInt();
                    if (recvedXid != xid) {
                        ZooLog.logError("Got bad xid: expected " + xid + " got " + recvedXid);
                        continue;
                    }
            ...
            if (result.winner.id >= 0) {
                ...
                if (result.winningCount > (self.quorumPeers.size() / 2)) {
                    ...
                    s.close();
                    self.state = (self.currentVote.id == self.myid) ? ServerState.LEADING : ServerState.FOLLOWING;
