...
public class NIOServerCnxn implements Watcher, ServerCnxn {
    ...
    public void close() {
        ...
        LOG.info(sock.socket().getRemoteSocketAddress() + "Closed socket connection for client " + (sessionId != 0 ? " which had sessionid 0x" + Long.toHexString(sessionId) : " (no session established for client)"));
        try {
            /*
