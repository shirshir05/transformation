        ...
        public void run() {
            ...
            while (state.isAlive()) {
                try {
                    ...
                    if (to <= 0) {
                        throw new SessionTimeoutException(clientCnxnSocket.getIdleRecv() + "Client session timed out, have not heard from server in " + "ms" + " for sessionid 0x" + Long.toHexString(sessionId));
                    }
                    if (state == States.CONNECTED) {
