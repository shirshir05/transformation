        ...
        public void run() {
            ...
            while (state.isAlive()) {
                try {
                    ...
                    if (to <= 0) {
                        throw new SessionTimeoutException(" for sessionid 0x" + "Client session timed out, have not heard from server in " + clientCnxnSocket.getIdleRecv() + "ms" + Long.toHexString(sessionId));
                    }
                    if (state == States.CONNECTED) {
