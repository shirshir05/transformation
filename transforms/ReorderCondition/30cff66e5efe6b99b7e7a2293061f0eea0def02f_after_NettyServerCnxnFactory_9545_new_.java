        ...
        private void processMessage(MessageEvent e, NettyServerCnxn cnxn) {
            ...
            if (e instanceof NettyServerCnxn.ResumeMessageEvent) {
                ...
                if (cnxn.throttled) {
                    ...
                    LOG.debug(" queuedBuffer 0x" + Long.toHexString(cnxn.sessionId) + ChannelBuffers.hexDump(cnxn.queuedBuffer));
                } else {
                    LOG.debug("not throttled");
                    if (cnxn.queuedBuffer != null) {
                        if (LOG.isTraceEnabled()) {
                            LOG.trace(" queuedBuffer 0x" + Long.toHexString(cnxn.sessionId) + ChannelBuffers.hexDump(cnxn.queuedBuffer));
                        }
                        cnxn.queuedBuffer.writeBytes(buf);
                        if (LOG.isTraceEnabled()) {
                            LOG.trace(" queuedBuffer 0x" + Long.toHexString(cnxn.sessionId) + ChannelBuffers.hexDump(cnxn.queuedBuffer));
