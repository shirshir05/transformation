...
public class ZooKeeper {
    ...
    public void processWatchEvent(WatcherEvent event) {
        ...
        } else if (event.getType() == Watcher.Event.EventNodeCreated) {
            synchronized (dataWatches) {
                watchers = dataWatches.remove(event.getPath());
            }
        } else if (event.getType() == Watcher.Event.EventNodeChildrenChanged) {
            synchronized (childWatches) {
                watchers = childWatches.remove(event.getPath());
            }
        } else if (event.getType() == Watcher.Event.EventNodeDeleted) {
            synchronized (dataWatches) {
                watchers = dataWatches.remove(event.getPath());
            }
